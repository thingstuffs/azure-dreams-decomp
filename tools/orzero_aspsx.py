...
pinned with `register s32 zero ASM_REG("$0")` carriers (`zero | K`).  A static census of retail bytes
...
row) plus this rewrite; 20 rows / 38 pins, all landed.
  ...
  `({ register s32 zero ASM_REG("$0"); [ASM_KEEP(zero);] zero | (K); })` -> K
  a `register s32 NAME ASM_REG("$0")` whose every use is `NAME | K` -> dropped, uses become K; its
      companion statements ASM_UNDEF(NAME); READ_ZERO(NAME); ASM_KEEP(NAME); and NON_MATCHING
      `NAME = 0;` arms go with it
  NON_MATCHING arms of the declaration (`s32 NAME = 0;`, `#define NAME 0`) -> folded
...
SX = re.compile(r'\(\{\s*register s32 (\w+) ASM_REG\("\$0"\);\s*(?:ASM_KEEP\(\1\);\s*)?\1 \| \(([^()]*)\);\s*\}\)')
BAD = re.compile(r'#ifdef NON_MATCHING\n#define \(\{[^\n]*\n#else\n#endif\n\n?')
REGDECL = r'[ \t]*register s32 (\w+) ASM_REG\("\$0"\);[^\n]*\n'
FOLD = [
    re.compile(r'#ifndef NON_MATCHING\n' + REGDECL + r'#else\n#define \1 0\n#endif\n'),
    re.compile(r'#ifndef NON_MATCHING\n' + REGDECL + r'#else\n[ \t]*s32 \1 = 0;\n#endif\n'),
]
FOLD2 = re.compile(r'#ifdef NON_MATCHING\n[ \t]*s32 (\w+) = 0;\n#else\n([ \t]*)register s32 \1 ASM_REG\("\$0"\);[^\n]*\n#endif\n')
...
def drop_name(t, name, scored_only=False):
    ...
    u = re.sub(r'^[ \t]*register s32 ' + name + r' ASM_REG\("\$0"\);[^\n]*\n', '', t, flags=re.M)
    u = re.sub(r'^[ \t]*(?:ASM_UNDEF|READ_ZERO|ASM_KEEP)\(' + name + r'\);[^\n]*\n', '', u, flags=re.M)
    if not scored_only:
    ...
    return re.sub(r'^(?!#).*$', lambda l: fix(l.group(0)), u, flags=re.M)
...
def rewrite(t, scored_only=False):
    ...
    t = re.sub(r'^(?!#).*$', lambda l: SX.sub(lambda m: m.group(2), l.group(0)), t, flags=re.M)   # never inside a directive
    if scored_only:
        t = re.sub(r'(#ifndef NON_MATCHING\n)' + REGDECL, r'\1', t)          # matching arm only
        t = re.sub(r'(#ifdef NON_MATCHING\n[ \t]*s32 (\w+) = 0;\n#else\n)[ \t]*register s32 \2 ASM_REG\("\$0"\);[^\n]*\n', r'\1', t)
        for name in dict.fromkeys(re.findall(r'\b(\w+) \|', _scored(t))):
            if re.search(r'(?:#define \b' + name + r'\b 0|\bs32 ' + name + r' = 0;)', t) or re.search(r'register s32 ' + name + r' ASM_REG\("\$0"\)', t):
    ...
    t = FOLD2.sub(lambda m: f'{m.group(2)}register s32 {m.group(1)} ASM_REG("$0");\n', t)
