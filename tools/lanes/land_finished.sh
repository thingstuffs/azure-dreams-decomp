...
# Stops when the file work/native_lane/STOP_LAND_FINISHED exists.  Never uses pgrep/pkill on its own name.
cd "$(dirname "$0")/../.."
INT=${1:-1200}; GLOB=${2:-"r6[5-9]_* r7[0-9]_*"}
while [ ! -f work/native_lane/STOP_LAND_FINISHED ]; do
...
for pat in sys.argv[1:]:
    for d in sorted(glob.glob('work/native_lane/' + pat)):
        ...
        for f in glob.glob(d + '/out/*/*.c'):
            ...
            if r and os.path.exists(b) and open(b).read().strip() == sha_text(clean_path(r).read_text()): fresh += 1
