...
"""Tree-wide lone-erasure census: every live pin site erased alone, screened against the pinned listing.
    ...
    python3 tools/lanes/erase_census.py OUT.jsonl [--procs 8] [--only ROWS.txt] [--diff 4]
...
def main():
    ...
    if a and a[0] == "summary":
        return summary(a[1])
    out = Path(a.pop(0)); procs = 8; only = None; diff_max = -1
    while a:
        k = a.pop(0)
        if k == "--procs":
            procs = int(a.pop(0))
    ...
    t0 = time.time(); n = 0
    with Pool(procs, initializer=_init, initargs=(diff_max,)) as p, out.open("a") as f:
