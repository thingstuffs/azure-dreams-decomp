...
<h1>azure-clean</h1><div class="muted">pin <code>{PIN}</code> · generated {now} · refreshes every 60 s · ovmovie parked (listed, not counted)</div>"""]
    ...
    for line in (
        ...
        f"{windowless} rows outside every gate window; {nonstock} rows non-stock at the pin" + ("" if not (windowless or nonstock) else " (both must be 0)"),
        ...
        "this tree owns its toolchain, disc, row database and gates (docs/SWAPOVER.md); tools/setup.sh --check, tools/build/container_check.py --rebuild-disc",
