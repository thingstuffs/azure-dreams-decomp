...
void func_8009CE1C(void *target, s16 base_power, s16 power_bonus, s16 elements,
    ...
    s32 affinity;
    register s16 requested_bonus ASM_REG("$19");
    s32 second_element;
    s32 third_element;
    M2C_UNK affinity_shift;
    register s32 target_first_element ASM_REG("$2");
    S_8009CE1C_2 *position;
    register u16 unsigned_base = base_power;
    register s16 element_mask ASM_REG("$21") = elements;
    s32 base_shift;
    ...
    if ((target != NULL) && (func_8009D218(target, element_mask & 0xFFFF, source_flags) == 0)) {
        if (requested_bonus == 0xFF) {
            ...
            base_shift = unsigned_base << 0x10;
        } else {
            base_shift = unsigned_base << 0x10;
        }
        ASM_USE_NV(requested_bonus);
...
apply_third_element:
        if (third_element != 0) {
            ...
            target_first_element = target_elements & 1;
            if (target_first_element) {
                affinity -= 1;
                goto scale_affinity;
            }
