...
void func_8004DCA8(void *object)
{
    ...
    register void *registered_ptr ASM_REG("$5") = object;   /* UNRESOLVED C shape (pin): removing it changes the instruction count (a copy retail keeps is dropped or added); the source shape that makes it unnecessary has not been found */
    if (registered_ptr == 0) {
        registered_ptr = D_80080B68;
    }
    D_80083D98[D_80081554++] = registered_ptr;
}
