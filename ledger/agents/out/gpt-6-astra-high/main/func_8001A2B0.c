...
{
    s32 selection_state;
    register s32 return_code ASM_REG("$17");   /* MATCH pin: keeps a constant in a register as retail does */
    s32 selection;
    s32 *selected_slot;
...
case_three:
    ...
    ASM_KEEP(selected_slot);   /* MATCH pin: load-bearing for the whole function shape */
    selected_slot -= 1820;
    ASM_KEEP(selected_slot);   /* MATCH pin: load-bearing for the whole function shape */
    if (selection != 0) {
        selected_slot++;
    ...
    *selected_slot = 0;
    D_804094E8 = 0;
    ASM_KEEP(return_code);   /* MATCH pin: load-bearing for the whole function shape */
    goto fast_done;
