...
void func_800423C0(S_800423C0_Obj *obj, s16 unused, S_800423C0_Src *source)
{
  ...
  s32 direction;
  register s32 upper_bits_mask ASM_REG("$5");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
  if (source != 0)
  {
    ...
    if (direction >= 0)
    {
      register u8 *direction_page ASM_REG("$3");   /* MATCH pin: slus-diff */
