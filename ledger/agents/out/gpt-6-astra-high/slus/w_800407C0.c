...
{
    register u8 *dst ASM_REG("$16");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    u8 *column_start;
    u8 *first_row_end;
    u8 *dst_end;
    register u8 *dst_byte ASM_REG("$4");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 header ASM_REG("$6");   /* MATCH pin: slus-diff */
    s32 stride;
    s32 size;
    s32 remaining;
    s32 dst_high;
    ...
    s32 nibble;
    s32 high_nibble;
    register s32 repeat_nibble ASM_REG("$5");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 saved_byte;
    s32 column_byte;
    u32 packed;
    u8 *src;
    ...
    src = src_start;
    ASM_KEEP_NV(src);   /* MATCH pin: slus-diff */
    dst = dst_start;
    dst_high = 0;
    column_start = dst;
    header = *src++;
