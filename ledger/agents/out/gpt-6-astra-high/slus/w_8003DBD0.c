...
{
    register u8 *position ASM_REG("$19");   /* MATCH pin: slus-diff */
    u8 *render_state;
    u8 *scratch;
    s32 view_angle_x;
    s32 view_angle_y;
    ...
    if ((U8_AT(point, 0) ^ U16_AT(scratch, 0x24)) & 1) {
        ...
        ASM_KEEP_DEP_NV(point_x, saved_offset_x);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    ...
    if ((U8_AT(point, 0) ^ U16_AT(scratch, 0x24)) & 2) {
        ...
        ASM_KEEP_DEP_NV(point_y, saved_offset_y);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
