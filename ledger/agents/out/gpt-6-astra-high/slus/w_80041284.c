...
void func_80041284(void *packed_data)
{
    s32 id_or_base;
    s32 target_time;
    register s32 offset_base ASM_REG("$2");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 id_base ASM_REG("$3");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
