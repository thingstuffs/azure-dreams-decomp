...
void func_800439F8(void)
{
    if (D_80080A8A[0] != 0) {
        u8 *ram_page;
        register u32 state_word ASM_REG("$3");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        u8 state_byte_4;
        register u8 state_byte_6 ASM_REG("$5");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
