...
s32 func_80053A88(S_80053A88_Node *first_node)
{
    ...
    s32 tpage;
    register void *next_link ASM_REG("$4");   /* MATCH pin: slus-diff */
    u32 addr_mask;
    u32 tag_mask;
    node = first_node;
    draw_base_ptr = &D_80083160[0];
    ASM_KEEP_NV(draw_base_ptr);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    addr_mask = 0x00FFFFFF;
    tag_mask = 0xFF000000;
    ASM_USE_NV(addr_mask);   /* MATCH pin: slus-diff */
    ASM_USE_NV(addr_mask);   /* MATCH pin: slus-diff */
    ASM_USE_NV(addr_mask);   /* MATCH pin: slus-diff */
    ASM_USE_NV(addr_mask);   /* MATCH pin: slus-diff */
    ASM_USE_NV(addr_mask);   /* MATCH pin: slus-diff */
    ...
    ASM_KEEP(next_link);   /* MATCH pin: slus-diff */
    return 0;
}
