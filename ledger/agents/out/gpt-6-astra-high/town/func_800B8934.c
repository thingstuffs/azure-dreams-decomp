...
void func_800B6094(void *entry_data, s32 display_base, s32 row_index) {
    register S_800B6094_0 *entry ASM_REG("$19") = entry_data;   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    register void *row_display ASM_REG("$17");   /* MATCH pin: load-bearing for the whole function shape */
    S_800B6094_3 *display_node;

    text_y = (row * 0x10) + 0xD8;
    row_display = (void *) (row * 4);
    ASM_KEEP(row);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    ASM_KEEP(entry);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    ...
    ASM_KEEP(display_value);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP(row_base);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP(row);   /* MATCH pin: retail schedule: same instructions, different order without it */
