...
void func_808B3620(s8 *state) {
    ...
    s32 word_0;
    s32 word_1;
    register s32 word_2 ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    s32 word_3;

    do {
        ASM_KEEP_NV(dst_words);   /* MATCH pin: retail address form (%hi/%lo vs buffer+offset) depends on it */
        word_0 = ((S_808B3620_0 *)src_words)->unk_00;
        ASM_KEEP(word_0);   /* MATCH pin: retail address form (%hi/%lo vs buffer+offset) depends on it */
        word_1 = ((S_808B3620_0 *)src_words)->unk_04;
        ASM_KEEP(word_1);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        src_words += 4;
        ASM_KEEP(src_words);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        dst_words += 4;
