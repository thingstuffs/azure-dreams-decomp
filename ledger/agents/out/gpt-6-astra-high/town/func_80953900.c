...
void func_80020900(TownEntity *menu)
{
    ...
    u8 *data_page;
    u8 *label_page;
    register TownRecord30 *label_record ASM_REG("$20");   /* MATCH pin: retail register colouring depends on it */
    s16 *label_cursor;
    register void *text_pool ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    register TownRecord30 *text_arg ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s32 text_color;
...
state_3:
    ...
    if (*(s32 *)(input + 0x10) & 0x40) {
        register Pair04 *selected_pair ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
