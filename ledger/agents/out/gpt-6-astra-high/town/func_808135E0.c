...
s32 func_808135E0(void *first_item)
{
    ...
    u8 *pool_or_endpoint;
    u8 *line_table;
    register u8 *work_ptr ASM_REG("$4");   /* MATCH pin: retail delay-slot fill depends on it */
    u8 *state_pool;
    u8 *state_table;
    u8 *line_packet;
    u8 *after_line;
    ...
    u32 *state_head;
    u32 *state_slot;
    register u32 address_mask ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+table_offset) depends on it */
    u32 length_mask;
    s32 screen_coords[2];
    s32 *screen_y;
    register s32 *out_x ASM_REG("$6");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register s32 *out_y ASM_REG("$7");   /* MATCH pin: retail address form (%hi/%lo vs base+table_offset) depends on it */
    s32 start_depth;
    register s32 depth_sum ASM_REG("$16");   /* MATCH pin: retail delay-slot fill depends on it */
    s32 end_depth;
    s32 depth_index;
    s32 table_offset;
    u32 draw_mode;
    ...
    do {
        if ((*(u16 *)(item + 0x24) & 1) == 0) {
            ...
            *(u32 *)(line_packet + 4) = *(u32 *)(item + 0x14);
            ...
            out_y = screen_y;
            ASM_USE2(out_x, out_y);   /* MATCH pin: retail keeps a computation the compiler would drop */
            ...
            if ((u16)depth_index < 0x200) {
                ...
                *line_slot = (*line_slot & length_mask) | ((u32)line_packet & address_mask);
                ...
                work_ptr = state_pool;
                if (state_packet != 0) {
                    register u32 state_pool_limit ASM_REG("$2");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
