...
/* Updates town movement, animation, facing, and action state. */
void func_800A5FDC(u8 *state, u8 *table, void *action_context)
{
    u8 *context;
    u8 *globals;
    u8 *coordinates;
    s32 animation_choice;
    s32 tile_id;
    s32 speed;
    s32 speed_delta;
    s32 sound_interval;
    ...
    if (((S_800A5FDC_0 *)state)->unk_0A.u <= 0) {
        ...
        animation_choice = func_800374F4(2);
        func_80097844(table, (u16)animation_choice + 2);
    }

    input_flags = ((S_800A5FDC_1 *)globals)->unk_08;
    if (input_flags & 0xF000) {
        func_80094C1C(state);
        move_angle = func_80094BC8(((S_800A5FDC_1 *)globals)->unk_08,
                               ((S_800A5FDC_1 *)globals)->unk_C8.s);
        if (move_angle != -1) {
            func_80094F58(move_angle, D_80100E20[0], table);
        }
        animation_choice = func_800374F4(1);
        func_80097844(table, (u16)animation_choice);
        goto after_updates;
    }
...
after_updates:
    height_limit = func_80095978(table, D_800FE488);
    if (((Rec_D_800E3D7C *)table)->unk_08.at02u.v >= height_limit) {
        func_80095A94(table, height_limit, D_800FE488);
        coordinates = D_80083780;
        goto coordinates_ready;
    }

    if (D_800CFCEE[1] != 0) {
        ((Rec_D_800E3D7C *)table)->unk_14.s32 = 0;
        func_800954F4(table);
        coordinates = D_80083780;
        goto coordinates_ready;
    }
...
coordinates_ready:
    ...
    if (func_800C1D44((u16)tile_id) != 0) {
        ((Rec_D_800E3D7C *)table)->unk_14.s32 -= func_800A5894(table);
    }

    speed = func_8003BD84(((Rec_D_800E3D7C *)table)->unk_0C.n,
                          ((Rec_D_800E3D7C *)table)->unk_10.at00.v);
    if (D_80100E20[0] != 0) {
