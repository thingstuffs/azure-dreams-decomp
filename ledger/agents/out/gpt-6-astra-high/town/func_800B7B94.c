...
void func_800B52F4(s32 text_id, S_800B52F4_1 *context) {
    ...
    register s32 prefix_word ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    register s32 prefix_tail ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    register s32 display_param ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    StackData *call_text;

    prefix_page = 0x80090000;
    ASM_KEEP(prefix_page);   /* MATCH pin: keeps a statement from moving across a call/branch */
    prefix = (M2C_UNK *)(prefix_page - 0x6D40);
    ASM_KEEP(prefix);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    call_text = text;
    ASM_KEEP(call_text);   /* MATCH pin: retail schedule: same instructions, different order without it */
    display_param = 0x88;
