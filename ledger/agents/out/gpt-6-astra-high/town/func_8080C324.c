...
s32 func_8080C324(void) {
    ...
    s32 remaining;
    s32 initial_offset;
    register s32 offset_addr ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    s32 clear_addr;
    s32 state_page;
    s32 initial_state;
    s16 line_x;
    ...
    ASM_KEEP_NV(offset_addr);   /* MATCH pin: load-bearing for the whole function shape */
    offset_addr += 0x6D6;
    offset_entry = (s16 *)offset_addr;
    clear_addr = 0x80530000;
    ASM_KEEP_NV(clear_addr);   /* MATCH pin: load-bearing for the whole function shape */
    clear_addr += 0x666;
    ...
    ASM_USE2(panel_template, panel_record);   /* MATCH pin: retail schedule: same instructions, different order without it */
    remaining = 8;
    line_x = 0xE0;
    ...
    *(s32 *)(state_page + 0x6D8) = initial_state;
    ...
    panel_width = 0xE0;
    ASM_KEEP_NV(panel_width);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
...

    {
        ...
        ASM_USE2(content_template, content_args);   /* MATCH pin: retail schedule: same instructions, different order without it */
        content_remaining = 8;
        ASM_KEEP_NV(content_remaining);   /* MATCH pin: retail register colouring depends on it */
        content_x = 0xE4;
        ...
        do {
            register void *entry_template ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
            register void *entry_record ASM_REG("$5");   /* MATCH pin: retail callee-saved set / frame layout depends on it */

            entry_template = D_80526970;
            content = *content_ptr;
            ASM_USE(content);   /* MATCH pin: retail basic-block layout depends on it */
            entry_record = &content_record;
            ASM_KEEP_NV(entry_record);   /* MATCH pin: retail schedule: same instructions, different order without it */
