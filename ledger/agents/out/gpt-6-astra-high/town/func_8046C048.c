...
void func_8001D048(void) {
    ...
    s32 mode;
    register s32 row_index ASM_REG("$19");   /* MATCH pin: retail register colouring depends on it */
    s32 row_offset;
    s32 check_offset;
    register s32 mode_offset ASM_REG("$22");   /* MATCH pin: retail register colouring depends on it */
    register u8 entry_value ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
    s8 *row;
    ...
    row_or_column = (long)D_80016000->unk_38;
...
mode_done:
    ...
    row_index = 0;
    ASM_KEEP(row_index);   /* MATCH pin: retail register colouring depends on it */
    ...
    do {
        ...
        ASM_KEEP(bounds_addr);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        row_or_column = 0;
...
process_entry:
            ...
            if (entry_value != 0) {
                ...
                ASM_KEEP(callback_arg);   /* MATCH pin: load-bearing for the whole function shape */
