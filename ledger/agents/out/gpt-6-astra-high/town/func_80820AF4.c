...
void func_800232F4(void *object)
{
    ...
    u16 position_value;
    register s32 quarter_step ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
...
state_one:
    ...
    quarter_step = step;
    position_value += step;
    ASM_KEEP(position_value);   /* MATCH pin: load-bearing for the whole function shape */
