...
s32 func_800212B8(void) {
...

    {
        ...
        do {
            register u8 *widget_handler ASM_REG("$4");   /* MATCH pin: keeps a constant in a register as retail does */
            register Init10 *widget_args ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
            widget_handler = D_800225F0;
            widget_args = widget_params;
            ASM_KEEP_DEP_NV(widget_args, widget_handler);   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    {
        s32 value_index = 4;
        register s32 template_page ASM_REG("$20") = (s32)0x80020000;   /* MATCH pin: retail keeps a computation the compiler would drop */
        register s32 value_offset ASM_REG("$19") = 0x38;   /* MATCH pin: retail register colouring depends on it */
        s16 value_y = 0x44;
        ...
        do {
            ...
            *text_buffer = *copy_src;
            ...
            ASM_KEEP_DEP_NV(handler_page, template_page);   /* MATCH pin: load-bearing for the whole function shape */
...

    {
        ...
        u8 *sprite_handler;
        register s16 sprite_scale ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
        u8 *grid_sprite;
        ASM_KEEP(handler_page);   /* MATCH pin: keeps a constant in a register as retail does */
    ...
    {
        u8 *new_obj;
        register u8 *child_obj ASM_REG("$16");   /* MATCH pin: keeps a constant in a register as retail does */
        register s32 child_index ASM_REG("$18");   /* MATCH pin: retail register colouring depends on it */
        u8 *sprite_state;
        s16 frame_offset;
        register u8 *child_slot ASM_REG("$20");   /* MATCH pin: retail keeps a computation the compiler would drop */
        sprite_state = D_80082E80;
        child_index = 2;
        ASM_KEEP_DEP_NV(child_index, sprite_state);   /* MATCH pin: keeps a constant in a register as retail does */
        frame_offset = 0xC;
        child_slot = owner + 8;
        do {
            new_obj = func_8003FD64(0x136, D_80083498);
            child_obj = new_obj;
            ASM_KEEP(child_obj);   /* MATCH pin: retail register colouring depends on it */
