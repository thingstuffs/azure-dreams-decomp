...
void func_800C1EA4(Entity *entity)
{
    ...
    register s32 axis_value ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 x_delta ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    s32 mode;
    s16 x_distance;
    s16 y_distance;

    reference_pos = (Position *)D_80083780;
    ASM_KEEP(reference_pos);   /* MATCH pin: retail immediate-load split depends on it */
    x_delta = entity->x;
