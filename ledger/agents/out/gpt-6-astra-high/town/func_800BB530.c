...
s32 func_800B8C90(void)
{
    ...
    if (D_800133BA != expected_value) {
        ...
        ASM_KEEP(data_base);   /* MATCH pin: keeps a constant in a register as retail does */
    }
    expected_value = 0x29;
    ...
    if (D_800133A6 != expected_value) {
        register u8 *data_base ASM_REG("$3") = (u8 *)0x80010000;   /* MATCH pin: keeps a constant in a register as retail does */
...
loop_preheader:
    ...
    ASM_KEEP(required_cursor);   /* MATCH pin: retail register colouring depends on it */
    return 1;
}
