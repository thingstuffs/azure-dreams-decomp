#include "common.h"



typedef struct S_8001A8DC_0 {
    s16 unk_00;
    s16 unk_02;
} S_8001A8DC_0;   /* other in func_8001A8DC */

typedef struct S_8001A8DC_1 {
    u16 unk_00;
    u16 unk_02;
} S_8001A8DC_1;   /* base in func_8001A8DC */

typedef struct S_8001A8DC_2 {
    s16 unk_00;
    s16 unk_02;
} S_8001A8DC_2;   /* out in func_8001A8DC */

/* Blend two points using the signed weight over the absolute weight sum; return the scaled y delta. */
s32 func_8001A8DC(void *base_point, void *other_point, s32 blend_weight, s32 other_weight, void *result_point)
{
    s32 scaled_y_delta;
    s32 other_x;
    s32 other_y;
    s32 abs_other_weight;
    s32 base_x;
    s32 base_y;
    register void *base ASM_REG("$4");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register void *other ASM_REG("$5");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 weight ASM_REG("$6");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 total_weight ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
    register void *out ASM_REG("$9");   /* MATCH pin: retail register colouring depends on it */

    base = base_point;
    other = other_point;
    weight = blend_weight;
    out = result_point;
    ASM_KEEP(base);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    ASM_KEEP(other);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    ASM_KEEP(out);   /* MATCH pin: retail register colouring depends on it */
    abs_other_weight = other_weight;
    total_weight = weight;
    if (weight < 0) {
        total_weight = -total_weight;
    }
    if (abs_other_weight < 0) {
        abs_other_weight = -abs_other_weight;
    }
    total_weight += abs_other_weight;
    other_x = ((S_8001A8DC_0 *)other)->unk_00;
    ASM_KEEP(other_x);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    base_x = ((S_8001A8DC_1 *)base)->unk_00;
    ASM_KEEP(base_x);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    ((S_8001A8DC_2 *)out)->unk_00 =
        base_x
        + ((other_x - (s16)((S_8001A8DC_1 *)base)->unk_00) * weight) / total_weight;
    other_y = ((S_8001A8DC_0 *)other)->unk_02;
    ASM_KEEP(other_y);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    base_y = ((S_8001A8DC_1 *)base)->unk_02;
    ASM_KEEP(base_y);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    scaled_y_delta =
        (other_y - (s16)((S_8001A8DC_1 *)base)->unk_02) * weight;
    ((S_8001A8DC_2 *)out)->unk_02 =
        base_y + scaled_y_delta / total_weight;
    return scaled_y_delta;
}
