...
s32 func_8008B654(s32 value_7c, s32 value_78) {
    ...
    s32 new_resource;
    s32 index;
    register s32 incomplete ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
...
allocate_pair:
    ...
    if (new_object != 0) {
        ...
        ASM_KEEP_NV(new_resource);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
