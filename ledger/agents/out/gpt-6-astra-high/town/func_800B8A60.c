...
void func_800B61C0(S_800B61C0_1 *entry, s32 widgets_base, s32 row)
{
    s32 detail_info[4];
    s32 entry_info[2];
    register s32 row_offset ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 text_y;
    register s32 entry_value ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 *info_out;
