...
void *func_8009792C(S_8009792C_2 *source, u32 angle) {
    ...
    u32 direction;
    u8 *callback_page;
    register void *init_object ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    void *init_data;
    ...
    if (effect != NULL) {
...
#endif
        ASM_KEEP_NV(callback_page);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_KEEP(x_offsets);   /* MATCH pin: retail immediate-load split depends on it */
        ...
        ASM_KEEP(effect_params);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
