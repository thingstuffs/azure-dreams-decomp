...
void func_80123A60(void *object_data) {
    ...
    slot = 0;
    if (category == 3) {
        ASM_KEEP(category);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        slot = category;
        first_image_id = slot * 0x10;
        ASM_KEEP(first_image_id);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    ASM_KEEP(grid_category);   /* MATCH pin: retail basic-block layout depends on it */
