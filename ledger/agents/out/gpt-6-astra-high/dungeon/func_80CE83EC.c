...
void func_80171BEC(void *attachment, S_80171BEC_3 *base_position)
{
    ...
    register u32 data_word_0 ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    register u32 data_word_1 ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    register u32 data_word_2 ASM_REG("$5");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (object != 0) {
        ...
        ASM_KEEP(data_word_2);   /* MATCH pin: retail schedule: same instructions, different order without it */
