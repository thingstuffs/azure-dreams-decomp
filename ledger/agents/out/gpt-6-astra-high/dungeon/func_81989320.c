...
void func_80024B20(void *effect) {
    u16 state;
    {
        register s32 segment_index ASM_REG("$5") = 0;   /* MATCH pin: retail register colouring depends on it */
        s32 doubled_index;
        do {
            register s32 next_index ASM_REG("$2");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
...
mode_two:
    {
        ...
        do {
            register s32 index_squared ASM_REG("$8") = segment_index * segment_index;   /* MATCH pin: retail register colouring depends on it */
            s32 speed_bias;
            segment_index--;
            speed_bias = base_speed - ((S_80024B20_2 *)effect)->unk_42.u;
            base_speed -= 0x18;
            ((S_80024B20_3 *)segment)->unk_14 = speed_bias + ((index_squared >> 2) * 0x14);
            ASM_KEEP(index_squared);   /* MATCH pin: load-bearing for the whole function shape */
            segment -= 6;
        } while (segment_index >= 0);
    }
    {
        s32 segment_index = 6;
        register s32 gap_limit ASM_REG("$4") = 0x160;   /* MATCH pin: retail delay-slot fill depends on it */
        register u8 *segment ASM_REG("$6") = (u8 *)effect + 0x24;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        register s32 next_offset ASM_REG("$7") = 0x2A;   /* MATCH pin: retail register colouring depends on it */
        do {
...
mode_three:
    {
        register u32 globals_base ASM_REG("$3") = 0x80080000;   /* MATCH pin: retail register colouring depends on it */
        ASM_KEEP(globals_base);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
