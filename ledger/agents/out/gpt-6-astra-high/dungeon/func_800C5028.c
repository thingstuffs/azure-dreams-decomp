...
s32 func_800CA788(void *object_ptr, void *action_context, void *target_ptr, void *actor_ptr)
{
    ...
    u8 *object;
    u8 *actor;
    register void *context ASM_REG("$21");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *target ASM_REG("$18");   /* MATCH pin: retail schedule: same instructions, different order without it */
    u8 *global_state;
    u16 global_flags;
    ...
    ASM_KEEP(context);   /* MATCH pin: load-bearing for the whole function shape */
...
check_flag_8:
    ...
    {
        void *action_actor;
        register void *action_target ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
        s32 actor_param;
        s32 action_mode;   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        s32 actor_flags_14;
        s32 actor_flags_1c;

        action_actor = actor;
        ASM_KEEP(action_actor);   /* MATCH pin: retail schedule: same instructions, different order without it */
