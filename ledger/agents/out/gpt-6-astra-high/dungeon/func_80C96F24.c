...
void func_80C96F24(S_80C96F24_0 *owner, void *origin) {
    ...
    do {
        s32 band_index;
        register s32 segment ASM_REG("$21");   /* MATCH pin: keeps a constant in a register as retail does */

        {
            register s32 band_raw ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
            register s32 band_shifted ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */

            band_raw = band;
            ASM_KEEP_NV(band_raw);   /* MATCH pin: load-bearing for the whole function shape */
            segment = 0;
        ...
        do {
            s32 end_radius = 0;
            s32 start_radius = 0;
            register s32 middle_band ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
            s32 last_band;
            void *effect;
            ...
            if (effect != 0) {
                register void *effect_arg ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
                void *render_state;
                register void *effect_data ASM_REG("$20");   /* MATCH pin: load-bearing for the whole function shape */
                void *owner_ref;
                S_80C96F24_5 *position;
                register s32 init_value ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
                s16 angle_index;
                s32 start_angle;
                s32 end_angle;
                ...
                ASM_SET(owner_ref);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
                owner_ref = owner;
                ASM_KEEP_NV(owner_ref);   /* MATCH pin: keeps a statement from moving across a call/branch */
                ...
                {
                    register void *origin_ref ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */

                    ASM_SET(origin_ref);   /* MATCH pin: retail schedule: same instructions, different order without it */
                    origin_ref = origin;
                    ASM_KEEP_NV(origin_ref);   /* MATCH pin: load-bearing for the whole function shape */
                ...
                {
                    register s32 second_segment ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
                    s32 segment_index = (s16)segment;

                    ASM_SET(second_segment);   /* MATCH pin: retail schedule: same instructions, different order without it */
                    second_segment = 1;
                ...
                {
                    register s32 scaled_trig ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
                    s32 end_radius_copy;
                    void *render_data;
                    s32 trig_angle;
                    s32 vertex_coord;
                    ...
                    ASM_KEEP_DEP_NV(effect_data, trig_angle);   /* MATCH pin: retail schedule: same instructions, different order without it */
                    ...
                    ASM_KEEP_NV(end_radius_copy);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                    ...
                    vertex_coord = scaled_trig >> 12;
                    ASM_USE2_NV(scaled_trig, vertex_coord);   /* MATCH pin: load-bearing for the whole function shape */
                    ((S_80C96F24_1 *)effect_data)->unk_58 = vertex_coord;
                }

                {
                    register s32 middle_band_z ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
                    s32 last_band_z;
                    s16 end_z = 0;
                    s16 start_z = end_z;
            ...
            {
                s32 segment_step;
                register void *counter_owner ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
                u16 effect_count;
        ...
        {
            register s32 prev_band ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
            s16 next_band;

            prev_band = band;
            next_band = (s16)(prev_band + 1);
            ASM_USE2_NV(prev_band, next_band);   /* MATCH pin: load-bearing for the whole function shape */
            band = (u16)next_band;
            if (next_band >= 3) {
                break;
            }
