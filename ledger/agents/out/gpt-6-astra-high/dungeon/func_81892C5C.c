...
s32 func_81892C5C(void *effect, S_81892C5C_2 *center)
{
    ...
    register u32 addr_mask ASM_REG("$23");   /* MATCH pin: retail immediate-load split depends on it */
...
loop:
    {
        ...
        s32 end_angle;
        s32 height;
        register s32 prim_mode ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
        s32 x_trig;
        register s32 end_y_scaled ASM_REG("$9");   /* MATCH pin: keeps a constant in a register as retail does */
        ...
        ASM_KEEP_NV(prim_mode);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        {
            register s32 end_y_offset ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
...

        {
            ...
            ASM_KEEP_DEP_NV(end_vertex, end_y_scaled);   /* MATCH pin: retail schedule: same instructions, different order without it */
            ASM_KEEP4_NV(start_vertex, mid_vertex, end_vertex, screen_start);   /* MATCH pin: keeps a statement from moving across a call/branch */
...

        {
            ...
            if (otz < 0x1E0) {
                ...
                {
                    u32 ot_link;
                    register u32 packet_length ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
...

    {
        ...
        if (next_effect != 0) {
            ...
            ASM_USE(effect);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            center = ((S_81892C5C_7 *)next_effect)->unk_08;
            ASM_KEEP(center);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
            func_800244AC();
        }
    }
    ASM_SCHED_BARRIER();   /* MATCH pin: keeps a statement from moving across a call/branch */
    {
        register s32 result ASM_REG("$2") = 0;   /* MATCH pin: retail register colouring depends on it */
