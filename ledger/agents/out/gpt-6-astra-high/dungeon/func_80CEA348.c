...
void func_80173B48(void *action_input, s32 x_offset_input, void *position_input, void *actor_input)
{
    ...
    register s32 x_offset ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *position ASM_REG("$17");   /* MATCH pin: retail delay-slot fill depends on it */
    void *actor = actor_input;
    register s32 remaining_steps ASM_REG("$20");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 next_y ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u32 saved_x;
    u32 saved_y;
    s32 action_state;
    u8 state_param;
    ...
    ASM_KEEP(action);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
