...
void func_8009D6F4(void) {
    ...
    s32 dimension_unit;
    s16 next_row;
    register s16 row_index ASM_REG("$11");   /* MATCH pin: retail register colouring depends on it */
    ...
    if ((1 << ((S_8009D6F4_0 *)grid_dims)->unk_16) > 0) {
        register s32 col_index ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
        s32 nibble_mask;
        register s32 row_wide ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
        register s32 next_col ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
...
col_loop:
            ...
            if (nibble_mask != 0) {
                register s32 value_bits ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
                s32 level;
                register s32 clamped_level ASM_REG("$4");   /* MATCH pin: retail keeps a data_source the compiler would otherwise drop/add */
                s32 col;
                register s16 shift;
                ...
                ASM_KEEP(value_bits);   /* MATCH pin: retail register colouring depends on it */
                level = value_bits;
                if (level >= 0x10) {
                    clamped_level = 15;
                    goto clamp_value;
...
advance_col:
            ...
            col_index = next_col;
            ASM_KEEP_NV(next_col);   /* MATCH pin: retail register colouring depends on it */
