...
s32 func_80173E94(void *node_arg, void *vertex_arg)
{
    ...
    ASM_KEEP(state_ptr);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ASM_KEEP(tag_mask);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
...
loop:
    ...
    *(u8 * volatile *)(scratch + 0x18) = packet + 0xC;
    ...
    if (depth_index < 0x1E0) {
        s32 shade_or_page;
        register s32 pixel_mode ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
        register s32 blend_mode ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
        s32 page_x;
        s32 tile_code;
        ...
        tile_code = 0x6A;
        ASM_SET(page_x);   /* MATCH pin: retail schedule: same instructions, different order without it */
        page_x = pixel_mode;
        ...
        {
            register u32 *ot_entry ASM_REG("$7");   /* MATCH pin: load-bearing for the whole function shape */
