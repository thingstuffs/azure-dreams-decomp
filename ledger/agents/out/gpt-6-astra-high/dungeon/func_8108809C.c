...
void *func_8017589C(s32 allocation_param, Copy24 *initial_data, S_8017589C_3 *source_node)
{
    u16 delta[3];
    s32 random_value;
    s32 random_quotient;
    ...
    if (object != NULL) {
        ...
        ASM_KEEP(random_quotient);   /* MATCH pin: retail register colouring depends on it */
        random_quotient >>= 0xC;
        if (random_value < 0) {
        ...
        node->unk_28 = source_node->unk_28;
        node->unk_0E = 0x80;
        node->unk_0D = 0x80;
        node->unk_0C = 0x80;
        node->unk_1E = 0x1000;
