...
s32 func_818C29D4(S_818C29D4_6 *sprite, S_818C29D4_4 *position)
{
    ...
    s32 vertex_coord;
    Poly818C29D4 *poly;
    register u8 *draw_mode ASM_REG("$16");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    ASM_KEEP(texture_v);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if (((S_818C29D4_2 *)scratch)->unk_B4.n < 0x1E0U) {
        ...
        ASM_KEEP4(reset_depth, reset_blend, reset_page_x, reset_page_y);   /* MATCH pin: retail immediate-load split depends on it */
        ...
        ASM_KEEP(draw_mode);   /* MATCH pin: retail schedule: same instructions, different order without it */
        addr_mask |= 0xFFFF; ASM_KEEP(addr_mask);
        ...
        ASM_KEEP(tex_depth);      /* MATCH pin: retail schedule: same instructions, different order without it */
