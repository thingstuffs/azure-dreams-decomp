...
void *func_800D3F74(s16 variant, u8 tile_x, u8 tile_y, s16 spawn_height)
{
    ...
    s8 *entity = NULL;
    s8 *object;
    register u8 saved_tile_y ASM_REG("$20") = tile_y;   /* MATCH pin: retail schedule: same instructions, different order without it */
    register u8 saved_tile_x ASM_REG("$21") = tile_x;   /* MATCH pin: retail schedule: same instructions, different order without it */
    s8 *sprite;
    s16 ground_height;
    s32 tile_flags;
    u8 sprite_tile_x;
