...
s16 func_800A384C(Actor *actor, Actor *target, u16 *out_angle, s32 prefer_ability)
{
  ...
  s16 best_score;
  u16 line_valid;
  register Actor *self ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
  Entity *actor_entity;
  register Entity *target_entity ASM_REG("$16");   /* MATCH pin: retail keeps a computation the compiler would drop */
  u8 *ability;
  u8 ability_id;
  s32 target_mode;
  s32 effect_id;
  ...
  s32 distance;
  s16 target_angle;
  register s16 slot ASM_REG("$21");   /* MATCH pin: retail keeps a computation the compiler would drop */
  s16 effect_blocked;
  register s32 slot_seed ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
  register s32 slot_offset ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
  s32 slot_index;
  register s32 ability_offset ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
  register s16 steps ASM_REG("$18");   /* MATCH pin: load-bearing for the whole function shape */
  s16 next_steps;
  s16 x;
  s16 y;
  s32 height_delta;
  register s32 actor_height ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
  s32 score;
  s32 x_steps_base;
  s32 flags;
  s32 ability_flags;
  register s32 step_offset ASM_REG("$19");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
  register s32 direction_shifted ASM_REG("$20");   /* MATCH pin: load-bearing for the whole function shape */
  s16 direction;
  u16 facing_angle;
  register u16 *x_step ASM_REG("$22");   /* MATCH pin: load-bearing for the whole function shape */
  selected_action = -1;
  facing_angle = actor->field2a;
  ASM_USE_NV(facing_angle);   /* MATCH pin: retail schedule: same instructions, different order without it */
  best_score = -0x100;
  ASM_KEEP(best_score);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
  ...
  *out_angle = target_angle;
  ...
  if (self->status98 & 1)
  {
    ...
    kind1:
    {
      ...
      ASM_SET(direction_shifted);   /* MATCH pin: load-bearing for the whole function shape */
      ...
      ASM_USE_NV(score);   /* MATCH pin: retail keeps a computation the compiler would drop */
