...
void func_8196B2F8(s32 unused_0, s32 unused_1, s32 unused_2, s32 offset_x, s32 offset_y, s32 offset_z) {
    s32 jitter;
    register s32 world_coord ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    s32 jittered_coord;
    s32 origin_coord;
    s16 random_size;
    u16 render_flags;
    register u16 render_value ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    ...
    if (effect != NULL) {
        ...
        ASM_KEEP(render_flags);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        render_value = 0x60;
        render_flags |= 2;
        ...
        ASM_KEEP(render_or_origin);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_KEEP(origin_coord);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
