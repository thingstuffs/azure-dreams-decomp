...
{
    s32 one;
    register s32 next_value ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
    s16 phase;
...
state_0:
        ...
        ASM_TAILSLOT_PIN_TIED(next_value);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        func_8002491C();
