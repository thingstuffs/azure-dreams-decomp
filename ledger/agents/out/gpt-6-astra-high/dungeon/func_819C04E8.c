...
s32 func_80025CE8(u16 x, u16 y, u16 z, u16 angle) {
    ...
    u8 *table_page;
    register Copy32 *offset_source ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
    s32 object_index;
    Object **slot;
    register Object *new_object ASM_REG("$3");   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    s32 x_offset;
    u16 y_offset;
    register u8 *data_entry ASM_REG("$3");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 previous_index ASM_REG("$5");   /* MATCH pin: retail keeps a direction_offsets the compiler would otherwise drop/add */
    ...
    register u16 render_flags ASM_REG("$3");   /* MATCH pin: keeps a statement from moving across a call/branch */
    u16 cleanup_flags;
    s32 color;
    Object *alloc_parent;
    u8 *alloc_page;
    ...
    ASM_KEEP(table_page);   /* MATCH pin: keeps a statement from moving across a call/branch */
    offset_source = (Copy32 *)(table_page + 0x4028);
    ASM_KEEP(offset_source);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ...
        *slot = new_object;
        if (new_object != 0) {
            ...
            if ((s16)object_index != 0) {
                ...
                ASM_KEEP(data_entry);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        slot_offset = object_index + 1;
        object_index = slot_offset;
        ASM_KEEP(object_index);   /* MATCH pin: retail register colouring depends on it */
