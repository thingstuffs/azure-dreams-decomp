...
void func_81934C5C(void *effect) {
    ...
    s32 height_work;
    s32 height_velocity;
    s32 x_fixed;   /* MATCH pin: retail basic-block layout depends on it */
    s32 x_velocity;
    s32 world_y;
    s32 world_x;
    s32 x_step;
...
block_state2:
    ...
    ASM_KEEP(x_step);   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    if (impact_effect != NULL) {
        ...
        if (func_80069EF8() & 1) {
            ...
            ASM_TAILSLOT_PIN(alt_animation);   /* MATCH pin: retail delay-slot contents depend on it */
            func_80024678();
            return;
        }
        {
