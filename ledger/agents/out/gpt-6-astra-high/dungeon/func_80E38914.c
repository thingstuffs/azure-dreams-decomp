...
s32 func_80172114(void *object, M2C_UNK state_input_a, M2C_UNK state_input_b) {
    ...
    ASM_KEEP(saved_input_a);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP(saved_input_b);   /* MATCH pin: retail schedule: same instructions, different order without it */
    call_result <<= 16;
