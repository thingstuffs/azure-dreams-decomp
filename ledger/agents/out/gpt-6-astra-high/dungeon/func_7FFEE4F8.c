...
void func_8008BC58(u8 *object, S_8008BC58_2 *view_params, void *update_context)
{
    ...
    if (state == 0) {
        ...
        register s32 selection_index ASM_REG("$2");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        u8 selection_id;
        register u8 *selection_positions ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
        s32 buttons;
