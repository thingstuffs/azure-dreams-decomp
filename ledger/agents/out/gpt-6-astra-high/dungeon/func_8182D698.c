...
void func_8182D698(Object *object, Motion *motion, Effect *effect) {
    ...
    ((s32 *)object->child)[3] |= 0x8000;
    {
        ...
        }
        {
            ...
            ASM_SCHED_BARRIER();   /* MATCH pin: keeps a statement from moving across a call/branch */
            {
                register s32 y_velocity_copy ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */

                y_velocity_copy = y_velocity;
                ASM_KEEP(y_velocity_copy);   /* MATCH pin: keeps a statement from moving across a call/branch */
