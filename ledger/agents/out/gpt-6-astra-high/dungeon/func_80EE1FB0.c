...
void func_801737B0(void *action, void *motion, void *sprite, void *actor) {
    ...
    register s32 particle_count ASM_REG("$18");   /* MATCH pin: retail keeps a computation the compiler would drop */
    s32 facing_x;
    s32 offset_z;
    s32 direction;
    s32 action_kind;
    ...
    register s32 offset_x ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
    register s32 offset_y ASM_REG("$16");   /* MATCH pin: retail register colouring depends on it */
    u16 timer;
    u16 sprite_flags;
    u16 raw_kind;
    u32 kind_index;
    ...
    u8 *effect_state;
    u8 *particle_origin;
    register void *target ASM_REG("$2");   /* MATCH pin: retail delay-slot fill depends on it */
...
copy_facing:
            {
                ...
                ASM_USE(target);   /* MATCH pin: load-bearing for the whole function shape */
...
particle_loop:
    ...
    ASM_USE(particle_origin);   /* MATCH pin: retail schedule: same instructions, different order without it */
    particle_zero = 0;
