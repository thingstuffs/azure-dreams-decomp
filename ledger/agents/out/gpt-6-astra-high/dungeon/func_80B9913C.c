...
selection_ready:
    ...
    ASM_KEEP(item_slot);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    {
        s32 use_special;

        use_special = is_special;
        ASM_KEEP(use_special);   /* MATCH pin: retail delay-slot fill depends on it */
        if (use_special != 0) {
            target = D_800814A8;
