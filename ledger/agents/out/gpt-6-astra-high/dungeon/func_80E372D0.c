...
void func_80170AD0(void *entity_arg, void *motion_arg, void *monster_arg)
{
    ...
    register void *motion ASM_REG("$21") = motion_arg;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *monster ASM_REG("$20") = monster_arg;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
