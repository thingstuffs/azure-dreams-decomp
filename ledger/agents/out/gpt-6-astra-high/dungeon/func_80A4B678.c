...
void func_80174E78(Source *source, Vec3i *center)
{
    ...
    for (; segment_index < 16; segment_index++) {
        ...
        s32 initial_count;
        s32 trig_angle;
        register s32 trig_value ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ...
        initial_count = 50;
        setup_entity = entity;
        ASM_KEEP(setup_entity);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        *prim_flags_ptr = prim_flags;
        ...
        angle <<= 8;
        trig_angle = angle;
        ASM_KEEP(trig_angle);   /* MATCH pin: retail schedule: same instructions, different order without it */
