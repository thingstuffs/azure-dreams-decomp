...
void func_80172AB4(void *action_in, void *motion_in, void *sprite_in, void *actor_in)
{
    ...
    register void *motion ASM_REG("$20") = motion_in;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *sprite ASM_REG("$18") = sprite_in;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *actor ASM_REG("$17") = actor_in;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    ASM_KEEP4_NV(action, motion, sprite, actor);   /* MATCH pin: retail delay-slot fill depends on it */
    ...
    }
    {
        ...
        ASM_KEEP_NV(finish_state);   /* MATCH pin: load-bearing for the whole function shape */
