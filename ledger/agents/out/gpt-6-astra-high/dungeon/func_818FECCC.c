    ...
    s32 z_offset)
{
    ...
    register s16 saved_param_34 ASM_REG("$21") = param_34;   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 saved_param_28 ASM_REG("$23") = param_28;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s16 saved_param_52 ASM_REG("$22") = param_52;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    s32 z_pos;
    u32 descriptor_page;
    register void *effect ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    void *effect_data;
    S_818FECCC_2 *x_dest;
    S_818FECCC_3 *y_dest;
    S_818FECCC_5 *z_dest;
    ...
    if (effect_cursor != NULL) {
        ...
        ASM_USE2(effect, effect_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
        z_random &= 0x1F;
        ...
        ASM_KEEP(effect_cursor);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ((S_818FECCC_0 *)effect_cursor)->unk_14 = saved_param_34;
        ASM_KEEP(saved_param_34);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ((S_818FECCC_0 *)effect_cursor)->unk_32 = saved_param_52;
        func_8004491C(effect, effect_data, z_dest);
        ASM_KEEP(saved_param_52);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ((S_818FECCC_0 *)effect_cursor)->unk_08.u = saved_param_28;
        ASM_KEEP(saved_param_28);   /* MATCH pin: retail schedule: same instructions, different order without it */
    }
}
