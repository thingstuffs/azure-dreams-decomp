...
s32 func_80AC50A4(void *render_state, void *coord_data)
{
    ...
    u32 initial_cursor;
    register u32 slot_word ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    u16 pos_x;
    u16 pos_y;
    u16 pos_z;
    register void *state ASM_REG("$19") = render_state;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    ...
    ASM_KEEP_NV(address_mask);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    position = scratch + 4;
    ASM_KEEP_NV(position);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    ...
    if (depth < 0x1E0U) {
        ...
        s32 rand_one = 1;
        u32 color_word;
        register u32 packet_byte ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
        ASM_KEEP(rand_zero);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ASM_KEEP(rand_one);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        ((S_80AC50A4_4 *)tile_packet)->unk_04.at00.v = color_word;
        {
            ...
            register u32 green ASM_REG("$6") = ((S_80AC50A4_4 *)tile_packet)->unk_04.at01.v;   /* MATCH pin: keeps a statement from moving across a call/branch */
            register u32 blue ASM_REG("$7") = ((S_80AC50A4_4 *)tile_packet)->unk_04.at02.v;   /* MATCH pin: keeps a statement from moving across a call/branch */
        ...
        *mode_slot = (*mode_slot & length_mask) | ((u32)mode_packet & address_mask);
        ASM_KEEP(length_mask);   /* MATCH pin: retail address form (%hi/%lo vs render_buffer+offset) depends on it */
    }

    next_node = ((S_80AC50A4_3_pre *)state)[-1].unk_00;
    if (next_node != 0) {
        register void *next_position ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
        ...
        ASM_KEEP(input);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
        next_position = scratch + 4;
        ASM_TAILSLOT_PIN(next_position);   /* MATCH pin: retail delay-slot contents depend on it */
        func_801708FC();
    }
    ASM_KEEP(render_buffer_ptr);   /* MATCH pin: keeps a statement from moving across a call/branch */
