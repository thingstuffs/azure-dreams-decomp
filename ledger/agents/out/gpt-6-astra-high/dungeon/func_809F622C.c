...
{
    void *controller;
    register void *motion ASM_REG("$19");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register void *sprite ASM_REG("$18");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register void *actor ASM_REG("$16");   /* MATCH pin: retail branch polarity depends on it */
    s32 actor_flags;
    u16 ticks_left;
    u8 *global_base;
    s32 state;
