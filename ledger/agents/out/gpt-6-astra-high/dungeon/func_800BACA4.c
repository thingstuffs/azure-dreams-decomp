...
void func_800C0404(DungeonObject *obj, MotionState *motion, EffectState *effect) {
    ...
    u8 *prim_list;
    s32 index_or_phase;
    register s32 area_origin ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register Primitive *first_prim ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    ASM_USE_NV(prim_list);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    if (obj->count > 0) {
        ...
        do {
            ...
            prim->field8 = prim->fieldC;
            {
                ...
                ASM_USE_NV(use_alt_buffer);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
            }
            prim->field4 = 0;
            func_800667BC(prim);
            func_8006658C(prim_list, prim);
