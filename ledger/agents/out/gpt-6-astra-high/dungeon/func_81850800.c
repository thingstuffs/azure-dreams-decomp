...
BODY_STORAGE void BODY_NAME(void *owner, void *motion, void *sprite)
{
    ...
    register s32 particles_left ASM_REG("$19");
    s32 tile_x;
    s32 tile_y;
    s32 color;
    s32 copy_value;
    s32 spawn_color;
    register s32 tile_step_x ASM_REG("$9");
    s32 tile_step_y;
    s32 pixel_step_x;
    register s32 pixel_step_y ASM_REG("$9");
    register s32 pixel_offset ASM_REG("$3");
    u16 timer;
    u16 flags;
    register u8 base_y ASM_REG("$4");
    s16 ground_height;
    u8 *caster_data;
    register u8 *caster_sprite ASM_REG("$23");
    u8 *work;
    u8 *obj;
    u8 *target;
    u8 *effect_data;
    ...
    register u8 *particle_anim ASM_REG("$20");
    u8 *step_x_table;
    u8 *step_y_table;
    s32 pix_x;
    s32 origin_y;
...
state_0:
    ...
    if (target == 0) {
        s32 tile_pixel_x;
        register s32 tile_pixel_y ASM_REG("$2");
        s32 min_height;
        register s32 probe_x ASM_REG("$17");
        register u16 probe_y ASM_REG("$16");
...
state_0_after:
    ...
    ASM_KEEP(pixel_step_x);
    tile_step_y = step_y * scale;
    ASM_KEEP_NV(tile_step_y);
