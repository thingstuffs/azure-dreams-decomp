...
s32 func_800A02AC(void *entity, u16 x, u16 y) {
    u16 *x_offsets;
    u16 *y_offsets;
    register void *entity_data ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void *current_entity;
    u16 base_x;
    u16 base_y;
    s32 result;
    register s32 attempt ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 direction;
