...
void func_800165B8(void) {
    ...
    u8 *actor_storage;
    u8 *actor;
    register u8 *call_target ASM_REG("$4");   /* MATCH pin: retail branch polarity depends on it */
    u8 *state;
    u8 *obj;
    u8 *entity;
    u8 *room;
    ...
    s32 sample_y;
    s32 state_config;
    register s32 *reverse_entries ASM_REG("$18");   /* MATCH pin: load-bearing for the whole function shape */
    s32 *state_entries;
    u8 *table_base;
    register u8 *entry_addr ASM_REG("$2");   /* MATCH pin: retail immediate-load split depends on it */
    u8 tile;
    u8 actor_x;
    u8 actor_y;
    u16 display_setting;
    register u16 lift_height ASM_REG("$7");   /* MATCH pin: keeps a statement from moving across a call/branch */
    u8 *settings_page;
    u8 *display_page;
    u8 *display_state;
    u8 *map_state;
    register u8 *bind_base ASM_REG("$4");   /* MATCH pin: retail branch polarity depends on it */
    register s32 bind_count ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register u8 *bind_state ASM_REG("$6");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register u8 *bind_angle ASM_REG("$7");   /* MATCH pin: keeps a statement from moving across a call/branch */
    u8 *defaults_page;
    register s32 height_limit ASM_REG("$6");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 init_value ASM_REG("$2");   /* MATCH pin: retail immediate-load split depends on it */
    u8 *status_page;
    s32 status_value;
    u16 init_flags;
    register s32 neutral_color ASM_REG("$3");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 entity_mask;
    s32 entity_flags;
...
retry_position:
        ...
        height_limit = -0x400;
        ASM_KEEP(height_limit);   /* MATCH pin: retail schedule: same instructions, different order without it */
        direction = -1;
...
check_neighbor:
        ...
        offset_index = direction & 7;
        {
            ...
            sample_x &= 0xFFE0;
            ASM_KEEP_NV(sample_x);   /* MATCH pin: retail schedule: same instructions, different order without it */
...
initialize_position:
    ...
    {
        u8 *entries_page;
        register s32 *entries_base ASM_REG("$2");   /* MATCH pin: retail immediate-load split depends on it */

        entries_page = (u8 *)0x800E0000;
        ASM_KEEP(entries_page);   /* MATCH pin: load-bearing for the whole function shape */
        entries_base = (s32 *)(entries_page + 0x3D80);
        ASM_KEEP(entries_base);   /* MATCH pin: keeps a constant in a register as retail does */
...
load_entries:
    call_target = state;
    bind_state = (u8 *)0;
    ASM_KEEP_NV(bind_state);   /* MATCH pin: retail schedule: same instructions, different order without it */
    table_base = (u8 *)0x800E0000;
    ASM_KEEP_NV(table_base);   /* MATCH pin: load-bearing for the whole function shape */
    table_base -= 0x2F88;
    entry_addr = (u8 *)((u32)entry_index + (u32)table_base);
    ASM_KEEP_DEP_NV(entry_addr, table_base);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    func_800489F4(call_target, *entry_addr, (s32)bind_state, (s32)bind_state);
    call_target = state;
    bind_state = (u8 *)0;
    zero_arg = bind_state;
    ASM_KEEP_DEP_NV(zero_arg, bind_state);   /* MATCH pin: retail schedule: same instructions, different order without it */
    table_base = (u8 *)0x800E0000;
    ASM_KEEP_NV(table_base);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    *(s32 *)((u8 *)D_800E3D48 + entry_offset) = ((S_800165B8_3 *)state)->unk_08;
    ...
    bind_angle = obj + 0x2A;
    ASM_KEEP(bind_count);   /* MATCH pin: retail schedule: same instructions, different order without it */
    init_value = -1;
    ...
    ASM_KEEP_NV(settings_page);   /* MATCH pin: keeps a statement from moving across a call/branch */
    table_base = (u8 *)0x80080000;
    ASM_KEEP(table_base);   /* MATCH pin: retail immediate-load split depends on it */
    init_value = ((S_800165B8_1 *)obj)->unk_A2;
    table_base += 0x3160;
    init_value |= 0x10;
    ...
    ASM_KEEP(display_page);   /* MATCH pin: retail immediate-load split depends on it */
    ...
    *(Copy12 *)(entity + 0x34) = *(Copy12 *)copy_addr;
...

    {
        ...
        u16 height_index;
        u16 display_value;
        register u16 *display_values ASM_REG("$3");   /* MATCH pin: retail schedule: same instructions, different order without it */

        display_values = D_800DD264;
        tile_x = ((S_800165B8_3 *)state)->unk_24;
        ASM_KEEP_NV(tile_x);   /* MATCH pin: retail schedule: same instructions, different order without it */
        display_index = *(u16 *)(settings_page + 0x20A2);
        tile_y = ((S_800165B8_3 *)state)->unk_25;
        ASM_KEEP_DEP_NV(display_index, tile_y);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    ...
    ASM_KEEP(init_flags);   /* MATCH pin: keeps a statement from moving across a call/branch */
    status_value = -0x24;
