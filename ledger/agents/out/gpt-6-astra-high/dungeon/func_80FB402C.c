...
state_zero:
    {
        ...
        s32 word_offset;
        u8 *facing_record;
        register u8 *record ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
        u8 *direction_table;
        u8 *counter_base;
        void *effect;
        register u8 *effect_state ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
        u16 facing;
