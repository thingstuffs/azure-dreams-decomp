...
void func_8181B1A0(void *source, s32 offset_x, s32 offset_y, s32 offset_z, s32 effect_value) {
    register s32 saved_value ASM_REG("$18") = effect_value;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    S_8181B1A0_2 *render;
    S_8181B1A0_0 *state;
    void *effect;
    ...
    state = effect + 0x20;
    if (effect != NULL) {
        ASM_KEEP(state);   /* MATCH pin: retail register colouring depends on it */
