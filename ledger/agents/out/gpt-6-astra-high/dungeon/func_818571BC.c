...
s32 func_800249BC(void *shape_data)
{
    ...
    register u32 address_mask ASM_REG("$21") = 0x00FFFFFF;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 depth_or_tag_mask;
    s32 y_span;
    s32 width_divisor;
    register s32 point_index ASM_REG("$19");   /* MATCH pin: retail register colouring depends on it */
#ifdef __mips__
    DungeonSignedProduct depth_product;
    s32 depth_sign;
    register s32 scaled_depth ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
#endif
    ...
    if ((u32)depth_or_tag_mask < 0x1E0) {
        point_index = 0;
        {
            ...
            do {
                ...
                u16 start_y;
                u16 end_y;
                register u8 tex_coord ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
                register u8 tex_span ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
                u32 *ordering_entry;
                DungeonDrawState *ordering_state;
                register u32 primitive_tag ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
                u32 ordering_tag;
                ...
                ASM_KEEP_NV(texture_attr);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                ...
                ASM_KEEP_NV(start_x);   /* MATCH pin: keeps a statement from moving across a call/branch */
                width_u16 = *(u16 *)(void *)&half_width;
                ASM_KEEP_NV(width_u16);   /* MATCH pin: keeps a statement from moving across a call/branch */
                next_point =
