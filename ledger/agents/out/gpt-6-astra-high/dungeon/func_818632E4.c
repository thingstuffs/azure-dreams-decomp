...
{
    register Controller *ctrl ASM_REG("$19") = controller;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register Motion *motion ASM_REG("$22") = motion_arg;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *root;
    ...
    u16 final_x;
    u16 final_y;
    register s32 step ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
