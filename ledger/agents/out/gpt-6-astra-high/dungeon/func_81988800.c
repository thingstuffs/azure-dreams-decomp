...
void BODY_NAME(void *state_data, void *position_data)
{
    ...
    void *object;
    u8 *work;
    register void *state ASM_REG("$18") = state_data;   /* MATCH pin: retail address form (%hi/%lo vs source+offset) depends on it */
    void *source;
    register void *position_ref ASM_REG("$20") = position_data;   /* MATCH pin: load-bearing for the whole function shape */
    void *actor;
    u8 *unused_fields;
    u16 timer;
...
jt_c3: {
        ...
        register u8 *entry_ptr ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
        s32 entry_index;
        ...
        if (((S_81988800_0 *)state)->unk_50.s <= 0) {
            ...
            if (object != 0) {
                ...
                for (entry_index = 7; entry_index >= 0; entry_index--, entry_ptr -= 6) {
                    ...
                    ASM_KEEP(entry_index);   /* MATCH pin: retail schedule: same instructions, different order without it */
                }
...
jt_c4: {
        if (((S_81988800_0 *)state)->unk_50.s == 8) {
            ...
            if (object != 0) {
                ...
                ASM_KEEP(data_base);   /* MATCH pin: load-bearing for the whole function shape */
