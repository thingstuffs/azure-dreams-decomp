...
void func_80025374(State *state_arg, Vec12 *position_arg, Graphic *graphic_arg)
{
    ...
    register void *data_base ASM_REG("$16");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *source_or_spawn ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *offset_page;
    register PackedOffsetChunks *offset_source ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
    u32 dispatch;
    s32 advance;
    s32 state_index;
    ...
    ASM_KEEP_NV(offset_page);   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    ASM_KEEP_DEP_NV(offset_source, offset_page);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    ASM_KEEP(offset_page);   /* MATCH pin: retail register colouring depends on it */
    state_index = state->stateA;
    ASM_KEEP(state_index);   /* MATCH pin: retail schedule: same instructions, different order without it */
    data_base = (u8 *)entity - 0x20;
    ASM_KEEP_NV(data_base);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
...
initialize:
    {
        D_80020000Page *page;
        register s32 next_state ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
        register u32 flags_or_result ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
        u16 source_z;
...
create_spawn:
    {
        RoomData *room;
        ByteEntry *table;
        register Graphic *spawn_graphic ASM_REG("$7");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
        register Vec12 *spawn_position ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
        register PackedOffsets *offset_base ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
        u8 *template_page;
        register PackedTemplate *template_source ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
        register s32 path_value ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
        register s32 coord_or_variant ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
        s32 offset_x;
        s32 offset_y;
        s16 steps;
        ...
        if (source_or_spawn != 0) {
            ...
            ASM_KEEP(offset_x);   /* MATCH pin: retail schedule: same instructions, different order without it */
            data_base = (u8 *)&((Spawned *)source_or_spawn)->data20;
            ASM_KEEP(data_base);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
            ...
            ASM_KEEP(coord_or_variant);   /* MATCH pin: retail basic-block layout depends on it */
            ...
            ASM_KEEP_NV(template_page);   /* MATCH pin: retail delay-slot fill depends on it */
            template_source = (PackedTemplate *)(template_page + 0x5900);
            ASM_KEEP_DEP_NV(template_source, template_page);   /* MATCH pin: retail delay-slot fill depends on it */
            ((SpawnData *)data_base)->template20 = *template_source;
            ASM_KEEP(template_page);   /* MATCH pin: retail register colouring depends on it */
            spawn_graphic->unk8 =
...
wait_finish:
    {
        ...
        if ((s16)(ticks + 1) >= 11) {
            ...
            if (pending == 0) {
                ...
                ASM_KEEP_NV(reset_page);   /* MATCH pin: keeps a statement from moving across a call/branch */
                *(s32 *)(reset_page + 0x346C) = 0;
                object_flags = ((u16 *)state)[-1];
                ASM_KEEP(object_flags);   /* MATCH pin: load-bearing for the whole function shape */
