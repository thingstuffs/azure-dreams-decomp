...
s32 func_80026864(void *objects, void *view_position, void *render_params)
{
    ...
    do {
        ...
        do {
            ...
            if (mesh_index != 0) {
                ...
                for (;;) {
                    ASM_SET(vert2_y);   /* MATCH pin: keeps a statement from moving across a call/branch */
                    ...
                    ASM_USE2_NV(vert3_ref_y, vert3_ref_y);   /* MATCH pin: retail register colouring depends on it */
                    ASM_USE_NV(vert3_ref_y);   /* MATCH pin: retail register colouring depends on it */
                    vert0_ref = vert0_ref << 3;
                    vert0_ref = vert0_ref + (u32)vertices;
                    xy1 = xy1 + cell_x;
                    xy1 = xy1 & 0xFFFF;
                    ASM_USE2_NV(xy1, xy1);   /* MATCH pin: retail register colouring depends on it */
                    ASM_USE2_NV(xy1, xy1);   /* MATCH pin: retail register colouring depends on it */
                    ...
                    ASM_KEEP_NV(vert0_ref);   /* MATCH pin: keeps a statement from moving across a call/branch */
                    height0 = U16((u8 *)vert0_ref, 4);
                    height_bias = ((Cell *)((u8 *)&D_80027120[0] + cell_index * 6))->bias;
                    height0 = height0 - height_bias;
                    ASM_USE2_NV(height0, height0);   /* MATCH pin: load-bearing for the whole function shape */
                    ASM_USE2_NV(height0, height0);   /* MATCH pin: load-bearing for the whole function shape */
                    ...
                    xy3 = xy3 + cell_x;
                    xy3 = xy3 & 0xFFFF;
                    ASM_USE_NV(y2_high);   /* MATCH pin: load-bearing for the whole function shape */
                    S16(scratch, 0x74) = height0;
                    index1 = U16((u8 *)face, 2);
                    vert1_addr = index1 * 8 + (u32)vertices;
                    ASM_KEEP_DEP_NV(vert1_addr, y1_high);   /* MATCH pin: keeps a constant in a register as retail does */
    ...
    object_index = 0;
    object_slot = objects;
    ASM_USE_NV(vertices);   /* MATCH pin: retail register colouring depends on it */
    do {
        ASM_KEEP_NV(object_slot);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
