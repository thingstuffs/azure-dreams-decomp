...
void func_8001F450(void)
{
    ...
    void *special_label;
    s32 draw_mode;
    register s16 draw_x ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
    s32 draw_y_fixed;
    register u8 blank_code ASM_REG("$21");   /* MATCH pin: load-bearing for the whole function shape */
    u8 digit;
    special_label = 0;
    if (*(s32 *)0x80012090 != 2) {
        if (D_8008146C == 0x28) {
