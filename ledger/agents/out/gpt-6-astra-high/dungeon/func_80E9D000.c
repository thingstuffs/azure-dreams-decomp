...
BODY_STORAGE s32 BODY_NAME(void *origin, void *actor) {
    ...
    if (tile_slot >= 0) {
        ...
        ASM_KEEP(shared_base);   /* MATCH pin: keeps a statement from moving across a call/branch */
        item_data = D_800E3548[tile_slot];
        ASM_SCHED_BARRIER();   /* MATCH pin: keeps a statement from moving across a call/branch */
        shared_slot = shared_base + 3065;
        ASM_KEEP(shared_slot);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (target == (void *)D_800E3D7C[0]) {
        ...
        }
        {
            ...
            }
            {
                ...
                s32 *shared_base;
#ifdef __mips__
                register s32 shared_slot_addr ASM_REG("$2");   /* MATCH pin: retail delay-slot fill depends on it */
#else
                ...
                ASM_TAILSLOT_PIN(shared_slot_addr);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                func_8015EADC();
                __builtin_unreachable();
            }
        }
...
#ifdef __mips__
        s32 *shared_base;
        register s32 *shared_slot ASM_REG("$2");   /* MATCH pin: retail delay-slot fill depends on it */
#else
        s32 *shared_base;
        s32 *shared_slot;
#endif
