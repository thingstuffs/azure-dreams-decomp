...
s32 func_818B6954(s32 context_value, void *source_state, s32 render_param) {
    s16 offset_angle;
    register s32 coord_work ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    s32 state_word_3;
    s64_local position_xy;
    s32 state_word_4;
    s32 state_word_5;
    register s32 object_result ASM_REG("$2");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register s32 offset_y ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    s32 random_angle;
    s32 random_rotation;
    s32 angle_dividend;
    ...
    if (object != NULL) {
        ...
        ASM_KEEP(random_rotation);   /* MATCH pin: retail basic-block layout depends on it */
        ...
        ASM_KEEP(object_result);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
