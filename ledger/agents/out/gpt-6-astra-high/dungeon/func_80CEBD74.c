...
{
    register Obj0 *controller ASM_REG("$18");   /* MATCH pin: retail schedule: same instructions, different order without it */
    void *context;
    Obj2 *animation;
    register Obj3 *actor ASM_REG("$17");   /* MATCH pin: load-bearing for the whole function shape */
    s32 actor_kind;
    ...
    ASM_KEEP_NV(animation);   /* MATCH pin: load-bearing for the whole function shape */
    actor = actor_arg;
#ifndef __mips__
#endif
    ...
    case 0:
        ...
        {
            register u8 *dir_table ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
