...
s32 func_800247D4(void *effect_data)
{
    ...
    u8 *render_ctx;
    register u32 constant_or_count ASM_REG("$8");   /* MATCH pin: keeps a constant in a register as retail does */
    s32 color_base;
    s32 delta_mask;
    s32 radial_step;
    u8 *scratch;
    ...
    s32 angle;
    s32 stage_index;
    register s32 start_or_fade ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    s32 end_or_blue;
    s32 offset_or_depth;
    s32 y_or_blue;
    s32 start_depth;
    ...
    (void)stage_labels;
    ASM_USE_NV(initial_render_ctx);   /* MATCH pin: retail schedule: same instructions, different order without it */
    constant_or_count = 7;
    delta_mask = 0xFFFF0000;
    color_base = 0x40;
    ASM_USE2_NV(delta_mask, color_base);   /* MATCH pin: retail schedule: same instructions, different order without it */
    radial_step = 0x15;
    scratch = (u8 *)0x1F800000;
    ASM_KEEP(constant_or_count);   /* MATCH pin: keeps a statement from moving across a call/branch */
...
case_early:
    {
        ...
        ASM_KEEP(end_or_blue);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        constant_or_count = 15;
        {
            ...
            {
                register s32 red ASM_REG("$2") = 0x40 + red_offset;   /* MATCH pin: keeps a statement from moving across a call/branch */
...
shared:
        ...
        project_work_b = scratch + 0x88;
        {
            ...
            ASM_USE2_NV(start_pos, start_screen);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ((S_800247D4_0 *)scratch)->unk_F4 = start_depth;
        {
            ...
            ASM_USE2_NV(end_pos, end_screen);   /* MATCH pin: load-bearing for the whole function shape */
