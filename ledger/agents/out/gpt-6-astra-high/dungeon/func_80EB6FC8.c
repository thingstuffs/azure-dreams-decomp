    ...
    *(volatile u16 *)((s8 *)action + 0x98) = (u16) (((S_801727C8_0 *)action)->unk_98 & 0xFF7F);
    {
        ...
        ASM_KEEP(special_check);   /* MATCH pin: retail delay-slot fill depends on it */
        if (special_check == 0) {
            goto block_25;
        }
    }
...
block_27:
    {
        register void *owner ASM_REG("$2") = target_owner;   /* MATCH pin: load-bearing for the whole function shape */
...
block_34:
    ...
    {
        register s32 delay_ticks ASM_REG("$3") = 6;   /* MATCH pin: load-bearing for the whole function shape */
