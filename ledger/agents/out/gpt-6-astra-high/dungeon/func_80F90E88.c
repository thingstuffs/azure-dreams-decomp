...
s32 func_80F90E88(void *object) {
    LocalFrame frame;
    register s32 max_xy ASM_REG("$18");   /* MATCH pin: keeps a constant in a register as retail does */
    register s32 min_xy ASM_REG("$19");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 address_mask ASM_REG("$22");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s32 tag_mask;
    s32 *quad;
    s32 *draw_mode;
    s32 next_object;
    s32 mode_ot_offset;
    register s32 ot_offset ASM_REG("$20");   /* MATCH pin: retail register colouring depends on it */
    s32 bottom_y;
    s32 right_color;
    s32 left_color;
    register s32 quad_ot_slot ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    s32 side;
    register s8 *base ASM_REG("$8");   /* MATCH pin: keeps a constant in a register as retail does */
    register u32 depth ASM_REG("$23");   /* MATCH pin: retail register colouring depends on it */
    register S_80F90E88_5 *mode_ot_slot ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    register WorkCell *endpoint ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 half_width;
    s32 left_offset;
    void *transform;
    u16 view_angle;
    s32 position_xy;
    s32 position_z;
    register s8 *stack_base ASM_REG("$29");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 upper_mask ASM_REG("$8");   /* MATCH pin: keeps a constant in a register as retail does */
    s32 coord;
    register u32 coord_bits ASM_REG("$3");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 point_addr ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    register void *screen_out ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 *depth_out;
    s32 bound_test;
    u32 upper_coord;
    register s32 right_x ASM_REG("$3");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 left_x;
    s32 midpoint_x;
    u16 center_x;
    s32 mode_tag_mask;
    s32 quad_ot_link;
    s32 quad_tag;
    register s32 mode_ot_link ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 mode_tag;
...
draw_object:
    ...
    ASM_CLOBBER("$2");   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    ASM_KEEP_NV(transform);   /* MATCH pin: retail schedule: same instructions, different order without it */
    min_xy &= 0xFFFF;
    max_xy &= 0xFFFF;
    ...
    do {
        ...
        ASM_USE_NV(depth_out);   /* MATCH pin: keeps a statement from moving across a call/branch */
        point_addr = (s32) base + point_addr;
        depth = func_80065420(point_addr, screen_out, depth_out, depth_out) - 4;
        ASM_KEEP_NV(depth);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (depth < 0x1E0U) {
        ...
        ASM_USE(bottom_y);   /* MATCH pin: retail register colouring depends on it */
        ot_offset = depth * 4;
        tag_mask = 0xFF000000;
draw_side:
        base = (s8 *) &D_80083160_alloc;
        ASM_KEEP_DEP_NV(base, tag_mask);   /* MATCH pin: retail branch polarity depends on it */
...
return_zero:
    {
        register s32 hard_zero ASM_REG("$0");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
        register s32 result ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
#ifdef NON_MATCHING
        hard_zero = 0;
#else
#endif
