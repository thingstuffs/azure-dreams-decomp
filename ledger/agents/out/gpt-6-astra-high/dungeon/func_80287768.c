...
s16 func_8001A768(u16 *facing, s32 origin_x, s32 origin_y, s8 *tiles, s32 search_arg, s32 radius,
    ...
    u16 saved_radius;
    u16 saved_filter;
    register s32 tile_filter ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
    s32 forward_limit;
    s32 search_radius;
    s32 facing_high;
    s32 facing_offset;
    register u16 tile_index_u16 ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s32 initial_x_steps;
    s32 x;
    s32 y;
    s32 failures;
    register s32 tile_index ASM_REG("$16");   /* MATCH pin: keeps a constant in a register as retail does */
    s8 tile;
    s32 forward_steps;
    s32 sides_left;
    s32 side;
