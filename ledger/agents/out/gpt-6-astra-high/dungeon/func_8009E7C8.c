...
void *func_800A3F28(s32 x, s32 y, void *end, void *owner)
{
    ...
    void *sentinel;
    void *current;
    register s32 lookup_id ASM_REG("$17");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 target_x;
    register s32 scratch ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *entry_data;
    s32 distance_x;
    s32 distance_y;

    x_or_radius = x;
    ASM_KEEP_NV(x_or_radius);   /* MATCH pin: keeps a statement from moving across a call/branch */
    target_y = y;
    sentinel = end;
    current = owner;
