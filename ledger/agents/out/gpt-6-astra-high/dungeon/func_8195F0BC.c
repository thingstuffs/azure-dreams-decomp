...
void func_8195F0BC(DungeonState *state, DungeonOrigin *origin) {
    register FadeColor *fade_color ASM_REG("$5");   /* MATCH pin: load-bearing for the whole function shape */
    s32 grid_x;
    register s32 grid_y ASM_REG("$19");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    s32 tile_x;
    s32 grid_y_fixed;
    s32 tile_y;
    s32 page_base;
    ...
    u16 *height_ptr;
    register u16 *height_row ASM_REG("$22");   /* MATCH pin: load-bearing for the whole function shape */
    void *tile;
    s32 height;
    u16 timer;
    register s32 lookup_x ASM_REG("$5");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 lookup_y ASM_REG("$6");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    void *page;
    u8 *delta_ptr;
    u8 height_delta;
