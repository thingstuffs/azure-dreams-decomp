...
{
    register s16 held_width ASM_REG("$16") = width;   /* MATCH pin: load-bearing for the whole function shape */
    s32 tile_y;
    s32 tile_x;
    s32 tiles_base;
    s32 row_width;
    ...
    ASM_KEEP(held_width);   /* MATCH pin: retail schedule: same instructions, different order without it */
}
