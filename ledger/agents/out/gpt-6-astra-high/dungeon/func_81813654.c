...
void *func_80028654(void *packet, void *coords) {
    register u32 packet_type ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
    ...
    *((u8 *)packet + 0xB) = coord_bytes[6];
    packet_type = 3;
    ASM_KEEP(packet_type);   /* MATCH pin: retail schedule: same instructions, different order without it */
