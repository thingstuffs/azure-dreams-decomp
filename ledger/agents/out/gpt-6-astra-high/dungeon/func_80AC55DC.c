    ...
    s32 offset_x, s32 offset_y, s32 offset_z)
{
    ...
    s32 offset_fixed;
    s32 duration_signed;
    register s32 duration_eighth ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (node != 0) {
        ...
        ASM_KEEP(duration_eighth);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        ...
        ASM_KEEP(sprite_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
