...
{
    register void *state ASM_REG("$17") = entity_state;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void *motion = entity_motion;
    register void *part ASM_REG("$19") = entity_part;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    if (D_80083462 & 0x2000) {
        ...
        if (early_callback == (EntityCallback)D_80170F74) {
            register void *incoming_state ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */

            ASM_KEEP(incoming_state);   /* MATCH pin: retail register colouring depends on it */
...

    {
        ...
        {
            register void *call_state ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
            register void *call_motion ASM_REG("$5");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
            register void *call_part ASM_REG("$6");   /* MATCH pin: retail delay-slot fill depends on it */
            void *call_context;

            ASM_KEEP(call_state);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            ASM_KEEP(call_part);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            call_state = state;
            call_motion = motion;
            call_part = part;
