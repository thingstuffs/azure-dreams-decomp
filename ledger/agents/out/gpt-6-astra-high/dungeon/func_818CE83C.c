    ...
    for (;;) {
        ...
        *(u8 * volatile *)(scratch + 0x18) = packet + 0xC;
        ...
        if (depth_index < 0x1E0) {
            ...
            register s32 texture_depth ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
            ...
            opcode = 0x6A;
            ASM_SET(page_x);   /* MATCH pin: keeps a statement from moving across a call/branch */
            page_x = texture_depth;
            ASM_KEEP_DEP_NV(page_x, opcode);   /* MATCH pin: keeps a statement from moving across a call/branch */
