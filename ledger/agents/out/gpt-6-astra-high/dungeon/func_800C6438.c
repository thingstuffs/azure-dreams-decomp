...
void *func_800CBB98(s32 tile_x, s32 tile_y, s16 height, void *owner) {
    register s16 saved_height ASM_REG("$20") = height;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register S_800CBB98_3 *saved_owner ASM_REG("$19") = owner;   /* MATCH pin: retail schedule: same instructions, different order without it */
    register s32 saved_tile_x ASM_REG("$17") = tile_x;   /* MATCH pin: retail schedule: same instructions, different order without it */
    register s32 saved_tile_y ASM_REG("$18") = tile_y;   /* MATCH pin: retail schedule: same instructions, different order without it */
    S_800CBB98_2 *sprite;
    S_800CBB98_0 *effect;
    ...
    if (effect != NULL) {
        ...
        ASM_KEEP(saved_owner);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ASM_KEEP(saved_tile_x);   /* MATCH pin: keeps a statement from moving across a call/branch */
        ASM_KEEP(saved_tile_y);   /* MATCH pin: keeps a statement from moving across a call/branch */
