...
void func_800B8FC8(s32 sprite, Position *clip_rect, Position *screen_pos, s32 clear_bg, volatile s32 draw_flags)
{
    ...
    void *ordering_table;
    Packet *area_packet;
    register s32 saved_clear_bg ASM_REG("$23");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 shift_y ASM_REG("$16");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    *(Packet **)(context + 0x8D0) = (Packet *)((u8 *)area_packet + 0xC);
    ...
    ASM_KEEP_DEP_NV(area_packet, saved_draw_flags);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    y = screen_pos->y;
    {
        ...
        ASM_KEEP(saved_clear_bg);   /* MATCH pin: load-bearing for the whole function shape */
