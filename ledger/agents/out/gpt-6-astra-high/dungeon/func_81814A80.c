...
void func_81814A80(State *state, s32 *sum, Flags *flags) {
    s32 update_count;
    register s32 more_updates ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    u16 next_tick;
...
mode1:
    if (state->unk1a > 0) {
        update_count = 0;
        ASM_KEEP(update_count);   /* MATCH pin: load-bearing for the whole function shape */
        func_800478B8(flags);
