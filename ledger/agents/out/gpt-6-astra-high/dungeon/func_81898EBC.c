...
/* Advance the effect through targeting, movement, spawning, and cleanup. */
void func_81898EBC(EffectState *effect_state, Motion *effect_motion, ColorPart *color_part)
{
    ...
    case 0:
...

        {
            ...
            u32 origin_x;
            register u32 x_offsets ASM_REG("$7");
            u32 x_offset;
            u32 y_offset;
    ...
    case 1:
        ...
        source_pos = PTR_AT(owner_meta, 8);
...

        {
            ...
            if ((U16_AT(owner_meta, 0x1E) | 0x2000) != 0) {
            ...
            do {
                ...
                axis_distance = S16_AT(axis_ptr, 24);
                if (state->duration < axis_distance) {
                    state->duration = U16_AT(axis_ptr, 24);
                }
                axis++;
                axis_ptr += 2;
            } while (axis < 3);
    ...
    case 4:
        ...
        if (state->status != 0) {
            ...
            U8_AT(effect, 0x28) = clamped_shade;
            U8_AT(effect, 0x29) = clamped_shade;
        }
advance_state:
        state->timer = 0;
        state->state++;
