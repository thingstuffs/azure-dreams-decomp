...
void BODY_NAME(void *quad, void *position, void *object, s16 depth_bias)
{
    ...
    FIELD(scratch, s32, 0x88) = FIELD(position, s16, 2);
    FIELD(scratch, s32, 0x8C) = FIELD(position, s16, 6);
    FIELD(scratch, s32, 0x90) = FIELD(position, s16, 0xA);
...

    {
        ...
        packet = FIELD(render_context, u8 *, 0x8D0);
        FIELD(render_context, u8 *, 0x8D0) = packet + 0x24;
    }

    FIELD(scratch, u16, 0xB0) = FIELD(quad, u16, 0x4C);
    FIELD(scratch, u16, 0xB8) = FIELD(quad, u16, 0x52);
    FIELD(scratch, u16, 0xC0) = FIELD(quad, u16, 0x58);
    FIELD(scratch, u16, 0xC8) = FIELD(quad, u16, 0x5E);
    FIELD(scratch, u16, 0xB2) = FIELD(quad, u16, 0x4E);
    FIELD(scratch, u16, 0xBA) = FIELD(quad, u16, 0x54);
    FIELD(scratch, u16, 0xC2) = FIELD(quad, u16, 0x5A);
    FIELD(scratch, u16, 0xCA) = FIELD(quad, u16, 0x60);
    FIELD(scratch, u16, 0xB4) = FIELD(quad, u16, 0x50);
    FIELD(scratch, u16, 0xBC) = FIELD(quad, u16, 0x56);
    FIELD(scratch, u16, 0xC4) = FIELD(quad, u16, 0x5C);
    FIELD(scratch, u16, 0xCC) = FIELD(quad, u16, 0x62);

    depth = func_80065590(scratch + 0xB0, scratch + 0xB8,
                          scratch + 0xC0, scratch + 0xC8,
                          packet + 8, packet + 0x10,
                          packet + 0x18, packet + 0x20,
                          scratch + 0xD0, scratch + 0xD4) - depth_bias - 6;
    FIELD(scratch, s32, 0x100) = depth;
    ...
    if ((u32)depth < 0x1E0) {
        if ((((u16)(FIELD(packet, u16, 8) + 0x20) < 0x181) &&
            ...
            tex_flags = texture[1];
            FIELD(object, u8, 0xF) = tex_flags;
            draw_flags = FIELD(scratch, u16, 0x28);
            if (draw_flags & 8) {
                FIELD(object, u8, 0xF) = (draw_flags & 4) ?
                    (tex_flags | 2) : (tex_flags & 0xFD);
            }

            FIELD(packet, u32, 4) = FIELD(quad, u32, 0xC);
            FIELD(packet, u32, 0xC) = FIELD(quad, u32, 0x10);
            FIELD(packet, u32, 0x14) = FIELD(quad, u32, 0x14);
            FIELD(packet, u32, 0x1C) = FIELD(quad, u32, 0x18);
            FIELD(packet, u8, 3) = 8;
            ...
            packet = FIELD(*render_context_ref, u8 *, 0x8D0);
            FIELD(*render_context_ref, u8 *, 0x8D0) = packet + 0xC;
