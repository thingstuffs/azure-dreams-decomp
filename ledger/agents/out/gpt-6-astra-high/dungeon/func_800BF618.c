...
void func_800C4D78(s32 payload, s32 trigger_event) {
    S_800C4D78_0 *object;
    u8 *counter_base;
    register s32 held_payload ASM_REG("$17") = payload;   /* MATCH pin: retail register colouring depends on it */
    register s32 held_event ASM_REG("$16") = trigger_event;   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (object != NULL) {
        ...
        ASM_KEEP(held_payload);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ASM_KEEP(held_event);   /* MATCH pin: keeps a statement from moving across a call/branch */
    }
}
