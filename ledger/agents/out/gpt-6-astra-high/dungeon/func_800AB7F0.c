...
void *func_800B0F50(void *owner)
{
  ...
  s32 prim_flags;
  s32 count_or_color;
  register s32 green_color ASM_REG("$6");
  register s32 blue_color ASM_REG("$7");
  s32 color_or_addr;
  register s32 vertex_color ASM_REG("$2");
  s32 text_width;
  register s32 text_addr ASM_REG("$4");
  register s32 red_color ASM_REG("$3");
  u16 text_id;
  void *owner_pos;
  void *panel_pos;
  void *digits;
    ...
    *((void **) (((s8 *) panel_data) + 0x28)) = prim;
    if (prim != 0)
    {
      ...
      *((s32 *) (((s8 *) panel_data) + 0x18)) = count_or_color;
      ...
      ASM_KEEP_NV(vertex_color);
      ASM_KEEP_NV(red_color);
      red_color |= 0x2040;
