...
BODY_STORAGE s32 BODY_NAME(void *origin, void *actor) {
    ...
    if (ground_slot >= 0) {
        ...
        ASM_KEEP(item_dest);   /* MATCH pin: keeps a statement from moving across a call/branch */
        item_data = D_800E3548[ground_slot];
        ASM_SCHED_BARRIER();   /* MATCH pin: keeps a statement from moving across a call/branch */
        item_slot = item_dest - 3079;
        ASM_KEEP(item_slot);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (target == (void *)D_800E3D7C[0]) {
        ...
        }
        {
            ...
            }
            {
                ...
                s32 *item_dest;
#ifdef __mips__
                register s32 item_slot_addr ASM_REG("$2");   /* MATCH pin: retail delay-slot fill depends on it */
#else
                s32 item_slot_addr;
#endif
                ...
                ASM_TAILSLOT_PIN(item_slot_addr);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                func_80158ADC();
                __builtin_unreachable();
            }
        }
...
#ifdef __mips__
        s32 *item_dest;
        register s32 *item_slot ASM_REG("$2");   /* MATCH pin: retail delay-slot fill depends on it */
#else
        s32 *item_dest;
        s32 *item_slot;
#endif
