...
#endif
        ASM_KEEP_NV(room_page);   /* MATCH pin: retail immediate-load split depends on it */
...
next_direction:
    ...
    {
        s32 target_x;
        register s32 distance_y ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
        s32 y_is_farther;
        s32 entity_flags;
...
move_entity:
    {
        register s32 world_x ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
        register s32 tile_origin_y ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
        register s32 world_y ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
        s32 aligned_x;

        world_x = ((S_807B040C_1 *)entity)->unk_AA.s16 << 6;
        ASM_KEEP_NV(world_x);   /* MATCH pin: load-bearing for the whole function shape */
        world_x += 0x20;
        tile_origin_y = ((S_807B040C_1 *)entity)->unk_AE.s16 << 6;
        ASM_KEEP_NV(tile_origin_y);   /* MATCH pin: load-bearing for the whole function shape */
