...
s32 func_80172504(void *action_state, void *transfer_data, void *origin, void *actor) {
    register void *entity ASM_REG("$20") = actor;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *state ASM_REG("$23") = action_state;   /* MATCH pin: retail register colouring depends on it */
    u8 *x_delta_table;
    register u32 initial_direction ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    s16 *delta_x;
    s16 *delta_y;
    register s32 step_count ASM_REG("$21");   /* MATCH pin: retail register colouring depends on it */
    register s32 tile_x ASM_REG("$19");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 tile_y ASM_REG("$18");   /* MATCH pin: load-bearing for the whole function shape */
    s32 signed_x;
    s32 signed_y;
    ...
    s16 next_height;
    u16 direction;
    register u32 origin_or_direction ASM_REG("$8");   /* MATCH pin: load-bearing for the whole function shape */
