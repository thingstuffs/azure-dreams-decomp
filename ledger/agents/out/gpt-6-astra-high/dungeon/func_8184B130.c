                  ...
                  s32 height, s16 radius, u8 blend_mode, u8 brightness)
{
    ...
    for (segment = 0; segment < 16; segment++) {
        register u8 *prim;
        register s32 next_angle ASM_REG("$16");   /* MATCH pin: load-bearing for the whole function shape */
        s32 angle_offset;
        s32 raw_angle;
        s32 biased_angle;
        s32 angle;
        ...
        {
            register s32 texture_u_max ASM_REG("$3") = 0x3F;   /* MATCH pin: keeps a statement from moving across a call/branch */
...

        {
            ...
            if (depth_or_mask < 0x1E0U) {
                register u32 address_mask ASM_REG("$4") = 0x00FFFFFFU;   /* MATCH pin: retail immediate-load split depends on it */
