...
void func_819A764C(void *effect_data)
{
    ...
    s32 state;
    s32 point_index;
    register void *point_slot ASM_REG("$17");   /* MATCH pin: load-bearing for the whole function shape */
    void *object_base;
    S_819A764C_1 *linked_object;
    register u32 page_base ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    u8 *message;
    register void *self ASM_REG("$18") = effect_data;   /* MATCH pin: load-bearing for the whole function shape */
