...
void func_818BC888(void *effect) {
    ...
    if (phase < 0x10) {
        ...
        ASM_TAILSLOT_PIN(intensity);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        func_8002414C();
        return;
    }
...
tail_c0:
    cycle_value = 0x7E00;
    ASM_TAILSLOT_PIN(cycle_value);   /* MATCH pin: retail delay-slot contents depend on it */
