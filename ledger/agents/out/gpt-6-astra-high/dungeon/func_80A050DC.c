...
void *func_8015E8DC(s32 kind, s32 part_byte_24, s32 part_byte_25, s32 copy_value)
{
    ...
    register s32 saved_part_byte_25 ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    void *allocated;
    register uptr setup_scratch ASM_REG("$8");   /* MATCH pin: keeps a constant in a register as retail does */
    s32 allocation_size;
...
flags_done:
    ...
    do {
        ...
        if (allocated != 0) {
            ...
            {
                register u8 *direction_page ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */

                direction_page = (u8 *)0x80160000;
                ASM_KEEP(direction_page);   /* MATCH pin: load-bearing for the whole function shape */
