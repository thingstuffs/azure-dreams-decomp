...
s32 FUNC_80AE9000_BODY(Input0 *render_data, Input1 *position_data) {
    ...
    register u32 addr_mask ASM_REG("$18") = 0x00FFFFFF;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    register u32 red ASM_REG("$3");   /* MATCH pin: retail basic-block layout depends on it */
    register u32 green ASM_REG("$6");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register u32 blue ASM_REG("$7");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 pinned_zero ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register s32 pinned_one ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register u32 color_word ASM_REG("$3");   /* MATCH pin: retail basic-block layout depends on it */
    u8 *packet;
    u32 draw_mode;
    u8 *packet_start;
    void *linked_node;

    ASM_KEEP(render_bytes);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    ASM_KEEP(context_addr);   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    if (scratch->index < 0x1E0U) {
        ...
        *(u32 *)packet = (*(u32 *)packet & length_mask) |
        ...
        {
            register u32 ot_tag ASM_REG("$3");   /* MATCH pin: retail basic-block layout depends on it */
    ...
    if (linked_node != 0) {
        ...
        ASM_KEEP(render_bytes);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
        ASM_KEEP(position_bytes);   /* MATCH pin: keeps a statement from moving across a call/branch */
        {
            register u32 scratch_addr ASM_REG("$17");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
            register u32 position_addr ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
#ifdef NON_MATCHING
            ...
            ASM_TAILSLOT_PIN(position_addr);   /* MATCH pin: retail delay-slot contents depend on it */
        }
        func_8014C8FC();
    }
