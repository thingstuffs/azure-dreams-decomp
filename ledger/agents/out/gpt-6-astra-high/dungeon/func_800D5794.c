...
void func_800DAEF4(void *source_owner, void *spawn_params)
{
    void *owner;
    void *params;
    register s32 remaining ASM_REG("$21");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *link;
    void *object;
    void *node;
    ...
    do {
        ...
        if (object != 0) {
            ...
            do {
                ...
                register s32 roll_remainder ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
                s32 biased_roll;
                register s32 offset_sum ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
                ...
                ASM_USE_NV(offset_roll_b);   /* MATCH pin: retail register colouring depends on it */
