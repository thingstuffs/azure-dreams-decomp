...
void func_80921B2C(S_80921B2C_3 *position, s32 x_offset, s32 y_offset, s32 z_offset) {
    ...
    u16 render_flags;
    register s32 effect_type ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register u8 *global_state ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register u8 *direction_state ASM_REG("$22");   /* MATCH pin: load-bearing for the whole function shape */
    S_80921B2C_2 *motion;
    S_80921B2C_5 *sprite;
    S_80921B2C_0 *effect;
    ...
    effect_type = 0x212;
    ASM_USE_NV(effect_type);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if (effect != NULL) {
        ...
        func_8004491C(effect, D_80045340);
        {
            ...
            ASM_USE(render_flags);   /* MATCH pin: keeps a statement from moving across a call/branch */
