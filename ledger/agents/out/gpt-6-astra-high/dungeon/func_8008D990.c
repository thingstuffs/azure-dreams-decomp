...
jt_c1:
    ...
    func_800945E8(state);
    func_800948BC();
    ASM_KEEP(saved_state_base);   /* MATCH pin: load-bearing for the whole function shape */
    {
        ...
        register s32 copy_word ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
        s32 middle_word;
...
block_23:
    ...
    {
        s32 command;
        register s32 zero_arg_1 ASM_REG("$5");   /* MATCH pin: keeps a constant in a register as retail does */
        register s32 zero_arg_2 ASM_REG("$6");   /* MATCH pin: keeps a statement from moving across a call/branch */
        register s32 zero_arg_3 ASM_REG("$7");   /* MATCH pin: load-bearing for the whole function shape */
        u8 *flags_base;
        register u16 flags ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */

        command = 6;
        zero_arg_1 = 0;
        zero_arg_2 = zero_arg_1;
        zero_arg_3 = zero_arg_1;
        ASM_KEEP4_NV(command, zero_arg_1, zero_arg_2, zero_arg_3);   /* MATCH pin: retail schedule: same instructions, different order without it */
        flags_base = (u8 *)0x80080000;
        ASM_KEEP(flags_base);   /* MATCH pin: keeps a statement from moving across a call/branch */
        flags = 0x8000;
