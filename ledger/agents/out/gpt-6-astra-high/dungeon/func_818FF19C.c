...
void func_818FF19C(void *effect, void *offset_state, void *render_state) {
    ...
    if (offset_applied == 0) {
        ...
        offset_step = -0x20000;
        ASM_KEEP(old_offset);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        ASM_TAILSLOT_PIN(offset_step);   /* MATCH pin: retail delay-slot contents depend on it */
        func_80024A54();
    }
