...
have_choice:
    ASM_KEEP(move_data);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    if (*move_data != 0) {
        ((S_80172970_0 *)action)->unk_98 &= 0xFF7F;
        special_check = is_special;
        ASM_KEEP(special_check);   /* MATCH pin: retail basic-block layout depends on it */
        if (special_check != 0) {
            target = D_800814A8;
