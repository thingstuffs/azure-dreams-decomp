...
void *func_8017087C(s16 kind_flags, s8 tile_x, s8 tile_y, s16 part_id)
{
    ...
    s32 flags_or_roll;
    s32 secondary_flags;
    register s8 saved_tile_x ASM_REG("$22");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s16 saved_part_id;
    register s8 saved_tile_y ASM_REG("$21");   /* MATCH pin: retail schedule: same instructions, different order without it */
    void *object_arg;
