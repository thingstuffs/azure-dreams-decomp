...
{
    register void *object ASM_REG("$16") = object_arg;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register void *motion ASM_REG("$17") = motion_arg;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register void *actor ASM_REG("$18") = actor_arg;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register void *room ASM_REG("$19") = room_arg;   /* MATCH pin: retail schedule: same instructions, different order without it */
    s16 timer;
    u16 old_timer;
    s32 room_ref;
    s32 *global_state;
