...
s32 func_81988C1C(void *first_item) {
    ...
    register void *next_node ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    ...
    do {
        ...
        if (depth_index < 0x1E0U) {
            ...
            register u8 tex_v;
            u8 tex_u;
            register u8 tex_width ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
            u16 screen_coord;
            register u32 ot_slot ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
            u32 addr_mask;
            register u32 color_or_tag_mask ASM_REG("$5");   /* MATCH pin: load-bearing for the whole function shape */
            ...
            ASM_KEEP(addr_mask);   /* MATCH pin: retail immediate-load split depends on it */
        }
...

    {
        ...
        register s32 return_value ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
        READ_ZERO(zero);
        return_value = zero;
        ASM_KEEP(return_value);   /* MATCH pin: retail schedule: same instructions, different order without it */
        return return_value;
    }
}
