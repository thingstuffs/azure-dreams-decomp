...
{
    s32 saved_mode;
    register s32 saved_byte_24 ASM_REG("$21");   /* MATCH pin: retail schedule: same instructions, different order without it */
    long value_or_part_b;
    long byte_or_work;
    register s32 alloc_code ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register void *alloc_data ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 kind;
    s32 kind_test;
    s32 flags_14;
    s32 flags_1c;
    ...
    ASM_KEEP_NV(alloc_code);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ASM_KEEP_NV(alloc_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
