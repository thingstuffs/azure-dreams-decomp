...
s32 func_81977584(Record *record) {
    ...
    s32 segment;
    register s32 prev_upper_color ASM_REG("$23");   /* MATCH pin: retail keeps record copy the compiler would otherwise drop/add */
    s32 lower_shade;
    s32 upper_shade;
    ...
    ASM_KEEP(shade_step);   /* MATCH pin: retail keeps record copy the compiler would otherwise drop/add */
    shade_step &= 0xffff;
    ASM_TAILSLOT_PIN(shade_step);   /* MATCH pin: retail delay-slot contents depend on it */
    func_80024E38();
...
type_3:
    {
        ...
        register s32 index_offset ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */

        ASM_KEEP_NV(shade_base);   /* MATCH pin: retail immediate-load split depends on it */
...

    {
        ...
        {
            register s32 color_pair ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
            s32 intensity;
            register s32 color_work ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
            register s32 shade_work ASM_REG("$5");   /* MATCH pin: load-bearing for the whole function shape */
            register s32 end_color ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */

            shade_work = (shade_step * 3) << 3;
            intensity = ((shade_work - shade_step) >> 16) & 0xff;
            ASM_KEEP_NV(intensity);   /* MATCH pin: load-bearing for the whole function shape */
            color_work = (intensity << 8) + intensity;
            lower_color = (color_work << 8) + intensity;
            ASM_KEEP_NV(lower_color);   /* MATCH pin: retail register colouring depends on it */

            trig_value >>= 4;
            ASM_KEEP_NV(trig_value);   /* MATCH pin: retail schedule: same instructions, different order without it */
            ...
            ASM_KEEP(intensity);   /* MATCH pin: retail keeps record computation the compiler would drop */
            intensity &= 0xff;
            ASM_KEEP_NV(intensity);   /* MATCH pin: load-bearing for the whole function shape */

            shade_work = (shade_work >> 16) & 0xff;
            ASM_KEEP_NV(shade_work);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    do {
        ...
        ASM_KEEP_NV(sample);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_KEEP_NV(scaled_coord);   /* MATCH pin: load-bearing for the whole function shape */
