...
void func_80172B00(void *parent_data, S_80172B00_6 *source_pos, void *sprite_template) {
    ...
    s8 *view_state;
    register CopyBlock *copy_dst ASM_REG("$6");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    CopyBlock *copy_src;
    ...
    if (effect_object != NULL) {
        ...
        ASM_KEEP(template_1c);   /* MATCH pin: retail schedule: same instructions, different order without it */
        setup_object = effect_object;
        ASM_KEEP(setup_object);   /* MATCH pin: load-bearing for the whole function shape */
