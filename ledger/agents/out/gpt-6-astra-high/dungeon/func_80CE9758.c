...
void func_80172F58(u8 *move_input, void *action_context, u8 *position_input, u8 *actor_input) {
    register u8 *move_state ASM_REG("$21") = move_input;   /* MATCH pin: retail delay-slot fill depends on it */
    register u8 *position ASM_REG("$20") = position_input;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register u8 *actor ASM_REG("$18") = actor_input;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    register s32 move_found ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s32 tile_mask;
    s32 base_angle;
    s16 move_angle;
    u8 next_x;
    ...
    ASM_KEEP(move_state);   /* MATCH pin: retail basic-block layout depends on it */
