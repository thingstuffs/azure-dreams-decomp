...
s32 func_81845068(u8 *first_node)
{
    ...
    u8 *node;
    u8 *effect_node;
    register u8 *next_link ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    u8 *render_state;
    register u8 *alloc_ctx ASM_REG("$3");   /* MATCH pin: retail immediate-load split depends on it */
    u8 *ot_ctx;
    register u8 *ot_entry ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    u8 *prim;
    Coord *screen_base;
    Coord *screen_pos;
    ...
    s32 tex_u_end;
    s32 random_value;
    register u32 addr_mask ASM_REG("$19");   /* MATCH pin: retail register colouring depends on it */
    register u32 ot_or_coord ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u16 vertex_x;
    u16 base_y;
    u16 tip_y;
    ...
    do {
        ...
        if ((u32)depth_bucket < 0x1E0) {
            ...
            {
                register u32 tag_mask ASM_REG("$5") = 0xFF000000;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    ...
    ASM_SET(next_link);   /* MATCH pin: retail register colouring depends on it */
    return 0;
}
