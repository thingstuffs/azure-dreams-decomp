...
s32 func_8009B88C(u8 *entry, s32 target_x, s32 target_y, s16 *out_x, s16 *out_y) {
    ...
    s32 far_y;
    s32 wrap_pending;
    register s32 wrap_check ASM_REG("$2");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    ...
    ASM_KEEP_NV(entry);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
...
search_nearby:
    ...
    near_y = (s16)target_y;
    ASM_USE(near_y);   /* MATCH pin: load-bearing for the whole function shape */
    slot = search_seed & 7;
    do {
        near_dir = slot;
        ASM_KEEP_NV(near_dir);   /* MATCH pin: retail keeps a computation the compiler would drop */
        ASM_USE2(target_x, target_x);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ASM_USE2(target_x, target_x);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ASM_USE2(target_y, target_y);   /* MATCH pin: keeps a constant in a register as retail does */
        if (*(occupied + near_dir + 1) == 0) {
            near_dx = D_8006CCD8 + near_dir;
            near_dy = D_8006CCE8 + near_dir;
            ASM_USE(near_dy);   /* MATCH pin: retail register colouring depends on it */
    ...
    do {
        far_dir = slot;
        ASM_KEEP_NV(far_dir);   /* MATCH pin: retail keeps a computation the compiler would drop */
        ASM_USE2(target_x, target_x);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ASM_USE2(target_y, target_y);   /* MATCH pin: keeps a constant in a register as retail does */
        if (*(occupied + far_dir + 9) == 0) {
            far_dx = D_800DCE6C + far_dir;
            far_dy = D_800DCE8C + far_dir;
            ASM_USE(far_dy);   /* MATCH pin: retail register colouring depends on it */
    ...
    ASM_USE_NV(entry);   /* MATCH pin: load-bearing for the whole function shape */
    return 0;
}
