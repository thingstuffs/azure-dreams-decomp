...
void func_800DCC3C(u8 *object) {
    ...
    register s32 vector_offset ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    s32 record_offset;
    register s32 vector_addr ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s32 vector_z;

    header = object;
    entry_index = 0;
    ASM_KEEP(entry_index);   /* MATCH pin: load-bearing for the whole function shape */
    record_page = (u8 *)0x80030000;
    ASM_KEEP(record_page);   /* MATCH pin: load-bearing for the whole function shape */
    record_template = (Template0 *)(record_page - 0x1A28);
    ASM_KEEP(record_template);   /* MATCH pin: load-bearing for the whole function shape */
    tail_byte = 4;
    ASM_KEEP(tail_byte);   /* MATCH pin: retail schedule: same instructions, different order without it */
    vector_page = (u8 *)0x80030000;
    ASM_KEEP(vector_page);   /* MATCH pin: retail immediate-load split depends on it */
    vector_template = (Vec3 *)(vector_page - 0x1A18);
    ASM_KEEP(vector_template);   /* MATCH pin: retail register colouring depends on it */
