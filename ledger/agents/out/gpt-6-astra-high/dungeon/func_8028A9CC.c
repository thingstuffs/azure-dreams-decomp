...
void func_8001D9CC(s16 left_x, s16 top_y, s16 right_x, s16 bottom_y, s32 tile_value_base) {
    ...
    register s32 tile_value ASM_REG("$11");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    ASM_USE_NV(bottom_row);   /* MATCH pin: load-bearing for the whole function shape */
    if (y < bottom_row) {
        do {
            y++;
            tile[0] = 0x11;
