...
void func_80174D24(void *state, void *output, void *target)
{
    ...
    void *object;
    Copy24 *source;
    register s32 copy_word_a ASM_REG("$3");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 copy_word_b;
    register s32 copy_word_c ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    register s32 copy_word_d ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
    s32 offset_mode;
    s32 object_id;
    s32 phase;
    u16 old_ticks;
    register u16 ticks ASM_REG("$3");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 signed_ticks ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    u16 target_flags;
    ...
    offset_mode = 0;
    ASM_KEEP(offset_mode);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    ticks = old_ticks + 1;
    ASM_KEEP(old_ticks);   /* MATCH pin: load-bearing for the whole function shape */
...
mode_zero:
    ...
    target_flags = ((S_80174D24_3 *)target)->unk_14;
    if ((target_flags & 0x6000) == 0) {
        goto out;
    }
    ((S_80174D24_3 *)target)->unk_14 = target_flags | 0x80;
...
mode_one:
    ...
    ((S_80174D24_3 *)target)->unk_14 &= 0xFF7F;
