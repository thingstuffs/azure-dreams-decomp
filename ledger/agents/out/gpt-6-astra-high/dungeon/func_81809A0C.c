...
void func_8002520C(void *menu_in, void *motion_in, void *appearance_in) {
    ...
    S_8002520C_14 *actor;
    void *side_object;
    register void *side_count ASM_REG("$5");
    void *side_cursor;
    u8 *state_base;
...
handle_input:
    ...
    if (D_80027156[0] == 0) {
        ...
        if ((input_angle != 0xFFF) && (((S_8002520C_4 *)state_base)->unk_10 & 0xF000)) {
            ...
            void *side_data;
            u16 *x_step_ptr;
            register S_8002520C_6 *cell ASM_REG("$8");
            ...
            cell = ((S_8002520C_5_pre *)side_data)[-1].unk_00;
            {
                ...
                ASM_KEEP_DEP_NV(base_x, step_value);
                x_value = base_x;
                y_value = cell_coord + step_value;
            }
            target_y = (u16) y_value;
            ASM_USE2_NV(step_index, side_data);
...
check_other_side:
            ...
            if (*other_slot != NULL) {
                ...
                if (target_x != other_x) {
                    ...
                    if (y_value == ((S_8002520C_8 *)((void *) step_or_cell))->unk_25) {
                        register s32 y_step_value ASM_REG("$2");
                        s32 x_step_value;

                        step_or_cell = step_index & 0xFFFF;
                        ASM_KEEP_NV(step_or_cell);
        ...
        if (((S_8002520C_4 *)state_base)->unk_10 & 0x40) {
            ...
            func_800A56E0(0x503);
            {
                ...
                register S_8002520C_12 *world_cell ASM_REG("$8");

                center_x = 1;
                center_y = center_x;
                ASM_KEEP_NV(center_x);
                ASM_KEEP_NV(center_y);
                world_page = (s32 *)0x800E0000;
                ASM_SCHED_BARRIER();
                side_offset = ((S_8002520C_0 *)menu)->unk_26;
                ASM_KEEP_NV(side_offset);
...
animate_exit:
            ...
            if ((exit_left << 0x10) <= 0) {
                s32 object_or_flags;
                s32 index_or_object;
                register s32 flags_or_page ASM_REG("$2");
                s32 object_or_state;
                register s32 *side_ptr ASM_REG("$6");
                s32 *flags_page;
