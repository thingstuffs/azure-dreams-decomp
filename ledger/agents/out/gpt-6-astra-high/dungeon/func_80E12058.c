...
void *func_80175858(void *direction_src, Copy24 *initial_data, void *render_src) {
    ...
    register u16 render_attr ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    void *direction_ref = direction_src;
    Copy24 *data_src = initial_data;
    void *render_ref = render_src;

    ASM_KEEP_NV(direction_ref);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP_NV(data_src);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ASM_KEEP_NV(render_ref);   /* MATCH pin: load-bearing for the whole function shape */
...
allocated:
    ...
    ASM_KEEP_NV(object_state);   /* MATCH pin: retail schedule: same instructions, different order without it */
