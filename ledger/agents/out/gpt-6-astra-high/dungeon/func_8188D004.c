...
void *func_80024804(s32 type_code, void *source, s32 unused, s32 object_value)
{
    ...
    register s32 stored_value ASM_REG("$21") = object_value;   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 type_id;
    s32 step;
    void *child;
    void *object;
    ...
    if (object != 0) {
        ...
        *(s32 *)((u8 *)object + 0x20) = stored_value;
        ASM_KEEP(stored_value);   /* MATCH pin: retail schedule: same instructions, different order without it */
