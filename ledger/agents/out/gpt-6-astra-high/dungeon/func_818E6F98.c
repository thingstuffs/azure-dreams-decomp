...
s32 func_818E6F98(void *sprite_data, void *position)
{
    ...
    register s16 *screen_base ASM_REG("$23");   /* MATCH pin: load-bearing for the whole function shape */
    register WorkOutput *scratch_base ASM_REG("$20");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s16 *screen_cursor;
    s32 point_index;
    u32 ordering_index;
    s32 width;
    ...
    if (node != 0) {
        ...
        ASM_USE2(sprite_data, callback_state);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        func_800247D4();
    }
    ASM_MEM_BARRIER();   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    {
