...
void func_80173CD4(void *action_in, void *context_in, void *sprite_in, void *actor_in)
{
    ...
    register void *sprite ASM_REG("$17") = sprite_in;   /* MATCH pin: retail callee-packed_adjustment set / frame layout depends on it */
    void *actor;
    register u8 *world_state ASM_REG("$20");   /* MATCH pin: retail address form (%hi/%lo vs world_counters+offset) depends on it */
...
state_one:
    {
        register u32 world_page ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
        u32 actor_flags;
        ...
        ASM_KEEP(world_page);   /* MATCH pin: load-bearing for the whole function shape */
...
animate:
    {
        ...
        {
            register u8 *world_counters ASM_REG("$2") = (u8 *)&D_80083460;   /* MATCH pin: load-bearing for the whole function shape */
            ((S_80173CD4_3 *)world_counters)->unk_0A++;
        }
    }
...
state_two:
    if (((S_80173CD4_1 *)sprite)->unk_14 & 0xE000) {
        register u8 *world_counters ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
