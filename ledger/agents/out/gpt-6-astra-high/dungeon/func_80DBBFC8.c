...
selection_ready:
    ...
    ((S_801737C8_0 *)controller)->unk_98 &= 0xFF7F;
    {
        ...
        ASM_KEEP(target_player);   /* MATCH pin: retail basic-block layout depends on it */
        if (target_player != 0) {
            target = D_800814A8;
...

    {
        ...
        if (item_data[0x12] == 2) {
            ...
            if (target != 0) {
                register u8 *target_actor ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
