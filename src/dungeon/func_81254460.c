...
void func_81254460(void) {
    ...
    void *temp_a1;
    void *temp_v1;
    register u8 *table_page ASM_REG("$18");   /* MATCH pin: load-bearing for the whole function shape */
    u32 high_bit;
    u8 *flags_base;
    u8 *global_page;
    u8 *global_page2;
    ...
    high_bit = 0x80000000;
    ASM_KEEP_DEP_NV(table_page, high_bit);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    *(s8 *)((u8 *)temp_a1 + 0xBA) = 1;
    ...
    ASM_KEEP_NV(global_page);   /* MATCH pin: load-bearing for the whole function shape */
    *(s16 *)(flags_base + 0x371A) = 0;
    *(s16 *)(flags_base + 0x3718) = 0;
    *(s16 *)(flags_base + 0x3716) = 0;
    global_value = *(s32 *)(global_page + 0x3F90);
    ASM_KEEP_DEP_NV(state_flags, global_value);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    state_flags = (state_flags | 9) & 0xFFEF;
    *(volatile s32 *)(flags_base + 0x371C) = global_value;
    global_page2 = (u8 *)0x80170000;
    ASM_KEEP_NV(global_page2);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    *(s16 *)((u8 *)temp_a1 + 0xB8) = 0;
    ...
    ASM_SET(high_bit);   /* MATCH pin: retail schedule: same instructions, different order without it */
}
