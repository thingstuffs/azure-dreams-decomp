...
void *func_8017089C(s32 kind, s32 tile_x, s32 tile_y, s32 copy_value)
{
    ...
    register s32 saved_tile_y ASM_REG("$18");   /* UNRESOLVED C shape (pin): removing it changes the address form (%hi/%lo vs base+offset); the source shape that makes it unnecessary has not been found */
    ...
    void *allocated;
    register uptr scratch_word ASM_REG("$8");   /* UNRESOLVED C shape (pin): removing it rematerialises a constant retail keeps in a register; the source shape that makes it unnecessary has not been found */
    s32 allocation_size;
