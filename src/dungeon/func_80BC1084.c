...
/* Project linked points and queue their tile and draw-mode packets for rendering. */
s32 func_80170884(u8 *point_data, u16 *position)
{
    ...
    for (;;) {
        ...
        depth = func_80065420(&scratch->in0, (u8 *)packet + 8,
        ...
        {
            u8 *next_point = *(u8 **)(point_data - 8);

            if (next_point == 0)
                break;
            point_data = next_point + 0x20;
            position = (u16 *)*(u8 **)(next_point + 8);
        }
    }
