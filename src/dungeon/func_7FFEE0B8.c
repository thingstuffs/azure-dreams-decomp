...
void func_8008B818(s32 arg0, s32 *arg1, u8 *arg2, u8 *arg3) {
    s32 mode = arg0;
    register s32 *list ASM_REG("$22") = arg1;   /* MATCH pin: retail schedule: same instructions, different order without it */
    u8 *context = arg2;
    u8 *object = arg3;
    ...
    register u8 *s2 ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 list_value;
    u32 *table_base;
    register u32 table_word ASM_REG("$5");   /* MATCH pin: load-bearing for the whole function shape */
    u32 object_state;
    ...
    *(u16 *)(record + 0x66) = (u16)mode;
    ...
    if (table_word != 0) {
        *(u8 **)(record + 8) = &D_80037714;
        ...
        ASM_SET(table_word);   /* MATCH pin: retail register colouring depends on it */
        *(u8 **)(record + 8) = 0;
    }
    *(u16 *)(record + 0x60) = 0;
