...
void func_818C8FD0(void *state_arg, void *position_arg, void *part_arg)
{
    ...
    u8 *object;
    u8 *owner_node;
    register u8 *output_pos ASM_REG("$19");   /* UNRESOLVED C shape (pin): removing it changes the address form (%hi/%lo vs base+offset); the source shape that makes it unnecessary has not been found */
    u8 *owner_pos;
...
mode3:
    if (FIELD(state, s16, 0x88) == active_marker) {
        register u8 *effect_node ASM_REG("$6") = FIELD(state, u8 *, 0xA8);   /* UNRESOLVED C shape (pin): removing it changes the whole function shape; the source shape that makes it unnecessary has not been found */
        register u8 *effect_pos ASM_REG("$4") = FIELD(state, u8 *, 0xAC);   /* UNRESOLVED C shape (pin): removing it changes the register colouring; the source shape that makes it unnecessary has not been found */
