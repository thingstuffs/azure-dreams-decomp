...
void func_800BB2E4(s32 *arg0, s32 *arg1, u8 *arg2, s32 arg3, s32 arg4)
{
    CoordPair point;
    u8 *arg2_r = arg2;
    register s32 arg3_r ASM_REG("$22") = arg3;   /* MATCH pin: retail immediate-load split depends on it */
    register s32 arg4_r ASM_REG("$19") = arg4;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register u8 **state_addr ASM_REG("$21");   /* MATCH pin: retail immediate-load split depends on it */
    u8 *state;
    register u8 *base ASM_REG("$17");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *packet;
    u8 *primitive;
    u16 height;
    s32 shifted_height;
    s32 y;
    register s32 comparison ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 shifted_arg ASM_REG("$6");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 shifted;
    register s32 shift_test ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 shift_test2 ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register u8 *call_base ASM_REG("$4");   /* MATCH pin: keeps a constant in a register as retail does */
    s32 primitive_flag;
    ...
    shifted = shifted_arg;
    ASM_KEEP_NV(arg2_r);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ASM_KEEP_NV(arg3_r);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if ((arg3 << 16) == 0) {
        ...
        ASM_KEEP_NV(shift_test);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        call_base = base;
        if (shift_test != 0) {
        ...
        ASM_UNDEF(arg4_r);   /* MATCH pin: retail register colouring depends on it */
...
draw_packet:
        ...
        ASM_USE2(shift_test2, y);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        if (shift_test2 != 0) {
            y -= 0xE0;
        }
    ...
    ASM_USE(arg3_r);   /* MATCH pin: load-bearing for the whole function shape */
}
