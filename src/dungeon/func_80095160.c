...
s32 func_8009A8C0(u32 arg0, FuncArg1 *arg1, FuncArg2 * volatile arg2, u16 arg3) {
    ...
    u16 sp18;
    s16 temp_v0_2;
    register s32 temp_v0_3 ASM_REG("$18");   /* MATCH pin: retail register colouring depends on it */
    s32 var_a3;
    u32 temp_idx;
    u8 *base_6ccd8;
    s32 temp_a0;
    ...
    s32 temp_s3;
    register s32 temp_v0_load ASM_REG("$2");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 var_v0;
    register s32 var_v1 ASM_REG("$3");   /* MATCH pin: keeps a constant in a register as retail does */
    s32 first_mask;
    s16 *limits;
    u16 *temp_fp;
    u16 *temp_s7;
    ...
    u32 temp_s0_2;
    u32 temp_s1_2;
    register u32 temp_s4 ASM_REG("$20");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 call_a0 ASM_REG("$4");   /* MATCH pin: retail keeps a computation the compiler would drop */
    register s32 call_a1 ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s32 call_a2;
    u32 call_a3;
    register u16 saved_arg3 ASM_REG("$7");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    u32 temp_t0;

    saved_arg3 = arg3;
    ASM_KEEP_NV(saved_arg3);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    temp_idx = (arg0 >> 9) & 7;
    temp_a0 = temp_idx;
    ASM_KEEP_NV(temp_a0);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        ...
        if (((1 << limits[10]) - 1) >= first_mask) {
            ...
            if (var_v1 != 0) {
                if (((1 << limits[11]) - 1) >= var_v1) {
                    ...
                    ASM_KEEP_NV(var_v1);   /* MATCH pin: retail schedule: same instructions, different order without it */
                    ...
                    temp_t0 = (u32)arg2;
                    ASM_KEEP_NV(temp_t0);   /* MATCH pin: retail basic-block layout depends on it */
                    ...
                    if (var_v0 != 0) {
                        ...
                        ASM_SET(temp_v0_load);   /* MATCH pin: keeps a statement from moving across a call/branch */
                    ...
                    if ((arg1->flag >= 0) || (temp_v0_2 = func_8009FB34((arg1->x + *temp_fp) & 0xFFFF, (arg1->y + *temp_s7) & 0xFFFF), (temp_v0_2 < 0)) || !(D_800E2970[temp_v0_2].flags & 2) || (var_v0 = 0, ((temp_t0 = (u32)arg2, ((FuncArg2 *)temp_t0)->flags & 0x2000) != 0))) {
                        if (sp10.value & 0x3300) {
                            ...
                            if (sp10.value & 0x40) {
                                {
                                    s32 late_a0;
                                    register u32 late_t0 ASM_REG("$8");   /* MATCH pin: load-bearing for the whole function shape */
                                    late_a0 = temp_s1_2;
                                    ASM_KEEP_NV(late_a0);   /* MATCH pin: retail schedule: same instructions, different order without it */
                                    temp_s0_2 = temp_s0 & 0xFFFF;
                                    ASM_USE(temp_s0_2);   /* MATCH pin: retail schedule: same instructions, different order without it */
                                    late_t0 = sp18;
                                ...
                                ASM_KEEP_NV(temp_v0_3);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
