...
void func_80171728(void *actor_in, void *context_in, void *sprite_in, void *creature_in)
{
    ...
    void *context;
    void *sprite;
    register void *creature ASM_REG("$18");   /* UNRESOLVED C shape (pin): removing it changes the address form (%hi/%lo vs base+offset); the source shape that makes it unnecessary has not been found */
    u8 *anim_table;
    s32 distance;
    s8 room_id;
    u16 action_flags;
    ...
    ASM_KEEP(creature);   /* UNRESOLVED C shape (pin): removing it changes the instruction count (a copy retail keeps is dropped or added); the source shape that makes it unnecessary has not been found */
