        ...
        if (*(s32 *)((s8 *)arg0 + 0x1C) & 0x2000) {
            if (!(*(s32 *)((s8 *)arg1 + 0x1C) & 0x2000)) {
                ...
                }
                {
                    ...
                    register void *p ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
                    register s32 a1 ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
                    register s32 s0 ASM_REG("$16");   /* MATCH pin: retail register colouring depends on it */
                    p = &D_800E1987;
                    ASM_KEEP_NV(p);   /* MATCH pin: retail schedule: same instructions, different order without it */
                    a1 = ret;
                    s0 = a1;
