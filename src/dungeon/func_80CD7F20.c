...
void func_8015B720(void *owner_arg, void *context, void *anim_obj_arg, void *state_arg) {
    ...
    register void *state ASM_REG("$18") = state_arg;   /* UNRESOLVED C shape (pin): removing it changes the whole function shape; the source shape that makes it unnecessary has not been found */
    void *current_table;
