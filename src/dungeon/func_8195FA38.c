...
s32 func_8195FA38(void *tile_data, s32 draw_arg_1, s32 draw_arg_2) {
    s32 column;
    register s32 row ASM_REG("$18");   /* UNRESOLVED C shape (pin): removing it changes the address form (%hi/%lo vs base+offset); the source shape that makes it unnecessary has not been found */
    s32 tile_y;
    s32 row_arg;
    s32 tile_size;
    s32 edge_size;
    register s32 last_tile ASM_REG("$23");   /* UNRESOLVED C shape (pin): removing it changes the whole function shape; the source shape that makes it unnecessary has not been found */
