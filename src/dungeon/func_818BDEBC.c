...
void func_800256BC(EffectState *arg0, Motion *arg1, register ColorPart *part) {
    ...
    register Motion *motion ASM_REG("$22");   /* MATCH pin: keeps a constant in a register as retail does */
    void *owner;
    u8 *color_part = (u8 *)part;
    register void *meta ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
    void *node;
    void *source;
    void *target;
    ...
    s32 value;
    u32 color;
    register s32 first_delta ASM_REG("$5");   /* MATCH pin: load-bearing for the whole function shape */
    s16 z;
    u16 ground;
    register s32 tile_x ASM_REG("$20");   /* MATCH pin: retail register colouring depends on it */
    s32 tile_y;
    u32 dead_page;
    ...
    u16 *update_y_entry;
    s32 table_offset;
    register s32 update_offset ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 loop_owner_z ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 next_x ASM_REG("$4");   /* MATCH pin: retail keeps a computation the compiler would drop */
    s32 next_y;
    register s16 *table ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    s32 out_idx;
    s32 offset_x;
    s32 offset_y;
    register s32 signed_x ASM_REG("$5");   /* MATCH pin: load-bearing for the whole function shape */
    s32 work_x;
    s32 work_y;
    u16 saved_y;
...
case_0:
    ...
    ASM_KEEP(color_part);   /* MATCH pin: retail delay-slot fill depends on it */
    ...
    if (PTR_AT(owner, 0x60) != 0) {
        ...
        register s32 y_rhs ASM_REG("$3") = S16_AT(motion, 6);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    do {
        ...
        tile_y = next_y;
        saved_y = next_y;
        ASM_KEEP4_NV(next_x, next_y, tile_x, tile_y);   /* MATCH pin: load-bearing for the whole function shape */
        work_x = next_x;
    } while (i < 8);

    target_pos = (Position16 *)&work.destination;
    ASM_KEEP_NV(target_pos);   /* MATCH pin: load-bearing for the whole function shape */
    i = 1;
    ...
    ASM_KEEP(signed_x);   /* MATCH pin: retail schedule: same instructions, different order without it */
    temp_t0 = saved_y;
    out_idx = (s16)state->direction;
    ASM_KEEP_DEP_NV(temp_t0, out_idx);   /* MATCH pin: retail register colouring depends on it */
