    ...
    if (target == (void *)D_800E3D7C[0]) {
        ...
        }
        {
            ...
            }
            {
                ...
                s32 *item_dest;
#ifdef __mips__
                register s32 item_slot_addr ASM_REG("$2");   /* UNRESOLVED C shape (pin): removing it changes a delay-slot fill; the source shape that makes it unnecessary has not been found */
#else
                s32 item_slot_addr;
#endif
...
#ifdef __mips__
        s32 *item_dest;
        register s32 *item_slot ASM_REG("$2");   /* UNRESOLVED C shape (pin): removing it changes a delay-slot fill; the source shape that makes it unnecessary has not been found */
#else
        s32 *item_dest;
        s32 *item_slot;
#endif
