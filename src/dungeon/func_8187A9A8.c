...
void func_8187A9A8(void *mesh_data, void *transform, void *object_data, s32 depth_bias)
{
    ...
    if (FIELD(mesh, u8, 0x4E) != 0) {
        ...
        s32 texture_adjust;
        u8 *render_state;
        register u8 **render_state_ref ASM_REG("$10");   /* UNRESOLVED C shape (pin): removing it changes the register colouring; the source shape that makes it unnecessary has not been found */
    ...
    if (FIELD(mesh, u8, 0x4E) == 0) {
        ...
        u32 color_code;
        s32 packet_code;
        register u8 *render_state ASM_REG("$3");   /* UNRESOLVED C shape (pin): removing it changes the whole function shape; the source shape that makes it unnecessary has not been found */
        register u8 **render_state_ref ASM_REG("$10");   /* UNRESOLVED C shape (pin): removing it changes the register colouring; the source shape that makes it unnecessary has not been found */
