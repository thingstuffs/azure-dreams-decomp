...
{
    register Controller *p ASM_REG("$19") = ctrl;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register Motion *m ASM_REG("$22") = motion;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *root;
    register void *third ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
    RootPrefix *prefix;
    Lookup *lookup;
    Motion *initial;
    register Motion *other ASM_REG("$16");   /* MATCH pin: retail register colouring depends on it */
    Child *child;
    u8 *link;
    s16 delta[3];
    u16 last_y;
...
advance:
    ...
    p->state++;
    {
        register Child **walk ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
        u8 *global_page;
        ...
        do {
            ...
            if (child != 0) {
                u16 flags;
                register u32 used ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
                ...
                flags |= 0x8000;
                used |= 0x8000;
                ASM_KEEP(used);   /* MATCH pin: retail schedule: same instructions, different order without it */
                child->flags = flags;
