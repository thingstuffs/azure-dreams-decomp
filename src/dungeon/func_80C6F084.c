...
/* Adds projected point primitives and draw-mode packets to the ordering table. */
s32 func_8016A884(u8 *object_data, u16 *point_data)
{
    Scratch *scratch = (Scratch *)0x1F800000;
    void **global_state = D_80083160;
    ...
    for (;;) {
        scratch->in0 = *(volatile u16 *)&point_data[1];
        packet = (Packet *)*(u8 * volatile *)&scratch->current;
        scratch->in1 = point_data[3];
        scratch->in2 = point_data[5];
        ...
        if ((u32)depth_index < 480U) {
            ...
            color_word = *(u32 *)(object_data + 8);
            packet->data.link = color_word;
            packet_code = 2;
...

        {
            ...
            point_data = (u16 *)*(u8 **)(next_object + 8);
        }
    }
