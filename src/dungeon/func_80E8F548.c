...
void func_80174D48(void *arg0, S_80174D48_4 *arg1, void *arg2)
{
    ...
    if (((*(u8 *)((u8 *)arg2 + 0x49)) != 0) &&
        ...
        ASM_KEEP(first_call_a0);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_KEEP(first_call_page);   /* MATCH pin: retail immediate-load split depends on it */
        ...
        ASM_SCHED_BARRIER();   /* MATCH pin: retail schedule: same instructions, different order without it */
        temp_half = 0xC90;
        ASM_KEEP(temp_half);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        temp_s1->unk_08 =
            func_8004A658((*(volatile u8 *)((u8 *)arg2 + 0x49)),
                          (*(u8 *)((u8 *)arg2 + 0x48)));
        ASM_KEEP(temp_s2);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        temp_byte = arg1->unk_24;
        ASM_KEEP(temp_byte);   /* MATCH pin: retail basic-block layout depends on it */
        call_a0 = temp_v0;
        ASM_KEEP(call_a0);   /* MATCH pin: load-bearing for the whole function shape */
        ((S_80174D48_1 *)temp_s2)->unk_A8 = temp_byte;
        temp_byte = arg1->unk_25;
        ASM_KEEP(temp_byte);   /* MATCH pin: retail basic-block layout depends on it */
        call_zero = 0;
        ASM_KEEP(call_zero);   /* MATCH pin: load-bearing for the whole function shape */
        ((S_80174D48_1 *)temp_s2)->unk_AA = temp_byte;
        ASM_KEEP(temp_s3);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
