...
void func_8016CB0C(void *state, s32 unused, s32 checkInput, s32 setupInput) {
    register s32 savedCheckInput ASM_REG("$18") ;   /* UNRESOLVED C shape (pin): removing it moves a statement across a call/branch; the source shape that makes it unnecessary has not been found */

    if (func_800AB1C0() != 0) {
        savedCheckInput = checkInput;
