...
/* Queues projected point primitives and draw modes for a linked list. */
s32 func_80158884(u8 *item, u16 *coords)
{
    ...
    for (;;) {
        ...
        if ((u32)depth_index < 480U) {
            ...
            register u32 header_byte ASM_REG("$2");   /* UNRESOLVED C shape (pin): removing it changes the register colouring; the source shape that makes it unnecessary has not been found */

            packed_color = *(u32 *)(item + 8);
            packet->data.link = packed_color;
            header_byte = 2;
            packet->code = header_byte;
            header_byte = 106;
            ...
            packet->data.color.command = header_byte;
        ...
        {
            u8 *next_item = *(u8 **)(item - 8);

            if (next_item == 0)
                break;
            item = next_item + 0x20;
            coords = (u16 *)*(u8 **)(next_item + 8);
        }
    }
