...
void func_80024818(void *arg0)
{
    ...
    if (small != 0) {
        ...
        limit = 3;
        ASM_TAILSLOT_PIN_TIED(limit);   /* UNRESOLVED C shape (pin): removing it changes a delay-slot's contents; the source shape that makes it unnecessary has not been found */
