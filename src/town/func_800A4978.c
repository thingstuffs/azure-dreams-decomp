...
loop_1:
    ...
    var_s0 += 1;
    ASM_TAILSLOT_PIN(var_s0);   /* UNRESOLVED C shape (pin): removing it changes a delay-slot's contents; the source shape that makes it unnecessary has not been found */
    func_800A2154();
call_succeeded:
    if (var_s0 != 0) {
        var_s0 -= 1;
