...
void func_800B52F4(s32 arg0, S_800B52F4_1 *arg1) {
    ...
    register s32 word ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    register s32 byte ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    register s32 fifth ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    StackData *call_data;
