...
| **Script symbol dump** | `ledger/evidence/script_symbols.json`, `docs/evidence/script_symbols.tsv`, `include/script_symbols.h`; dispatch in `ledger/evidence/fno_table.json`; names in `ledger/evidence/names_proposed.tsv` | The scene tool's symbol dump shipped inside the TOWN.BIN devkit blob: 3,587 named constants (36-byte records, value + name): `FNO_` script function numbers, `F_`/`Ft_`/`Fr_`/`Ftt_`/`GOODS_` event flags, `SSTP_` scene steps, `S_` script system calls, `IMG_` portraits, `PSN_DM_*` person commands, `V_` variables, `sn_` scenes ... The `FNO_` numbers index the dispatch table at `0x800D3D0C` (TOWN.BIN `0x565AC`, entry = number − 100; 111/112 named numbers land on function starts), so the text after `FNO_` is the developer's name of that C function: 52 rows named (47 town, 5 resident). Identical on the Japanese disc. `docs/SYMBOLS.md` §5. | **hard fact** for the numbers and the function names behind the dispatch table; the flag/step/step constants still need the accessor routines identified before a literal can be replaced |
