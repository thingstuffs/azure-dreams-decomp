...
| **Script symbol dump** | `ledger/evidence/script_symbols.json`, `docs/evidence/script_symbols.tsv`, `include/script_symbols.h`; dispatch in `ledger/evidence/fno_table.json`; names in `ledger/evidence/names_proposed.tsv` | The scene tool's symbol dump shipped inside the TOWN.BIN devkit blob: 3,587 named constants (36-byte records, value + name): `FNO_` script function numbers, `F_`/`Ft_`/`Fr_`/`Ftt_`/`GOODS_` event flags, `SSTP_` scene steps, `S_` script system calls, `IMG_` portraits, `PSN_DM_*` person commands, `V_` variables, `sn_` scenes ... The `S_` and `FNO_` numbers are one numbering into the script **call table** at `0x800D3CC8` (TOWN.BIN `0x56568`, entry = number + 1; published through `0x80016000` → `D_801131B8` and installed into the bytecode VM object `D_80082A38+0x40`; opcodes 46/37/47), so the text after `S_` / `FNO_` is the developer's name of that C function: 125 numbers resolve to functions the tree owns (112 town rows, 13 resident). Rule proven by the 2026-09-08 research workflow (`docs/evidence/script_call_table_20260908.md`), which also corrected an earlier off-by-17 reading. Identical on the Japanese disc. `docs/SYMBOLS.md` §5. | **hard fact** for the numbers and the function names behind the dispatch table; the flag/step/step constants still need the accessor routines identified before a literal can be replaced |
...
identifier, 52 rows are named by the script dispatch table, 52 randomizer code sites resolve to a
function (24 DUNGEON rows) and 22 rows reference a randomizer-named data table, 72 SLUS rows are
pointer-table targets, 408 SLUS rows have a prior note: 613 rows in all (`STATUS.md` keeps the live table). 34 assertion copies sit in functions the tree does
...
- **L4 names (`config/names.tsv`, `include/records/`)**: start from the evidence rows. Order of
  trust: assertion text, developer identifiers and script-dump function names (verbatim;
  `ledger/evidence/names_proposed.tsv` is ready to apply through the alias mechanism), randomizer
  table names (verbatim for data symbols: `D_8006D168` → `initialStatsTable`), pointer-table roles
  (opcode numbering), prior notes (verify), call-site roles. A name with no evidence line is not
  applied. Script constants (`include/script_symbols.h`) replace a literal only once the accessor
  routine that consumes it is identified.
...
```sh
python3 tools/evidence.py import --town-bin work/disc/containers/TOWN_TOWN.BIN   # script symbol dump + FNO dispatch (after tools/setup.sh)
python3 tools/evidence.py build          # from docs/evidence/*.csv, ledger/evidence/*.json, knowledge/, baserom, config/names.tsv
