...
### Compiler probe: cdk always splits symbol addresses

Eleven tiny functions (one load, two loads, the same symbol three times, a store, load-then-store, address taken, array element, array constant index, two struct fields, a load in a loop, a store in a loop) compiled at `2.7.2-cdk -G0` and at FSF `2.7.2 -G0`: cdk emits an explicit `lui` into a real register with `%hi`/`%lo` in every case, FSF emits the assembler macro form (`lh $2,D_1`, `la $4,D_1`, `lw $2,A_1($4)`) in every case, which the assembler expands through `$at`. No C shape moves between the two, and cdk rejects `-mno-split-addresses` and `-mno-gas`. So a row whose retail bytes carry the `$at` expansion did not come from cdk, whatever its C; the 53 pin-free nonconforming rows exact only at FSF 2.6.3/2.7.2 inside cdk-G0 modules are therefore most likely separate translation units that the module grouping merged (the census's contiguity check reports them), not rows to reshape. Two calibration facts for the listing screen: byte-exact recipe pairs differ by two listing lines in 62 of 150 calibration rows (delay-slot spelling), so the perturbation search now rescores candidates within that band with the byte scorer.
