...
| L3 | locals named, one-line summary comment, control flow simplified where match allows; the accepted body **landed in `src/`** (`ledger/promotions.jsonl`, verified; there is no second tree); **every remaining pin, marker and tail-call spelling was attempted for removal** — by the lane that refined the row for rows above 100 B (journalled counts before/after), and for the ≤ 100 B band by the T2 sweep plus the tracked shape queue `work/pin_queue_tiny.txt` (the 2026-09-08 pilot showed the per-row lane adds nothing there: 0 removals in 40 rows at 5× the cost) |
