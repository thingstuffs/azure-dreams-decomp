...
| L3 | locals named, one-line summary comment, control flow simplified where match allows; the accepted body **landed in `src/`** (`ledger/promotions.jsonl`, verified; there is no second tree); **every remaining pin, marker and tail-call spelling was attempted for removal** — by the lane that refined the row for rows above 100 B (journalled counts before/after), and for the ≤ 100 B band by the T2 sweep plus the tracked shape queue `work/pin_queue_tiny.txt` (the 2026-09-08 pilot showed the per-row lane adds nothing there: 0 removals in 40 rows at 5× the cost); **and** every tail-jump dependency (a noreturn pseudo-call, or a call to a `config/sibcall_syms*.txt`/`noreturn_syms*.txt` symbol) has a `ledger/split_audit.jsonl` record that is not `intra` or `unresolved` (owner ruling 2026-09-22 evening: a retail `j` to a "function" symbol may mean a mis-split, and that deserves a check before L3) |
| L4 | the L3 criteria, plus: in a module with a shared header; struct declared once; globals typed; names and module membership agree with the row's evidence (`docs/EVIDENCE.md`: a row carrying `main.c:40` is in `main.c`); **zero pins** (not merely swept) and **zero tail-jump dependency of any kind**, audited or not — the sibcall/noreturn tail-jump scaffolding is a residue class of its own, covered here (owner ruling: "needs to be covered by a level, somewhere") |
| L5 | **strict, nothing left** beyond L4's own gate: no fidelity site of any audit class (blocking or byte-derived), no computed-goto table, no inline asm, no `NON_MATCHING` guard. A row that cannot get there stays at L4 with each residue documented with the measured reason (`ledger/levels.jsonl` `l5_residue` names what is left of L5's list, `l4_residue` names which of pins/tail_jump/not_in_module blocks L4) |
