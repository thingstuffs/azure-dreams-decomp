...
import probe as P
local=HERE/'c8_accessors';local.mkdir(exist_ok=True)
if not (local/'include').exists():(local/'include').symlink_to(HERE/'include')
P.HERE=local
stem='w_8003F5D4'
P.RECIPES['src/'+stem+'.c']={'src':'src/'+stem+'.c','out':'build/src/'+stem+'.o','ccver':'2.7.2-cdk','ccflags':'-G32','asflags':''}
...
(HERE/'c8_accessors_receipt.json').write_text(json.dumps(report,indent=2)+'\n')
