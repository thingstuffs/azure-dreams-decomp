#!/usr/bin/env python3
"""Exact sink-retirement candidate unit and isolated full overlay-window gate."""
...
ROOT = HERE.parents[4]
CANDIDATE = ROOT / "work/native_lane/gp_partition_only_tools/retire_sink/candidate"
GUARDED_BEFORE = ROOT / "work/native_lane/selfinc_local_guard/candidate"
CLI = CANDIDATE / "maspsx.py"
PACKAGE = CANDIDATE / "maspsx/__init__.py"
EXP = "retire_sink_20260924_sol"
...
EXPECTED = {
    ...
    "package": "b89099797be1352d3122455164062d428dcc74529344a919d169e05f663afc52",
}
...
def main() -> int:
    ...
    try:
        ...
        receipt["unit_exit_code"] = run_to_log(
            ...
            HERE / "unit.log", env)
        before_env = dict(env, PYTHONPATH=str(GUARDED_BEFORE))
        receipt["guarded_before_unit_exit_code"] = run_to_log(
            [str(PYTHON), "-m", "unittest", "discover", "-s", "tools/maspsx/tests", "-t", "."],
            HERE / "unit_before.log", before_env)
        receipt["candidate_failed_tests"] = failed_tests(HERE / "unit.log")
        receipt["guarded_before_failed_tests"] = failed_tests(HERE / "unit_before.log")
        receipt["unit_failure_sets_equal"] = (
            receipt["candidate_failed_tests"] == receipt["guarded_before_failed_tests"])
