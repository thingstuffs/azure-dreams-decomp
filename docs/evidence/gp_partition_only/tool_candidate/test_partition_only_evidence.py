...
REPO = next(p for p in HERE.parents if (p / "tools/common.py").is_file())
sys.path[:0] = [str(CANDIDATE / "tools"), str(CANDIDATE / "tools/build"),
                str(REPO / "tools"), str(REPO / "tools/build")]
...
def load(name, path):
    ...
    assert Path(module.__file__).resolve().is_relative_to(CANDIDATE)
    return module


M = load("slus_modules", CANDIDATE / "tools/build/slus_modules.py")
P = load("slus_partitions", CANDIDATE / "tools/build/slus_partitions.py")
C = load("slus_module_context", CANDIDATE / "tools/slus_module_context.py")
E = load("slus_module_evidence", CANDIDATE / "tools/slus_module_evidence.py")
O = load("private_partition_ownership", CANDIDATE / "tools/fidelity/prove_slus_ownership.py")
CERT = load("private_partition_certifier", CANDIDATE / "tools/fidelity/certify_slus_module.py")
ISO = load("private_partition_iso", CANDIDATE / "tools/fidelity/slus_iso.py")
DB = load("private_partition_row_db", CANDIDATE / "tools/row_db.py")
