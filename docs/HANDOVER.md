...
tokens (round 58, same note: thirteen mechanism-briefed Gemini packs add 51 pins (46 of 156 rows against 1 of 12 unbriefed, the yield falling with the residue size), and an Opus pass over the corpus of 128 lane reports ([the audit](PIN_LANE_RULES_AUDIT_20260919.md)) paid the same day: t74_multiset 48 pins, a lane-harvest audit 5 rows, an arity repair from an Opus variety lane (t76), the symbol-class analysis (t77); **5,682 / 1,175 rows** after the Gemini queue drained (packs 14-16) and codex's return ([round 59](evidence/pin_research_round59_20260919.json): eleven class-grouped codex packs briefed with solved exemplars, 23 of 135 sol rows on 1-3-pin mid/large/far residues (large 13/60, mid 8/45, far 2/30, the per-slice yield falling), luna 0 of 28 on multi-pin rows; chain F keeps queuing 15-row sol slices while capacity lasts), by reading gcc 2.7.2's `sched.c`: the old list scheduler fills blocks in reverse and
...
**Active zero-pin research objective (2026-09-17): 6,012 pins in 1,269 rows,
91 gated removals from the new 6,103-pin baseline.** The owner superseded the old
