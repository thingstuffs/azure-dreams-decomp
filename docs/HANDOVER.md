...
tokens (round 58, same note: thirteen mechanism-briefed Gemini packs add 51 pins (46 of 156 rows against 1 of 12 unbriefed, the yield falling with the residue size), and an Opus pass over the corpus of 128 lane reports ([the audit](PIN_LANE_RULES_AUDIT_20260919.md)) paid the same day: t74_multiset 48 pins, a lane-harvest audit 5 rows, an arity repair from an Opus variety lane (t76), the symbol-class analysis (t77); **5,699 / 1,192 rows** after the Gemini queue drained (packs 14-16) and codex's return (round 59: class-grouped packs with solved exemplars; sol pays on 1-3-pin mid/large residues, luna nothing on multi-pin rows); more sol packs queued), by reading gcc 2.7.2's `sched.c`: the old list scheduler fills blocks in reverse and
