...
Rows probed: 1794.  Every ASM_* site of the row erased at once; the scorer's aligned distance and the shape of the residue below.
...
## The near band: 490 rows within 12 words of pin-free (173,188 B, 1025 pins)
...
|---|---:|---:|---:|---|
| broad | 109 | 223 | 34,836 | no single named signal; read the row's regions view |
| length-drift | 74 | 181 | 23,960 | retail keeps or drops a word gcc does not |
| reg-rename | 56 | 83 | 26,132 | same instructions, two registers swapped: the variable map, not the register |
| reorder-only | 43 | 89 | 16,276 | same instructions, one moved by the scheduler |
| code-motion | 32 | 72 | 9,436 | a statement crossed a call or a branch |
...
| hold-set | 11 | 18 | 1,852 | a different callee-saved set / frame layout |
