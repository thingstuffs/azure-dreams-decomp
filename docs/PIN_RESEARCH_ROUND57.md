...
### Round 59 results (2026-09-19, receipt [pin_research_round59_20260919.json](evidence/pin_research_round59_20260919.json))

Codex returned at 06:05Z. The residues pack (eight Gemini residue rows with mechanism notes) landed one row of two pins. The luna class packs paid nothing: 0 of 13 on the small CHANGED classes at 3-5 pins, 0 of 15 on MOVED at 3-8 pins, exemplars or not; luna does not pay on multi-pin rows. Sol on 1-3-pin rows, fifteen rows a slice, briefed with six solved diffs of the slice's own classes: large residues (17-32) 13 of 60 over four slices (4, 3, 5, 1), mid (9-16) 8 of 45 over three (2, 3, 3), far (33+) 2 of 30 over two. Every landing went through the locked pipeline (cascade, T2, covering windows + SLUS MATCH) and its port-arm-restored twin (no twin produced a candidate this round). **5,709 / 1,197 -> 5,682 / 1,175**, 27 pins, committed through the ten-minute snapshot committer while the session was away (9b114667 .. 13d0bf7c).

What it says: the exemplar brief moves sol from 0 of 12 on never-served rows (round 56) to about one row in six on single-pin mid/large residues, and the yield per slice falls as the pool's easy rows are served (large 4, 3, 5 then 1). The far band stays at one in fifteen. The 23 sol-solved rows were appended to the exemplar index (144 rows) before the next slices were built.

Queued (2026-09-20 04:00Z): chain F (scratchpad `codex59f.sh`, sentinel `CODEX59F_END`) builds and launches sol slices itself, large then mid alternating, and stops on an empty pool or six failed hourly capacity probes; the large pool held 74 unserved 1-3-pin rows at its first build. After the pools drain the remaining sol leads are the 4+-pin rows of the same classes (untested with sol; Gemini and luna both 0 there) and the never-run compositions named by the lane-rule audit.
