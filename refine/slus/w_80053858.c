...
s32 func_80053858(S_80053858_rec *first_text)
{
  ...
  S_80053858_rec *text;
  int texture_page;
  register S_80053858_rec *next_node ASM_REG("$4");   /* MATCH pin: slus-diff */
  int link_mask = 0xFFFFFF;
  do
  {
    s32 char_index = 0;
    register s32 cursor_x ASM_REG("$20");   /* MATCH pin: slus-diff */
    text = text_record;
    cursor_x = text->unk14;
    if (text->unk4[0] != 0)
    {
  ...
  while (next_node != 0);
  ASM_KEEP(next_node);   /* MATCH pin: slus-diff */
 do { return 0; } while (0);
}
