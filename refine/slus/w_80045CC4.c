...
void func_80045CC4(void *context, s32 position, S_80045CC4_Arg2 *sprite, s16 depth_bias)
{
    ...
    void *global_base;
    u8 uv_edge;
    register u8 gpu_code ASM_REG("$4");   /* MATCH pin: slus-diff */
...
next_entry:
    if (!(entry[0] & 0x20)) {
        ...
        if ((entry[0] ^ SP_U16(0x24)) & 1) {
            ...
            ASM_KEEP_DEP_NV(offset_byte, pivot);   /* MATCH pin: slus-diff */
            ASM_KEEP_DEP_NV(offset_byte, size);   /* MATCH pin: slus-diff */
        ...
        ASM_KEEP(scratchpad);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        depth = RotAverage4(
        ...
        if ((u32)depth < 0x1E0U) {
            ...
            if ((u32)((((S_80045CC4_0 *)quad)->unk_14 + 0x20) & 0xFFFF) < 0x181U) {
                {
                    ...
                    ASM_USE_NV(visible_2);   /* MATCH pin: slus-diff */
            ...
            if ((u32)((((S_80045CC4_0 *)quad)->unk_1C.u + 0x20) & 0xFFFF) < 0x181U) {
                {
                    ...
                    ASM_USE_NV(visible_3);   /* MATCH pin: slus-diff */
            ...
            if (any_visible | visible_3) {
                ...
                *(volatile u16 *)&sprite->unk14 &= 0x7FFF;
                ...
                {
                    register s32 uv_end ASM_REG("$2");   /* MATCH pin: slus-diff */
                    s32 uv_start;
                    s32 v_start;
                    ...
                    uv_end += uv_start;
                    uv_start = v_start;
                    ASM_KEEP_NV(uv_start);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                    SP_S32(0x10) = uv_end;
                    uv_end = SP_S32(0x14);
...

                {
                    ...
                    ASM_KEEP_NV(matrix_xx);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                ...
                }
                {
                    ...
                    ASM_KEEP_NV(matrix_yy);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                ...
                flags = SP_U16(0x24);
                if (flags & 8) {
                    register u32 blend_code ASM_REG("$2");   /* MATCH pin: slus-diff */
                    if (flags & 4) {
