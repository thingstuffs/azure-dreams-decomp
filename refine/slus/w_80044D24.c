...
void func_80044D24(void *unused, void *sprite_data, s32 ot_depth)
{
    ...
    u8 *context;
    u8 *parts;
    register void *matrix ASM_REG("$4");   /* MATCH pin: slus-diff */
    register void *translation ASM_REG("$5");   /* MATCH pin: slus-diff */
    register void *part ASM_REG("$18");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register void *packet ASM_REG("$17");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 depth;
    s16 corner_x;
    s16 corner_y;
    u16 flags;
    ...
    ASM_USE(part);   /* MATCH pin: slus-diff */
    packet = (u8 *)prim + 4;
        ...
        if (!(parts[0] & 0x20)) {
            ...
            if ((parts[0] ^ U16_AT(scratch, 0x24)) & 1) {
                ...
                if (U16_AT(sprite, 0x14) & 0x400) {
                    ...
                    s32 offset_byte = U8_AT(part, 1);
                    ASM_KEEP_NV(offset_byte);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                    offset = (offset_byte << 24) >> 24;
                }
                {
                    register s32 flipped_offset ASM_REG("$3") = -offset;   /* MATCH pin: slus-diff */
                    ASM_KEEP_NV(flipped_offset);   /* MATCH pin: slus-diff */
                ...
                u16 double_size;
                s32 offset_byte;
                register s32 far_edge ASM_REG("$2");   /* MATCH pin: slus-diff */
                s32 offset;
                offset_byte = U8_AT(part, 1);
                offset_byte <<= 24;
                double_size = U16_AT(sprite, 0x14) & 0x400;
                ASM_KEEP_DEP_NV(offset_byte, double_size);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            ...
            U16_AT(sprite, 0x14) &= 0x7FFF;
            {
                ...
                register s32 u_start ASM_REG("$4") = U32_AT(scratch, 8);   /* MATCH pin: slus-diff */
                register s32 v_end ASM_REG("$2") = U32_AT(scratch, 0x14);   /* MATCH pin: slus-diff */
                ...
                {
                    v_end = v_start;
                    ASM_KEEP_NV(v_end);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            ...
            {
                u8 prim_code;
                register u16 sprite_flags ASM_REG("$3");   /* MATCH pin: slus-diff */
                ...
                if (sprite_flags & 8) {
                    ...
                    ASM_KEEP_DEP_NV(blend_code, sprite_flags);   /* MATCH pin: slus-diff */
