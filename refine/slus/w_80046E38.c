...
void func_80046E38(s16 entry_id, u8 *stream)
{
    ...
    s32 data_offset;
    s32 remaining;
    register u8 *table_base ASM_REG("$2");   /* MATCH pin: slus-diff */
    s32 table_offset;
    s32 shifted_id;
    ...
    stream_pos = stream;
    ASM_KEEP_NV(stream_pos);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    stream_start = stream;
    ...
    ASM_KEEP_NV(shifted_id);   /* MATCH pin: slus-diff */
