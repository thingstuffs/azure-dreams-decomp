...
S_8004A330_Entity *func_8004A330(s32 style_flags, s32 start_x, s32 start_y, s32 target_x,
    ...
    register S_8004A330_Sub20 *state ASM_REG("$17");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 buffer_addr;
    s32 saved_style_flags = style_flags;
    register s32 saved_start_x ASM_REG("$22") = start_x;   /* MATCH pin: slus-diff */
    register s32 saved_start_y ASM_REG("$20") = start_y;   /* MATCH pin: slus-diff */
    register s32 saved_target_x ASM_REG("$18") = target_x;   /* MATCH pin: slus-diff */
    register s32 saved_target_y ASM_REG("$19") = target_y;   /* MATCH pin: slus-diff */
    register s32 saved_width ASM_REG("$21") = width;   /* MATCH pin: slus-diff */
    u16 entity_flags;
    s32 global_flags;
#ifndef NON_MATCHING
    s32 *global_page;
    ...
    if (entity != 0) {
        ...
        if (buffer_addr != 0) {
            ...
            func_8004A24C(state, saved_style_flags, content_id, (s16)saved_start_x,
                          (s16)saved_start_y,
                          (s16)saved_width);
            ASM_KEEP(saved_start_x);   /* MATCH pin: slus-diff */
            ASM_KEEP(saved_start_y);   /* MATCH pin: slus-diff */
            ASM_KEEP(saved_target_x);   /* MATCH pin: slus-diff */
            ASM_KEEP(saved_target_y);   /* MATCH pin: slus-diff */
            ASM_KEEP(saved_width);   /* MATCH pin: slus-diff */
