...
void func_80047054(void *data, s32 flagged_x_offset, s32 y_offset, s32 x_offset)
{
    void *header = data;
    s32 skip_bytes;
    register u32 entry_addr ASM_REG("$3");   /* MATCH pin: slus-diff */
    u8 *entry;
    u8 *part;
    register u8 *coords ASM_REG("$8");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 part_entry_type ASM_REG("$11");   /* MATCH pin: slus-diff */
    u32 entries_end;
    u16 x_or_end_flag;
    u8 flags;
    ...
    ASM_KEEP(entry_addr);   /* MATCH pin: slus-diff */
