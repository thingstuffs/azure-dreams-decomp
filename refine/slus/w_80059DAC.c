...
{
    s32 saved_entry[2];
    register s32 entry_count ASM_REG("$8");   /* MATCH pin: slus-diff */
    s32 entry_index;
    s32 compare_index;
    void *scan_entry;
    void *entry;
    void *entries;
    register void *compare_entry ASM_REG("$4");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    void *sort_entry;
    ...
    if (entry_count > 0) {
        ...
        do {
            ...
            if (entry_index < compare_index) {
                register s32 compare_offset ASM_REG("$2");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
