...
void func_8017364C(void *action, void *motion, void *sprite, void *actor) {
    ...
    s32 ability_id;
    s32 normal_ability;
    register s32 use_player ASM_REG("$20");   /* MATCH pin: keeps a constant in a register as retail does */
    u16 effect_ticks;
    u16 finish_ticks;
    u8 *global_state;
    u8 *ability;
...
check_target_type:
    {
        ...
        ASM_KEEP(ability_table);   /* MATCH pin: keeps a statement from moving across a call/branch */
        ability_kind = *ability;
        ability_table -= 0x21DC;
        ability_entry = ability_kind * 20;
        ability_entry += (u32)ability_table;
        ASM_KEEP(ability_entry);   /* MATCH pin: load-bearing for the whole function shape */
...
find_target:
    *(void * volatile *)((u8 *)actor + 0x60) =
    ...
    ASM_KEEP(use_player);   /* MATCH pin: keeps a statement from moving across a call/branch */
    {
