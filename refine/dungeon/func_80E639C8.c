...
selection_ready:
    ...
    ((S_801731C8_0 *)action)->unk_98 &= 0xFF7F;
    {
        ...
        ASM_KEEP(special_flag);   /* MATCH pin: retail basic-block layout depends on it */
        if (special_flag != 0) {
            target = D_800814A8;
...
state_2:
...

    {
        ...
        do {
            ...
            ASM_KEEP(particle);   /* MATCH pin: retail keeps a computation the compiler would drop */
            {
                void *particle_part;
                s32 color_roll;
                register s32 color_choice ASM_REG("$4");   /* MATCH pin: retail keeps a computation the compiler would drop */
                ...
                *(LocalPacket *)((u8 *)particle + 0x40) = D_801755D4;
                ...
                ASM_KEEP(particle_part);   /* MATCH pin: retail keeps a computation the compiler would drop */
...
assign_counter:
            ...
            ASM_KEEP(next_count);   /* MATCH pin: load-bearing for the whole function shape */
counter_test:
            ;
