...
void func_80093E74(s32 slot_table, void *unused_arg1, void *unused_arg2, S_80093E74_0 *owner) {
    ...
    u8 effect_y;
    s32 record_index;
    register s32 record_slot_addr ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
    register s32 saved_index ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void **record_table;
    void **record_slot;
    ...
    {
        s32 cleared_flags;
        register s32 flags_index ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
        register S_80093E74_1 *slot_entry ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
        void **source_table;
        u8 index_byte;
        ...
        flags_index &= 0x1F;
        ASM_KEEP(flags_index);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        index_byte = (u8)flags_index;
        saved_index = index_byte;
        record_index = saved_index;
        ASM_KEEP(record_index);   /* MATCH pin: retail delay-slot fill depends on it */
    ...
    *(Copy140 *) record_storage = *(Copy140 *) source_record;
    ...
    ASM_KEEP(record_table);   /* MATCH pin: retail schedule: same instructions, different order without it */
