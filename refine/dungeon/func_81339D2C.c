...
void func_80170D2C(s32 unused, s16 duration, s32 offset_x, s32 offset_y, s32 offset_z) {
    register s32 duration_value ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    s32 step_divisor;
    s32 delta_z;
    s32 delta_y;
    register s32 delta_x ASM_REG("$6");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 fixed_value;
    s32 rounded_x;
    s32 rounded_y;
    s32 rounded_z;
    ...
    if (obj != 0) {
        ...
        ASM_KEEP(duration_value);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        rounded_x = delta_x;
        if (rounded_x < 0) rounded_x += 3;
        ((S_80170D2C_0 *)work)->unk_68 = rounded_x >> 2;
        ASM_KEEP(delta_x);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        rounded_y = delta_y;
