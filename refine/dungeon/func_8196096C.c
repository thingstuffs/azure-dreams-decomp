...
void func_8196096C(s32 y_offset, Input *origin, Input *quad_data, s32 draw_depth) {
    ...
    register s32 *scratch_words ASM_REG("$1");   /* MATCH pin: retail immediate-load split depends on it */
    register s32 saved_y_offset ASM_REG("$21") = y_offset;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *texture_config;
    u16 corner_uv;
    s32 neutral_color;
    s32 shade;
    register s32 coord_x ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    ...
    ASM_KEEP_NV(rotation);   /* MATCH pin: retail immediate-load split depends on it */
    ASM_KEEP_NV(matrix);   /* MATCH pin: retail immediate-load split depends on it */
    ...
    if (quad_input != 0) {
        ...
        ASM_KEEP_DEP_NV(record, corner_uv);   /* MATCH pin: retail keeps a computation the compiler would drop */
        quad_packet = record;
