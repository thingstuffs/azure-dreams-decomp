...
void func_80025710(void *list_head, s32 x, s32 y, s16 upper_bound, s32 lower_bound) {
    ...
    void *range_data;
    void *position_data;
    register void *next_entry ASM_REG("$2");   /* MATCH pin: keeps a statement from moving across a call/branch */
    void *entry;
    void *list_link;
    register void *head ASM_REG("$17");   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    if (list_link != head) {
        ASM_KEEP(list_link);   /* MATCH pin: load-bearing for the whole function shape */
