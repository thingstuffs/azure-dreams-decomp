...
void *BODY_NAME(s32 setup_bits, s8 grid_x, s8 grid_y, s16 placement_value) {
    ...
    register void *node_data ASM_REG("$20");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    DungeonNode *node;
    DungeonSub1 *state;
    DungeonSub1 *init_state;
    register void *setup_alias ASM_REG("$19");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    DungeonArgBits saved_setup;
    s32 state_flags;
    register s32 status_flags ASM_REG("$3");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 setup_mask ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    saved_grid_y = grid_y;
    ASM_KEEP_DEP_NV(saved_grid_x, call_count);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ASM_KEEP_DEP_NV(saved_placement, call_target);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ASM_KEEP_DEP_NV(saved_grid_y, call_target);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
