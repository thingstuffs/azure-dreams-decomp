...
void FUNC_819A6800_BODY(void *sequence, void *out_position)
{
    ...
    register u8 *owner_object ASM_REG("$16") = owner - 0x20;   /* MATCH pin: load-bearing for the whole function shape */
    u16 timer;
    s32 state;
...
state2:
    {
        ...
        u8 *effect_data;
        register u8 *sprite ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
...
state5:
    ...
    {
        u16 unused_state;
        register u16 cleanup_delay ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
