...
s32 func_80024D58(void *node) {
    ...
    void **render_context;
    s32 grid_origin;
    register u8 *rotated_points ASM_REG("$22");   /* MATCH pin: load-bearing for the whole function shape */
    u8 *grid;
    u32 address_mask;
    s32 point_index;
    s32 ring;
...
next_object:
    ...
    ASM_KEEP_NV(ring_params);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ring_x = grid_origin + 672;
    do {
        s32 point_offset = ring * 8;
        s32 half_count = 8 - ring;
        u8 *ring_points = source_points + point_offset;
        ASM_KEEP_NV(ring_points);   /* MATCH pin: keeps a statement from moving across a call/branch */
...
next_side:
        ...
        case 1:
            ...
            if (point_index < (point_index + frame.p.f528)) {
                ...
                do {
                    ...
                    ASM_KEEP_NV(ring);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
                    ASM_USE2_NV(grid, grid);   /* MATCH pin: load-bearing for the whole function shape */
                    point_count = ((S_80024D58_5 *)(&frame.p))->unk_18;
                } while (point_index < (ring + frame.p.f528));
            }
            break;
        ...
        span_base -= 2;
        ASM_USE2_NV(span_base, span_base);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ring_params -= 6;
        grid_row -= 0x40;
        ring -= 1;
        ring_x -= 0x60;
...
next_quad:
            {
                ...
                u8 *top_pair;
                u8 *bottom_pair;
                register s32 row_offset ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
                ...
                ASM_USE(quad);   /* MATCH pin: retail schedule: same instructions, different order without it */
