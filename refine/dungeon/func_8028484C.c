...
retry:
        ...
        {
            register u8 *position_page ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
            u8 *meta_page;
            u8 *meta;
            ...
            ASM_KEEP(position_page);   /* MATCH pin: load-bearing for the whole function shape */
            position = position_page + 0x39c8;
            meta_page = (u8 *)0x800e0000;
            ASM_KEEP(meta_page);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        entries = (SpawnEntry *)D_800E3CD8;
        {
            ...
            s32 cell_offset;
            s32 row_offset;
            register s32 update_row_shift ASM_REG("$3");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
            register s32 copy_row_shift ASM_REG("$3");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
            s32 flags_row_shift;
            s32 cell_flags;
            s32 cell_index;
            MapCell *spawn_cell;
            ...
            ASM_KEEP(flags_row_shift);   /* MATCH pin: retail schedule: same instructions, different order without it */
            cell_flags = 1;
            ASM_KEEP(cell_flags);   /* MATCH pin: retail delay-slot fill depends on it */
