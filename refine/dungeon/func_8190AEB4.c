...
void func_8190AEB4(void *effect, s32 *position, void *transform)
{
    ...
    u16 next_x;
    u16 next_y;
    register s32 entry ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
    u8 *shuffle_order;
    u8 *slot;
    u8 saved_entry;
    s32 order_index;
    ...
    case 2:
        ...
        ASM_KEEP(next_y);   /* MATCH pin: retail keeps a computation the compiler would drop */
        ASM_TAILSLOT_PIN(next_x);   /* MATCH pin: load-bearing for the whole function shape */
        func_800248E8();
    ...
    case 6:
    {
        register Rect *draw_rect_ptr ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
        s32 *draw_data;
        ...
        do {
            ...
            ASM_KEEP(draw_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
