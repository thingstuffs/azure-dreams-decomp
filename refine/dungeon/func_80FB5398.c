...
s32 func_80174B98(void *object_data, void *unused, void *appearance)
{
  ...
  s32 min_xy;
  s32 max_xy;
  register s32 depth_out_or_phase ASM_REG("$20");   /* MATCH pin: load-bearing for the whole function shape */
  s32 last_y;
  ...
  for (;;)
  {
    ...
    s32 far_z;
    s32 near_z;
    register Vertex *vertex ASM_REG("$3");   /* MATCH pin: keeps a constant in a register as retail does */
    s32 camera_z;
    s16 min_y;
    s16 max_y;
    s16 base_y;
    ...
    far_z = -160;
    object = object_data;
    ASM_USE_NV(object);   /* MATCH pin: retail schedule: same instructions, different order without it */
    near_z = 16;
    left_x = -right_x;
    vertex = vertex_base + 3;
    ASM_KEEP4(vertex, right_x, left_x, index_or_row);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP_NV(far_z);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP_NV(near_z);   /* MATCH pin: keeps a statement from moving across a call/branch */
    do
    {
      vertex->x = right_x;
      if (index_or_row < 2)
    ...
    ASM_KEEP4_NV(projection_ptr, projection_ptr, projection_ptr, projection_ptr);   /* MATCH pin: retail schedule: same instructions, different order without it */
    min_xy &= 0xFFFF;
    max_xy &= 0xFFFF;
    index_or_row = 3;
    ...
    ASM_KEEP_MEM_NV(camera_z, projection);   /* MATCH pin: keeps a statement from moving across a call/branch */
      ...
      do
      {
        ...
        if (last_y >= index_or_row)
        {
          ...
          do
          {
            ...
            ASM_USE2_NV(depth_out_or_phase, next_phase);   /* MATCH pin: keeps a statement from moving across a call/branch */
            ASM_KEEP4_NV(layer_phase, layer_phase, layer_phase, layer_phase);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
