...
void *func_81961A04(void *position)
{
    ...
    register s32 last_slot ASM_REG("$18");   /* MATCH pin: keeps a statement from moving across a call/branch */
    void *object;
    s32 slot_offset;
    register s32 control_value ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    register u16 control_scale ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    u16 pos_x;
    u16 pos_y;
    u16 pos_z;
...
allocate_slot:
    ...
    if (object != NULL) {
        ...
        if (slot != last_slot) {
            slot_offset = 0x200;
            ASM_TAILSLOT_PIN(slot_offset);   /* MATCH pin: retail delay-slot contents depend on it */
            func_800272BC();
            return (void *)0x200;
        }
        ASM_KEEP(last_slot);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
