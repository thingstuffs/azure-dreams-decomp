...
void func_800D6804(void *effect, void *state_data)
{
...

    {
        ...
        s32 scaled_delta;
        register s32 delta_copy ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
        s32 accumulated;
        s32 decayed_delta;
        ...
        delta_copy = delta;
        ASM_KEEP(delta_copy);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        scaled_delta += delta;
