...
void func_8009F644(void *object_ptr, s32 action_code, s32 payload, s8 extra_byte) {
    ...
    register s32 saved_action ASM_REG("$21") = action_code;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    s32 kind;
    register s32 shifted_action ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    s32 entry_tag;
    s32 flag_bit;
    s16 action_offset;
    u32 dispatch_index;
...
jt_return:
    ASM_KEEP(saved_action);   /* MATCH pin: load-bearing for the whole function shape */
}
