...
s32 func_8017237C(void *action_data, void *effect_data, void *target_data, void *actor_data)
{
    ...
    u16 *global_flags;
    u8 *global_page;
    register s32 action_ready ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 target_code;
    u8 actor_flags;
    ...
    actor_flags = ((S_8017237C_0 *)actor)->unk_71;
    {
        ...
        ASM_KEEP(actor_flags);   /* MATCH pin: keeps a statement from moving across a call/branch */
        ...
        ASM_KEEP(global_flags);   /* MATCH pin: retail delay-slot fill depends on it */
        action_ready = 0;
        ...
        {
            register void *check_actor ASM_REG("$4") = actor;   /* MATCH pin: retail schedule: same instructions, different order without it */
            register s32 result ASM_REG("$17") = target_code;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
            ...
            ASM_KEEP(effect);   /* MATCH pin: retail schedule: same instructions, different order without it */
