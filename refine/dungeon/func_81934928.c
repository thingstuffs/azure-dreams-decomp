...
{
    register void *self ASM_REG("$18") = effect;   /* MATCH pin: retail address form (%hi/%lo vs origin_component+offset) depends on it */
    register void *output_data ASM_REG("$7");   /* MATCH pin: retail delay-slot fill depends on it */
    register void *owner ASM_REG("$20");   /* MATCH pin: retail register colouring depends on it */
    register void *search_origin ASM_REG("$16");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    void *target;
    void *saved_origin;
    register void *tail_origin ASM_REG("$6");   /* MATCH pin: retail delay-slot contents depend on it */
    void *particle;
    register s32 rand_quotient ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    s16 state;
    s32 spawn_index;
    ...
    if (state != 1) {
        ...
        ASM_KEEP(output_data);   /* MATCH pin: retail register colouring depends on it */
...
main_state:
    ...
    if (((S_81934928_0 *)self)->unk_0C.u == 0x1E) {
        ...
        if (search_origin != NULL) {
            u8 *search_page;
            register u8 *lookup_table ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
            s32 lookup_key_a;
            s32 lookup_key_b;
            void *lookup_origin;
            void *lookup_context;
            saved_origin = search_origin;
            ASM_KEEP_NV(saved_origin);   /* MATCH pin: keeps a constant in a register as retail does */
#ifdef NON_MATCHING
            ...
            if (target != NULL) {
                ...
                ASM_TAILSLOT_PIN(tail_origin);   /* MATCH pin: retail keeps a computation the compiler would drop */
                func_80024344();
            }
        }
    }
