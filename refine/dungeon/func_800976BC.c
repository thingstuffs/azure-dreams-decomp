...
void func_8009CE1C(void *target, s16 base_power, s16 power_bonus, s16 elements,
    ...
    s16 affinity_adjust;
    s32 power;
    register s32 target_elements ASM_REG("$19");   /* MATCH pin: retail register colouring depends on it */
    s32 damage_delta;
    M2C_UNK direction_offset;
    s32 affinity;
    register s16 requested_bonus ASM_REG("$19");   /* MATCH pin: retail register colouring depends on it */
    s32 second_element;
    s32 third_element;
    M2C_UNK affinity_shift;
    register s32 first_affinity ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    S_8009CE1C_2 *position;
    register u16 base = base_power;
    register s16 element_mask ASM_REG("$21") = elements;   /* MATCH pin: load-bearing for the whole function shape */
    s32 base_shift;
    ...
    if ((target != NULL) && (func_8009D218(target, element_mask & 0xFFFF, source_flags) == 0)) {
        ...
        ASM_USE_NV(requested_bonus);   /* MATCH pin: load-bearing for the whole function shape */
