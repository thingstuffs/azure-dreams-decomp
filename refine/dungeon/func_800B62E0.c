...
void *func_800BBA40(s32 tile_x, s32 tile_y, s16 pos_z, M2C_UNK transform_data, s32 scale, s32 setup_word_5c, s32 setup_word_60) {
    ...
    register s32 held_tile_y ASM_REG("$20") = tile_y;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s16 held_pos_z ASM_REG("$21") = pos_z;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register M2C_UNK held_transform ASM_REG("$22") = transform_data;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    if (effect != NULL) {
        ...
        ASM_KEEP(held_tile_y);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ASM_KEEP(held_pos_z);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ASM_KEEP(held_transform);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        ...
        do {
            ...
            angle += 0x999;
            angle_index += 1;
            ASM_KEEP(angle_index);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            setup_cursor += 2;
        } while (angle_index < 5);
        ASM_KEEP(setup_cursor);   /* MATCH pin: retail register colouring depends on it */
