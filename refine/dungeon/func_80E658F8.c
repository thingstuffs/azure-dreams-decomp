...
void func_801750F8(void *action_in, void *direction_data_in, void *sprite_in, void *actor_in)
{
    ...
    register void *sprite ASM_REG("$17") = sprite_in;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void *actor = actor_in;
    u8 state;
...
timer_ge_11:
        ...
        scale_y = ((S_801750F8_2 *)sprite)->unk_1E;
        scale_x += 0x258;
        ASM_KEEP(scale_x);   /* MATCH pin: retail branch polarity depends on it */
        scale_y -= 0x258;
        goto store_xy;
