...
void func_80026680(void *effect, void *transform, void *render_data)
{
    s32 history_index;
    s32 brightness;
    register s32 brightness_delta ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register u8 source_brightness ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register u16 brightness_offset ASM_REG("$3");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s16 phase;
    s16 frames_left;
    u16 fade_brightness;
    u32 next_frame;
    ...
    void *related;
    void *parent;
    register void *parent_render ASM_REG("$3");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
...
attached:
    ...
    ASM_KEEP(brightness_delta);   /* MATCH pin: retail register colouring depends on it */
