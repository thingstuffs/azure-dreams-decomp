    ...
    s32 offset_y, s32 offset_z)
{
    ...
    register s32 saved_duration ASM_REG("$22") = duration;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 saved_offset_x = offset_x;   /* MATCH pin: load-bearing for the whole function shape */
    s32 saved_offset_y = offset_y;
    s32 saved_offset_z = offset_z;
    register void *object ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void *state;
    ...
    ASM_KEEP(saved_offset_x);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
}
