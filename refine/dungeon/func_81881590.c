...
void *func_80024D90(void *source_pos, s16 effect_param) {
    ...
    register void *effect_state ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s32 fixed_offset;
    ...
    do {
        ...
        if (effect != NULL) {
            ...
            fixed_offset = 0x400;
            ASM_KEEP(fixed_offset);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
