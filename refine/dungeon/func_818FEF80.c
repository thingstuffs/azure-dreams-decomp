...
void func_818FEF80(void *state, void *position, void *visual) {
    void *object;
    register void *linked_visual ASM_REG("$8");   /* MATCH pin: load-bearing for the whole function shape */
    s16 ticks_left;
    ...
    if (ticks_left < 0x3C) {
        ...
        ASM_TAILSLOT_PIN(shade_delta);   /* MATCH pin: retail basic-block layout depends on it */
        func_80024954();
        return;
    }
