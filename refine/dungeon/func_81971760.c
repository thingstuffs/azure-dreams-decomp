...
void func_81971760(void *unused_0, void *origin_data, s32 unused_2, s32 offset_x, s32 offset_y, s32 offset_z) {
    register u16 jittered_x ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    register void *effect_arg ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    register u16 coord ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    register u16 position_x ASM_REG("$2");   /* MATCH pin: retail basic-block layout depends on it */
    s32 random_value;
    ...
    register s32 initial_x ASM_REG("$17") = offset_x;   /* MATCH pin: retail basic-block layout depends on it */
    register s32 initial_y ASM_REG("$19") = *(volatile s32 *)&offset_y;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 initial_z ASM_REG("$20") = *(volatile s32 *)&offset_z;   /* MATCH pin: retail basic-block layout depends on it */
    ...
    if (effect != NULL) {
        ...
        ASM_KEEP4_NV(initial_x, effect_state, initial_y, initial_z);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        coord = origin->unk_02;
        ASM_KEEP(coord);   /* MATCH pin: keeps a statement from moving across a call/branch */
        position_x = jittered_x;
        ASM_KEEP(position_x);   /* MATCH pin: load-bearing for the whole function shape */
