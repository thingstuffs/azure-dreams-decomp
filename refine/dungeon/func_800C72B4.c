...
void func_800CCA14(u16 x, u16 y, u16 z) {
    ...
    ASM_KEEP(first_base_angle);   /* MATCH pin: retail schedule: same instructions, different order without it */
    part_index = 3;
    ...
    do {
        ...
        first_angle = angle;
        ASM_KEEP(first_angle);   /* MATCH pin: retail schedule: same instructions, different order without it */
        part_index -= 1;
    ...
    if (target != 0) {
        func_8009CE1C(target, 0xC, D_800E3D7C->unk_11, 9, 0x400, 0, 3);
    }
}
