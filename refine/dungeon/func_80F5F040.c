...
void func_80172840(void *action, void *position, void *sprite_in, void *actor_in)
{
    ...
    register s32 saved_reg ASM_REG("$17");   /* MATCH pin: keeps a statement from moving across a call/branch */
    u16 coords[7];
    volatile s32 direction_x;
    s32 direction_y;
    S_80172840_10 *effect_base;
    register s32 move_x ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 move_y;
    ...
    register u8 *direction_y_table ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *direction_y_ptr;
    register void *sprite ASM_REG("$20") = sprite_in;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *actor ASM_REG("$19") = actor_in;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ASM_KEEP4_NV(sprite, actor, saved_reg, heading);   /* MATCH pin: retail schedule: same instructions, different order without it */
...
state_0:
{
    s32 move_kind;
    void *move_object;
    register s32 special_flag ASM_REG("$2");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    if ((u32)(((S_80172840_4 *)sprite)->unk_04.u - 1) < 2U &&
        ...
        if ((s16)((S_80172840_1 *)action)->unk_A8.u >= 0) {
            ...
            for (; counter >= 0; counter--) {
                register void *particle_data ASM_REG("$16");   /* MATCH pin: retail schedule: same instructions, different order without it */
                particle = func_8003FD64(0x312, D_80083498);
                if (particle != 0) {
                    func_8004491C(particle, D_80045340);
                    particle_data = ((S_80172840_7 *)particle)->unk_0C;
                    ASM_KEEP_NV(particle_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
                    {
                        register void *particle_callback ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
                    ...
                    }
                    {
                        ...
                        s32 origin_x;
                        s32 offset_x;
                        register s32 heading_x ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */

                        random_x = func_800A6D30() & 0x3F;
                        origin_x = ((S_80172840_5 *)position)->unk_02;
                        offset_x = coords[4];
                        heading_x = direction_x;
                        ASM_USE2_NV(heading_x, random_x);   /* MATCH pin: retail schedule: same instructions, different order without it */
                        origin_x += offset_x;
