...
void func_800B2074(s32 world_x, s32 world_z) {
    ...
    void *object;
    s32 pos_x;
    register s32 pos_z ASM_REG("$19");   /* MATCH pin: load-bearing for the whole function shape */
    void *init_object;
    void *init_data;
    void *callback;
    S_800B2074_5 *state_fields;
    register u16 inherited_value ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    s8 *map_state;
    ...
    if (object != NULL) {
        init_object = object;
        ASM_KEEP(init_object);   /* MATCH pin: retail schedule: same instructions, different order without it */
        callback = D_800B1F34;
        ...
        ASM_KEEP(pos_z);   /* MATCH pin: keeps a statement from moving across a call/branch */
        ...
        ASM_KEEP(state_fields);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
