...
s32 func_800CA93C(void *actor_arg, s32 action_arg, void *origin_arg) {
    ...
    s32 clear_flag_mask;
    S_800CA93C_3 *target;
    register void *lookup_origin ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 found_entry;
    void *target_entry;
    ...
    if (target_entry != 0) {
        ...
        if (found_entry == (s32)((S_800CA93C_0 *)actor)->unk_60) {
            ASM_KEEP(target);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
            ASM_KEEP(clear_flag_mask);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    ASM_KEEP(saved_actor);   /* MATCH pin: keeps a statement from moving across a call/branch */
    ASM_KEEP(action);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ASM_KEEP(origin);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
