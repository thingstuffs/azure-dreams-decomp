...
{
    register s16 saved_x ASM_REG("$20");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 saved_render_param ASM_REG("$21");   /* MATCH pin: keeps a statement from moving across a call/branch */
    u16 flags;
    ...
    ASM_KEEP_NV(saved_render_param);   /* MATCH pin: retail schedule: same instructions, different order without it */
    object = func_8003FC64(2);
    if (object != NULL) {
        saved_x = x;
        ASM_KEEP(saved_x);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
