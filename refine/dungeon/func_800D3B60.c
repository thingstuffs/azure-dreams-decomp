...
void func_800D92C0(void *move_state, void *context, S_800D92C0_2 *position, void *actor) {
    ...
    s16 target_heading;
    register M2C_UNK direction_or_x ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s32 next_y;
    s32 room_cache;
    s32 turn_flags;
    register s32 move_result ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 flags_or_heading ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    s32 actor_flags;
            ...
            if ((func_8009A66C((s16)candidate_heading, position, actor, 0x20) << 0x10) > 0) {
                ...
                }
                {
                    ...
                    direction_or_x = ((u16) ((S_800D92C0_0 *)actor)->unk_2A.u >> 8) & 0xE;
                    {
                        ...
                        register s32 next_x ASM_REG("$2") = position->unk_24.at00.v;   /* MATCH pin: load-bearing for the whole function shape */
