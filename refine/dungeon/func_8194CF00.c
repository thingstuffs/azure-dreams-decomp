...
void func_80024700(void *source_object)
{
    ...
    register s32 object_arg ASM_REG("$4") = 0x212;   /* MATCH pin: retail schedule: same instructions, different order without it */
    register u8 *resource ASM_REG("$5");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 angle_or_zero;
    u32 link_page;
    ...
    if (object != NULL) {
        ...
        ASM_KEEP(link_page);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_KEEP(offset_length);   /* MATCH pin: retail basic-block layout depends on it */
        angle_or_zero = 0;
        ASM_KEEP(angle_or_zero);   /* MATCH pin: retail delay-slot fill depends on it */
        ...
        ASM_KEEP(resource);   /* MATCH pin: keeps a statement from moving across a call/branch */
