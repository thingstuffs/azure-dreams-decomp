...
{
    void *action;
    register void *context ASM_REG("$19");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register void *sprite ASM_REG("$17");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    void *actor;
    s32 state;
    register u8 *direction_table ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    register void *actor_to_check ASM_REG("$4");   /* MATCH pin: retail basic-block layout depends on it */
    u8 *status;
    s32 actor_flags;
