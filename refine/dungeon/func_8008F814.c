...
s32 func_80094F74(Ctx *ctx, char *text, Item *item, Unit *unit) {
    ...
    if ((ground_item->b1 == 14) || ((ground_item->b1 == 12) && (ground_item->b0 == 4)) || ((ground_item->b1 == 18) && (ground_item->b0 == 1))) {
        ...
        if (item_kind == 12) {
            ASM_USE2(special_item, item_kind);   /* MATCH pin: load-bearing for the whole function shape */
            ...
            ASM_KEEP(state_page);   /* MATCH pin: retail register colouring depends on it */
            *(u16 *) 0x8001209A = 0;
        ...
        *pickup_slot = *(u32 *) consumed_item;
        ...
        if ((slot_index < 0) || (entry_index < 0)) {
            ...
            *(void ***) (inventory_page + (entry_index * 4) + 0x29C) = inventory_slot;
            ...
            ASM_KEEP_NV(entity_page);   /* MATCH pin: retail immediate-load split depends on it */
