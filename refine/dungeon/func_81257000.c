...
void BODY_NAME(void *root_arg, void *out_pos_arg, void *dst_part_arg)
{
    s16 pos_offset[3];
    void *root = root_arg;
    register void *out_pos ASM_REG("$17") = out_pos_arg;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    S_81257000_3 *dst_part = dst_part_arg;
    S_81257000_1 *owner;
    register void *src_part ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
