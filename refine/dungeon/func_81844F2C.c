...
void func_81844F2C(void *effect_data) {
    u8 *effect;
    register u8 *flag_page ASM_REG("$3");   /* MATCH pin: keeps a constant in a register as retail does */
    register s32 tick_or_index;
    register u32 saved_state ASM_REG("$6");   /* MATCH pin: retail immediate-load split depends on it */
    u32 position;
    u32 step;
    s32 state;
    u32 color_delta;
    ...
    ASM_KEEP(tick_or_index);   /* MATCH pin: retail schedule: same instructions, different order without it */
    tick_or_index -= 1;
