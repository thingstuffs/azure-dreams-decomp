...
void func_800D1824(u8 *tiles)
{
    ...
    u8 *tile_z;
    register u8 *packet_len ASM_REG("$17");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    u8 *vertex_0;
    u8 *render_state;
    u8 *draw_prim;
    u8 *vertex_1;
    ...
    ASM_KEEP(tile);   /* MATCH pin: retail register colouring depends on it */
    if (*tile != 0) {
