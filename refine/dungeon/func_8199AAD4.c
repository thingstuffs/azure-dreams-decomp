...
void func_8199AAD4(void *self, void *coords)
{
    ...
    s32 *global_flags;
    void *delta_out;
    register s32 delay_frames ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    D_80024A70++;
...
finish:
    ...
    count++;
    flags_value |= 0x8000;
    ASM_KEEP(flags_value);   /* MATCH pin: retail schedule: same instructions, different order without it */
