...
{
    register s32 neighbor_x ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 tile_or_turn ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    s32 direction;
    register s32 lookup_value ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
    u32 config_or_dir;
    register u32 dir_or_tiles ASM_REG("$9");   /* MATCH pin: retail register colouring depends on it */
    u32 y_offsets;
    u32 x_offsets;
    register s32 origin_y ASM_REG("$12");   /* MATCH pin: load-bearing for the whole function shape */
    s32 origin_x;
    register s32 row_work ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    u32 tiles_or_shift;
    ...
    origin_x = neighbor_x;
    ASM_KEEP_NV(origin_x);   /* MATCH pin: retail schedule: same instructions, different order without it */
    origin_y = tile_or_turn;
    ASM_KEEP_NV(origin_y);   /* MATCH pin: retail schedule: same instructions, different order without it */
    config_or_dir = 0x80080000;
    ASM_KEEP_NV(config_or_dir);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    ASM_KEEP_NV(lookup_value);   /* MATCH pin: retail immediate-load split depends on it */
...
#endif
    ASM_KEEP_NV(y_offsets);   /* MATCH pin: retail register colouring depends on it */
    row_work += y_offsets;
