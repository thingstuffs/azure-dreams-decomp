...
{
    register s32 match_x ASM_REG("$22") = tile_x;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 match_y ASM_REG("$21") = tile_y;   /* MATCH pin: keeps a statement from moving across a call/branch */
    void *head = node;
    ...
    while (node != head) {
        ...
        if ((tile->unk_24 == (match_x & 0xFFFF)) &&
            ...
            ((u32)(((S_80174800_1 *)node)->unk_13 - 0x33) >= 4U)) {
            actor->unk_84 = 0x7C;
            actor->unk_85 =
        ...
        node = (u8 *)((S_80174800_1 *)node)->unk_5C + 0x20;
    }
}
