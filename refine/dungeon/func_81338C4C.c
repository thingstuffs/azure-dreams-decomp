...
void func_8016FC4C(void *effect, void *entity_data, void *object_data) {
    ...
    u16 move_ticks;
    s16 turned_angle;
    register u16 angle ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    u8 phase;
    ...
    case 4:
        ...
        if (((s16)turn_ticks == 3) || ((s16)turn_ticks == 6) || ((s16)turn_ticks == 9) || ((s16)turn_ticks == 0xC)) {
            ...
            ASM_KEEP(angle);   /* MATCH pin: retail branch polarity depends on it */
        ...
        ASM_KEEP(turn_phase);   /* MATCH pin: retail schedule: same instructions, different order without it */
...
update_sprite:
    ...
    if (((S_8016FC4C_8 *)((u8 *)common_page))->unk_5D50 != 0) {
        ...
        ASM_KEEP(sprite_direction);   /* MATCH pin: load-bearing for the whole function shape */
