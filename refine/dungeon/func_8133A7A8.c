...
void func_801717A8(Entity *entity) {
    ...
    register void *focus_arg ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    register void *offset_arg ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    s32 transition_ticks;
    u16 previous_state;
    register s32 coord_sum ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
    register s32 camera_mode ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    register s16 *camera_offset ASM_REG("$17");   /* MATCH pin: keeps a constant in a register as retail does */
    register s16 *focus_pos ASM_REG("$16");   /* MATCH pin: retail register colouring depends on it */
    register s32 focus_y ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
...
jt_call_common:
    entity->timer = 0;
    ASM_KEEP(transition_ticks);   /* MATCH pin: retail schedule: same instructions, different order without it */
...
jt_c4: {
    ...
    camera_mode = 1;
    ASM_KEEP(camera_mode);   /* MATCH pin: retail schedule: same instructions, different order without it */
