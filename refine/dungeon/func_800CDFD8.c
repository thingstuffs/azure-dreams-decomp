...
void func_800D3738(void *unused, void *position_in, void *sprite_in, s16 depth_bias_in) {
    s32 screen_y3;
    s32 left_x;
    register s32 half_left_x ASM_REG("$3");
    ...
    s32 vertex3_visible;
    s32 camera_y;
    register u16 height ASM_REG("$4");
    u16 coord;
    u16 pivot_x;
    u16 pivot_y;
    register u32 width ASM_REG("$4");
    u32 texture_u;
    u32 flipped_width;
    u32 texture_v;
    register u32 texture_right ASM_REG("$3");
    register u32 packed_v ASM_REG("$2");
    u32 screen_bound;
    register u32 sprite_flags ASM_REG("$3");
    u32 any_visible;
    u32 depth;
    u32 ot_depth;
    u8 part_u;
    ...
    void *quad;
    u8 *scratch;
    register u8 *globals_page ASM_REG("$9");
    register void *position ASM_REG("$3");
    u8 *page_dependency;
    void *matrix_or_part;
    void *part;
    register void *render_state ASM_REG("$22");
    void *packet;
    void *sprite;
    register s16 depth_bias ASM_REG("$16");

    position = position_in;
    sprite = sprite_in;
    ASM_KEEP_NV(sprite);
    depth_bias = depth_bias_in;
    ASM_KEEP_NV(depth_bias);
    {
        u8 *world_pos;
        void *screen_pos;
        void *depth_out;
        register void *projection_flags ASM_REG("$7");
        register void *render_root ASM_REG("$8");
        world_pos = (u8 *)0x1F800000;
        screen_pos = world_pos;
        ASM_KEEP_NV(screen_pos);
        screen_pos = (void *) ((u32) screen_pos | 0xB8);
        depth_out = world_pos;
        ASM_KEEP_NV(depth_out);
        ASM_KEEP_MEMDEP_NV(world_pos, page_dependency, *(void **)D_80083160);
        scratch = world_pos;
        ASM_KEEP_NV(scratch);
        ...
        ASM_KEEP_MEMDEP_NV(scratch, page_dependency, *(void **)D_80083160);
#ifdef NON_MATCHING
        globals_page = D_80083160 - 0x3160;
#else
        ASM_SET(globals_page);
#endif
    ...
    if (ot_depth < 0x1D6U) {
        ...
        func_800649A0();
        {
            void *angles;
            void *local_matrix;
            register u16 screen_offset ASM_REG("$2");
            angles = scratch;
            ASM_KEEP_NV(angles);
            ...
            ASM_SET(local_matrix);
            local_matrix = scratch;
            ASM_KEEP_NV(local_matrix);
        ...
        }
        {
            ...
            ASM_KEEP_NV(view_matrix);
            ASM_SET(local_matrix);
            local_matrix = scratch;
            ASM_KEEP_NV(local_matrix);
            local_matrix = (void *) ((u32) local_matrix | 0xD0);
            ASM_SET(combined_matrix);
            combined_matrix = scratch;
            ASM_KEEP_NV(combined_matrix);
