...
void func_80170F68(void *effect) {
    ...
    u8 *particle_page;
    Particle *particle;
    register s32 timer_snapshot ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    void *actor;
    s16 state;
    s16 timer;
    u16 old_timer;
    ...
    s32 early_x;
    s32 early_y;
    register u32 flag_mask ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    s32 enabled;
...
brighten:
    ...
    ASM_KEEP(timer_snapshot);   /* MATCH pin: retail basic-block layout depends on it */
    if (ramp_count > 0) {
        do {
            ramp_count--;
