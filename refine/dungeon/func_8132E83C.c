...
void func_8016583C(S_8016583C_0 *source, s16 duration, s32 scale, s32 offset_x, s32 offset_y, s32 offset_z, s32 velocity_z) {
    ...
    void *effect;
    register s16 held_duration ASM_REG("$22") = duration;   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register s32 held_scale ASM_REG("$21") = scale;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 held_offset_y = offset_y;
    register s32 held_offset_x ASM_REG("$16") = offset_x;   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if (effect != NULL) {
        ...
        ASM_KEEP(held_duration);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ASM_KEEP(held_scale);   /* MATCH pin: retail schedule: same instructions, different order without it */
