...
void func_80172F04(S_80172F04_0 *action, S_80172F04_3 *motion, S_80172F04_1 *animation, void *actor_ptr)
{
    ...
    ASM_KEEP_NV(animation);   /* MATCH pin: retail register colouring depends on it */
    ASM_KEEP_NV(motion);   /* MATCH pin: retail keeps a computation the compiler would drop */
    actor = actor_ptr;
    state = action->unk_9B;
    if (state == 1) {
        goto state_1;
...
state_1:
    ...
    if (((s16)timer == 4) || (animation->unk_14 & 0x8000)) {
        ...
        animation->unk_2C = direction_table;
