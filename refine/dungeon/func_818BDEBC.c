...
void func_800256BC(EffectState *effect, Motion *effect_motion, register ColorPart *part) {
    ...
    register void *owner_meta ASM_REG("$17");
    void *owner_node;
    void *source_motion;
    void *target_motion;
    ...
    s32 next_state;
    u32 color;
    register s32 x_distance ASM_REG("$5");
    s16 source_z;
    u16 ground_z;
    ...
    s32 next_y;
    register s16 *direction_steps ASM_REG("$3");
    s32 direction_index;
    s32 offset_x;
    s32 offset_y;
    register s32 dest_x ASM_REG("$5");
    s32 end_tile_x;
    u16 end_tile_y;
...
case_0:
    ...
    if (PTR_AT(owner, 0x60) != 0) {
...

        {
            ...
            register s32 motion_y ASM_REG("$3") = S16_AT(motion, 6);
            ASM_SCHED_BARRIER();
    ...
    direction_index = (s16)state->direction;
    ASM_KEEP_DEP_NV(table_base, direction_index);
