...
s32 func_80174228(u8 *item_data)
{
    ...
    s32 strip_offset;
    s32 tex_coord;
    register s32 far_z ASM_REG("$7");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 near_z ASM_REG("$6");   /* MATCH pin: keeps a statement from moving across a call/branch */
    u8 *next_node;
    register s32 left_coord ASM_REG("$4");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 right_x ASM_REG("$4");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 top_y;
    register s32 bottom_y;
    register s32 half_width;
    register s32 left_x;
    u32 addr_mask;
    register u32 tag_mask ASM_REG("$5");   /* MATCH pin: retail immediate-load split depends on it */
    register u8 *init_vertex ASM_REG("$3");   /* MATCH pin: keeps a constant in a register as retail does */
    u8 *vertex;
    u8 *prim;
    u8 shade;
    u8 tex_height;
    ...
    register s32 render_term ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
    u8 *render_state;
    register u8 *profile_storage ASM_REG("$4");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register u8 *transform ASM_REG("$4");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    do {
        ...
        ASM_USE_NV(render_term);   /* MATCH pin: keeps a statement from moving across a call/branch */
        ...
        do {
            ...
            ASM_USE2_NV(screen_coord, biased_depth);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_KEEP_MEMDEP_NV(profile_peak, profile_dst, storage[0]);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        if ((u32)depth < 0x1E0U) {
            ...
            do {
                ...
                ASM_USE(profile_base);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                render_term = 0xB;
                render_term -= i;
