    ...
    do {
        {
            ...
            if ((s8)particle_view->level >= 48) {
                ...
                particle_view->level = 0;
                {
                    ...
                    ASM_KEEP(reset_index);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                }
            }
        }
        {
...

            {
                ...
                if (level >= 33) {
                    ...
                    register s32 fade_level ASM_REG("$2") = level - 32;   /* MATCH pin: load-bearing for the whole function shape */
                    ASM_KEEP(saved_intensity);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                    ASM_TAILSLOT_PIN(fade_level);   /* MATCH pin: load-bearing for the whole function shape */
                    func_800F6FE0();
                }
                if (level < 16) {
            ...
            ASM_KEEP_NV(quad_fields);   /* MATCH pin: retail immediate-load split depends on it */
