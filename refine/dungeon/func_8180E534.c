...
void *func_80027534(s16 pos_x, s16 pos_y, s16 pos_z)
{
  ...
  register s32 angle_value ASM_REG("$16");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
  s32 sample_angle;
  s32 scale_shift;
  register s32 motion_angle ASM_REG("$23");   /* MATCH pin: retail register colouring depends on it */
  s32 sample_index;
  s32 group_index;
  register s16 held_y ASM_REG("$5") = pos_y;   /* MATCH pin: keeps a statement from moving across a call/branch */
  register s16 held_z ASM_REG("$6") = pos_z;   /* MATCH pin: keeps a statement from moving across a call/branch */
  register s32 setup_origin ASM_REG("$6");   /* MATCH pin: keeps a statement from moving across a call/branch */
  s32 setup_size;
  register void *setup_asset ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
  register void *setup_data ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
  register s32 motion_component ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
  s32 base_shift;
  int index_step;
  register s32 angle_arg ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
  s32 trig_result;
  register void *render_state ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
  void *effect_state;
  void *effect;
  sp.sp1A = held_z;
  ...
  group_index = 0;
  ASM_USE2_NV(setup_asset, setup_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
  {
    register u16 facing_angle ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
  ...
  do
  {
    sample_index = 0;
    {
      register s32 base_angle ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
      base_angle = sp.sp28;
      index_step = 1;
      base_shift = 5;
    ...
    {
      {
        register void *init_effect ASM_REG("$4") = effect;   /* MATCH pin: retail schedule: same instructions, different order without it */
        register void *init_data ASM_REG("$5") = D_800CEEFC;   /* MATCH pin: keeps a statement from moving across a call/branch */
        register void *callback ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
        ASM_USE2(init_effect, init_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
        callback = D_8002744C;
      ...
      {
        void *positions;
        register u16 initial_y ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
        register u16 initial_z ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
      ...
      trig_result = func_800644B8(sample_angle);
      {
        ...
        *((s16 *) (((s8 *) render_state) + 0x1C)) = 0x400;
        ...
        {
          register void *render_asset ASM_REG("$2") = D_80028880;   /* MATCH pin: load-bearing for the whole function shape */
        ...
        *((s16 *) (((s8 *) effect_state) + 0x66)) = 0xC;
        {
          register u16 effect_angle ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
