...
void *func_80171D64(s32 spawn_flags, s32 sprite_x, s32 sprite_y, s32 height)
{
    ...
    register s32 y_or_state ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 init_flags ASM_REG("$23");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    void *object;
    register void *position ASM_REG("$22");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 mode_or_object ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    ...
    saved_x = sprite_x;
    ASM_KEEP_DEP_NV(saved_x, mode_or_object);   /* MATCH pin: keeps a statement from moving across a call/branch */
    kind_or_position = (s32)D_80083498;
    ASM_KEEP_DEP_NV(kind_or_position, saved_x);   /* MATCH pin: retail immediate-load split depends on it */
    height_or_sprite = height;
    ASM_KEEP_DEP_NV(height_or_sprite, kind_or_position);   /* MATCH pin: retail schedule: same instructions, different order without it */
    y_or_state = sprite_y;
    ...
    ASM_KEEP(height_or_sprite);   /* MATCH pin: retail schedule: same instructions, different order without it */
    mode_or_object = saved_flags & 3;
    ((S_80171D64_3 *)((void *)height_or_sprite))->unk_2C = default_frames;
    ASM_KEEP(default_frames);   /* MATCH pin: retail schedule: same instructions, different order without it */
    state_flags = 0x20;
    ASM_KEEP(state_flags);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if (mode_or_object >= 2) {
        ASM_KEEP(mode_or_object);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (((init_flags & ~3) << 16) == 0) {
        ...
        ASM_KEEP_NV(mode_or_object);   /* MATCH pin: load-bearing for the whole function shape */
        if (!(((S_80171D64_1 *)result)->unk_14 & 0x200)) {
            kind_or_position = (s32)position;
            ASM_KEEP(kind_or_position);   /* MATCH pin: retail delay-slot contents depend on it */
