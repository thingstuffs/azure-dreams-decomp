...
void func_80171104(void *actor_arg, void *motion_arg, void *object_arg)
{
    ...
    s16 floor_height;
    s16 actor_height;
    register s32 view_index_copy ASM_REG("$20");   /* MATCH pin: keeps a constant in a register as retail does */
    u8 *animation;
...
compare_direction:
    ...
    if (!(flags & 0x8000)) {
        ...
        ASM_KEEP(view_index_copy);   /* MATCH pin: load-bearing for the whole function shape */
