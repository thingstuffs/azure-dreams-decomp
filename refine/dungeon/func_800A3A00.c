...
{
    DungeonGroup *group;
    register DungeonGroup *cell_group ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
    ...
    do {
        ...
        if (group->count != 0) {
            ...
            cell_group = group;
            ASM_KEEP(group_bits_offset);   /* MATCH pin: retail schedule: same instructions, different order without it */
            ASM_KEEP(cell_group);   /* MATCH pin: retail register colouring depends on it */
            cell_offset = 0;
            do {
                if (!(cell_index & 7)) {
