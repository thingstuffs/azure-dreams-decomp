...
void func_80172D1C(void *actor, void *motion, void *sprite, void *entity) {
    ...
    s32 velocity_y;
    s32 speed_work;
    register s32 component_work ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    register s32 part_or_half_speed ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 anim_id ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register s32 anim_frame ASM_REG("$6");   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if (((S_80172D1C_0 *)actor)->unk_9B == 1) {
        ...
        do {
            ...
            if (particle != 0) {
                ...
                ASM_KEEP(component_work);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
                ((S_80172D1C_3 *)((u8 *)component_work))->unk_48 = 10;
                sprite_flags = ((Rec_D_80082E80 *)sprite)->unk_28.at00_s32.v;
                ASM_KEEP(sprite_flags);   /* MATCH pin: retail schedule: same instructions, different order without it */
