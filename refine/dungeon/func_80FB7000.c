...
void *BODY_NAME(s32 spawn_flags, s8 pos_x, s8 pos_y, s16 part_a_value)
{
    ...
    void *obj;
    register s32 object_type ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register void *object_pool ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
    S_80FB7000_3 *part_b;
    void *part_a;
    s16 saved_flags;
    s32 kind;
    s32 primary_flags;
    s32 secondary_flags;
    register s32 entry_index ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s32 entry_count;
    u16 actor_value;
    register s32 twice_index ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
    register s32 entry_offset ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    u8 *selected_entry;
    s16 *rect_ptr;
    s16 rect[4];
    u8 *entry;
    register s8 saved_x ASM_REG("$21");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    work = 0;
    object_type = 0x112;
    ASM_USE_NV(object_type);   /* MATCH pin: retail schedule: same instructions, different order without it */
    saved_x = pos_x;
    saved_part_value = part_a_value;
    object_pool = D_80083498;
    ASM_USE2_NV(saved_part_value, object_pool);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if (obj != 0) {
        ...
        if (kind == 1) {
            ...
            ASM_KEEP(primary_flags);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
...
post_kind:
        ...
        entry_index = 0;
        ASM_USE_NV(entry_index);   /* MATCH pin: retail basic-block layout depends on it */
    ...
    scan_entries:
        ...
        ASM_TAILSLOT_PIN(entry_index);   /* MATCH pin: retail delay-slot contents depend on it */
        func_8016AA40();
scan_done:

        rect_ptr = rect;
        ASM_KEEP(rect_ptr);   /* MATCH pin: retail schedule: same instructions, different order without it */
