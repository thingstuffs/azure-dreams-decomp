...
void func_818154FC(void *effect, S_818154FC_5 *position, Rec_D_80082E80 *sprite) {
    ...
    s32 state;
    register s32 tracking_state ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    u16 spawn_tick;
    u16 angle;
    u8 *point_table;
...
update_position:
  {
    u16 position_z;
    register u16 final_z ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    register u16 delta_z ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (spawn_tick & 1) {
        ...
        if (node != NULL) {
            ...
            if (((S_818154FC_3 *)parent)->unk_60.u != NULL) {
                ...
                s32 compare_one;
                u8 *motion;
                register s32 z_step ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
                s32 other_z;
                ...
                compare_one = 1;
                ASM_KEEP(compare_one);   /* MATCH pin: load-bearing for the whole function shape */
...
follow_effect:
    if (((S_818154FC_0 *)effect)->unk_88 == 0x63) {
        register u8 *node ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
        register u8 *tracked_position ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
