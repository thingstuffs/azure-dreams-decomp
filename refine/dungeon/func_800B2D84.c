...
void func_800B84E4(DungeonPosition *position, DungeonParameters *parameters, s32 draw_order, s32 blend_mode)
{
  ...
  s32 center_xy;
  s32 outer_color3;
  register u8 *state_base ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
  u8 *quad_packet;
  register s32 packet_order ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
  ...
  *((volatile s32 *) (((volatile u8 *) scratchpad) + 0x14)) = parameters->y1;
  ...
  if (parameters->count != 0)
  {
    ...
    do
    {
      ...
      *((volatile u8 *) (((volatile u8 *) packet_code) + 6)) = center_v;
      ...
      segment += 1;
      ASM_USE(segment);   /* MATCH pin: load-bearing for the whole function shape */
      {
        s32 u_offset;
        register s32 scaled_offset ASM_REG("$8");   /* MATCH pin: load-bearing for the whole function shape */
        u_offset = *((volatile s32 *) (((volatile u8 *) scratchpad) + 0x10));
        scaled_offset = u_offset * trig_value;
        ASM_USE2_NV(u_offset, scaled_offset);   /* MATCH pin: load-bearing for the whole function shape */
      ...
      ASM_SCHED_BARRIER();   /* MATCH pin: keeps a constant in a register as retail does */
      {
        ...
        ASM_USE2_NV(v_offset, scaled_offset);   /* MATCH pin: load-bearing for the whole function shape */
      ...
      trig_value = func_80064584(angle_step / parameters->count);
      {
        ...
        ASM_USE2_NV(x_offset, scaled_offset);   /* MATCH pin: load-bearing for the whole function shape */
      ...
      trig_value = func_800644B8(angle_step / parameters->count);
      {
        ...
        ASM_USE2_NV(y_offset, scaled_offset);   /* MATCH pin: load-bearing for the whole function shape */
      ...
      *((volatile u16 *) (((volatile u8 *) packet_code) + 7)) = parameters->texture;
      ...
      ASM_KEEP(packet_code);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
  ...
  {
    register u8 *state_slot ASM_REG("$8");   /* MATCH pin: load-bearing for the whole function shape */
    state_slot = D_80083160;
    ASM_KEEP(state_slot);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
