...
void func_80024A5C(volatile s32 draw_data, S_80024A5C_5 *clip_rect, void *screen_pos, u16 clear_rect, s32 draw_param)
{
    ...
    s32 rect_y;
    u8 *clip_packet;
    register u8 *page_ptr ASM_REG("$18");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 saved_param ASM_REG("$23");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    u32 tag_mask;
    u8 *pos_or_ot;
    u8 *order_table;
    void *draw_page;
    S_80024A5C_3 *packet_page;
    register s32 offset_y ASM_REG("$16");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void *call_pos;
    s32 call_data;
