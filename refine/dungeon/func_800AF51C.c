...
void *func_800B4C7C(s32 flags, u8 *source_data, s16 value, u16 callback_mode) {
    ...
    void *callback;
    s16 glyph_or_index;
    register u16 callback_bits ASM_REG("$8");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 callback_kind ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
    s32 number;
    register s32 space ASM_REG("$7");   /* MATCH pin: retail delay-slot fill depends on it */
    register s32 format ASM_REG("$4");   /* MATCH pin: retail keeps a computation the compiler would drop */
    register s32 number_style ASM_REG("$22");   /* MATCH pin: keeps a constant in a register as retail does */
    register s32 text_style ASM_REG("$20");   /* MATCH pin: keeps a constant in a register as retail does */
    void *saved_flags;
    s16 saved_value;
    void *object;
    s8 *text_cursor;
    ...
    void *state;
    void *new_object;
    register void *text_storage ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
    S_800B4C7C_0 *source_object;
    S_800B4C7C_2 *source_position;
    register void *anchor ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */

    text_stack.arg3 = callback_mode;
    new_object = func_8003FD64(0x212, D_80083498);
    saved_flags = (void *)(u32) flags;
    ASM_KEEP(saved_flags);   /* MATCH pin: load-bearing for the whole function shape */
    object = new_object;
    if (object != NULL) {
        saved_value = value;
        ...
        text = text_storage;
        ASM_KEEP(text);   /* MATCH pin: keeps a constant in a register as retail does */
