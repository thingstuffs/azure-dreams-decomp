...
void *func_8015E854(s16 flags, s8 x, s8 y, s16 part_id)
{
    ...
    void *init_obj;
    register void *query_part ASM_REG("$5");   /* MATCH pin: retail delay-slot contents depend on it */
...
default_kind:
        ...
        if (((flags & ~3) << 16) == 0) {
            if (!(work->unk_14 & 0x200)) {
                query_part = part_a;
                ASM_KEEP(init_obj);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
