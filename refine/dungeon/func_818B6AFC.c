...
#else
register s32 implicit_lo ASM_REG("lo");   /* MATCH pin: retail immediate-load split depends on it */
#define M2C_READ_MUL_HI(dst, a, b) \
    do { \
...
void func_818B6AFC(void *effect_data, M2C_UNK render_arg, void *color_out) {
    ...
    s16 frame;
    s16 fade_in_blue;
    register s32 div_adjust ASM_REG("$5");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 fade_work;
    s32 rounded_color;
    register s32 hold_bits ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    s32 channel_bit;
    register s32 fade_out_red ASM_REG("$10");   /* MATCH pin: load-bearing for the whole function shape */
    s32 fade_in_half;
    s32 fade_out_half;
    s32 packed_channel;
    s32 fade_out_remainder;
    register s32 cycle_quotient ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    s32 packed_color;
    s32 end_frame;
    s32 frames_left;
    s32 div_seven_magic;
    ...
    u16 hold_tick;
    u16 fade_out_tick;
    register s32 product_result ASM_REG("$12");   /* MATCH pin: retail register colouring depends on it */
    s8 red;
    s8 green;
    s8 blue;
    void *fade_in_target;
    ...
    if (frame < 8) {
        ...
        ASM_KEEP_NV(cycle_quotient);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_KEEP(product_result);   /* MATCH pin: retail register colouring depends on it */
    ...
    if (frame < 0x30) {
        ...
        ASM_KEEP_NV(fade_work);   /* MATCH pin: keeps a statement from moving across a call/branch */
        end_frame = 0x30;
        ASM_KEEP_NV(end_frame);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
