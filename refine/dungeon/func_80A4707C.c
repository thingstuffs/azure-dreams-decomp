...
void *func_8017087C(s16 kind_flags, s8 tile_x, s8 tile_y, s16 part_id)
{
    ...
    s32 entry_id_fixed;
    void *part_b;
    register void *work ASM_REG("$20");   /* MATCH pin: retail address form (%hi/%lo vs entry_id_fixed+offset) depends on it */
    register s8 saved_x ASM_REG("$19");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s16 saved_part_id;
    register s8 saved_y ASM_REG("$18");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 setup_value;
    register s32 work_flags;
    register void *alloc_or_entry ASM_REG("$4");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 entry_id ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register s32 entry_mode ASM_REG("$6");   /* MATCH pin: retail keeps a computation the compiler would drop */
    s32 child_index;
    u8 *slot;
    s32 entry_index;
    ...
    ASM_USE_NV(alloc_or_entry);   /* MATCH pin: retail schedule: same instructions, different order without it */
    saved_x = tile_x;
...
init_parts:
        ...
        do {
            ...
            if (child_obj != 0) {
                register void *child_work ASM_REG("$17") = (u8 *)child_obj + 0x20;   /* MATCH pin: retail register colouring depends on it */
                ...
                if (((S_8017087C_6 *)child_work)->unk_02 > 0) {
                    register s32 entry_offset ASM_REG("$19");   /* MATCH pin: retail schedule: same instructions, different order without it */
                    s32 child_id_fixed;
                    ...
                    do {
                        ...
                        ASM_USE(entry_mode);   /* MATCH pin: retail schedule: same instructions, different order without it */
                        entry_offset += 48;
