...
void func_8017516C(u8 *owner_data, Position *position_arg, Source *source_arg, Context *context_arg) {
    ...
    u16 hit;
    s32 direction;
    register s32 trial_dir ASM_REG("$16");   /* MATCH pin: retail register colouring depends on it */
    s32 target_y;
    s32 target_x;
    Object *object;
    Display *display;
    ...
    s32 offset_y_index;
    u32 initial_result;
    register u8 *table_or_owner ASM_REG("$9");   /* MATCH pin: retail register colouring depends on it */
    Position *position;
    Source *source;
    Context *context;
    ...
    ASM_KEEP_NV(position);   /* MATCH pin: retail schedule: same instructions, different order without it */
    target_y = target_x = 0;
...

    {
        ...
        register u16 *trial_x_step ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
        u16 *first_x_step;
        u16 *first_y_step;
        s32 trial_result;
        ...
        x_steps = D_8006CCD8;
        {
            register s32 direction_offset ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */

            ASM_SET(direction_offset);   /* MATCH pin: retail immediate-load split depends on it */
...
search_done:
        ...
        ASM_USE_NV(trial_dir);   /* MATCH pin: load-bearing for the whole function shape */
        goto allocate;

trial_success:
        {
            register s32 trial_offset ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
            s32 trial_y_address;
            ...
            ASM_KEEP(trial_x_step);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
