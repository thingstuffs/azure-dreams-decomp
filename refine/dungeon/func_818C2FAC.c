...
s32 func_818C2FAC(void *owner, void *source_position, s32 direction)
{
    ...
    s32 object_type;
    register s32 offset_angle ASM_REG("$17");   /* MATCH pin: retail keeps a computation the compiler would drop */
    register uptr effect ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register uptr render_or_radius ASM_REG("$16");   /* MATCH pin: retail delay-slot fill depends on it */
    register s32 render_arg_low ASM_REG("$4");   /* MATCH pin: retail delay-slot contents depend on it */
    register u32 page_base_low ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    s32 render_arg_high;
    register u32 page_base_high ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    s32 scaled_radius;
    s32 divisor_reciprocal;
    S_818C2FAC_1 *effect_state;

    object_type = 0x212;
    ASM_KEEP_NV(object_type);   /* MATCH pin: retail schedule: same instructions, different order without it */
    offset_angle = direction;
    effect = (uptr)func_8003FC64(object_type);
    ASM_KEEP(position_addr);   /* MATCH pin: retail schedule: same instructions, different order without it */
    if (effect != NULL) {
...
#ifndef NON_MATCHING
        ASM_KEEP(render_arg_low);   /* MATCH pin: retail basic-block layout depends on it */
        page_base_low = 0x800E0000;
        ASM_PAGEBASE_PIN(page_base_low);   /* MATCH pin: retail delay-slot contents depend on it */
        func_800248CC();
#else
...
#ifndef NON_MATCHING
        ASM_KEEP(render_arg_high);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        page_base_high = 0x800E0000;
        ASM_PAGEBASE_PIN(page_base_high);   /* MATCH pin: retail delay-slot contents depend on it */
        func_800248CC();
#else
        func_800248CC((void *)render_or_radius, 0x800E0000);
#endif
    case0:
        {
            ...
            ASM_TAILSLOT_PIN(appearance_id);   /* MATCH pin: retail delay-slot contents depend on it */
            func_800248C0();
        }
    case1:
        {
    ...
    case3:
        ...
        ASM_USE_NV(divisor_reciprocal);   /* MATCH pin: retail immediate-load split depends on it */
        ...
        {
            register s32 effect_result ASM_REG("$2");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
            ...
            ASM_KEEP(effect_result);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
