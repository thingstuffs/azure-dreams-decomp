...
void func_80175594(S_80175594_0 *sprite, Rec_func_800D6DC0_arg1 *position, Rec_func_800D6DC0_arg2 *render, s16 depth_bias)
{
    ...
    if ((u32)((S_80175594_1 *)scratch)->unk_C0 < 0x1E0U) {
        ...
        for (;;) {
            ...
            {
                register void *vertex_1 ASM_REG("$5") = scratch + 0x78;   /* MATCH pin: retail schedule: same instructions, different order without it */
                register void *vertex_2 ASM_REG("$6") = scratch + 0x80;   /* MATCH pin: retail schedule: same instructions, different order without it */
                register void *vertex_3 ASM_REG("$7") = scratch + 0x88;   /* MATCH pin: retail schedule: same instructions, different order without it */
                register void *screen_0 ASM_REG("$8");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
                s32 bottom_y;
                register s32 bottom_scale ASM_REG("$3");   /* MATCH pin: retail delay-slot fill depends on it */

                bottom_y = ((S_80175594_1 *)scratch)->unk_72.u;
                ASM_KEEP(bottom_y);   /* MATCH pin: retail delay-slot fill depends on it */
                screen_0 = scratch + 0xF0;
                ASM_KEEP_DEP_NV(bottom_y, screen_0);   /* MATCH pin: retail delay-slot fill depends on it */
