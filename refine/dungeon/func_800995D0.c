    ...
    if (!(D_800E296C & 0x80)) {
        ...
        if (object != NULL) {
            ...
            func_8004491C(object, &D_8009E798);
            {
                u16 field_value;
                void *object_state;
                register void *child ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
                register s32 color ASM_REG("$7");   /* MATCH pin: retail register colouring depends on it */
                void *config_value;
                register void *render_data ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
