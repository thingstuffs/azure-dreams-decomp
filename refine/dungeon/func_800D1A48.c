...
void *func_800D71A8(u8 *sprite_list, s32 count_hint, s32 render_state_addr, u8 *placement, u8 *packet_buf)
{
  ...
  register s32 index ASM_REG("$22");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
  register s32 count_or_x;
  s32 state_addr;
  register s32 render_state;
  register u8 *sprite_placement ASM_REG("$21");   /* MATCH pin: retail schedule: same instructions, different order without it */
  u8 *sprite;
  register u8 *packet ASM_REG("$23");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
  u8 *packet_data;
  u8 *quad_data;
  u8 *part;
  register u8 *quad ASM_REG("$20");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
  s32 bottom_y;
  ...
  ASM_KEEP_NV(render_state);   /* MATCH pin: retail schedule: same instructions, different order without it */
  sprite_placement = placement;
  ASM_KEEP_NV(sprite_placement);   /* MATCH pin: retail schedule: same instructions, different order without it */
  sprite = 0;
  index = (u32) sprite;
  packet = packet_buf;
        ...
        *((s16 *) (((u8 *) packet_data) + 0x1E)) = bottom_y;
        {
          ...
          if (clip_test != 0)
          {
            ...
            *((s16 *) (((u8 *) packet_data) + 0x18)) = (*((u16 *) (((u8 *) scratch) + 0x14))) + (*((u16 *) (((u8 *) scratch) + 0x08)));
            {
              s32 bottom_right_uv;
              register s32 right_x ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
            ...
            {
              register u16 part_flags ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
              register u8 flags;
              ...
              *((u8 *) (((u8 *) part) + 0xF)) = flags;
              if (part_flags & 8)
              {
                ...
                *((u8 *) (((u8 *) part) + 0xF)) = draw_flags;
                ASM_KEEP(part_flags);   /* MATCH pin: load-bearing for the whole function shape */
              }
            }
            {
              u8 flags;
            ...
            *((s32 *) (((u8 *) packet_data) + 0)) = *((s32 *) (((u8 *) part) + 0xC));
            if (!((*((u16 *) (((u8 *) sprite) + 4))) & 0x8000))
            {
              ...
              {
                u32 *ot_entry;
                register u32 packet_tag ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
                u32 ot_tag;
                ot_entry = (u32 *) ((depth << 2) + ((u32) ordering_table));
                ASM_KEEP(ot_entry);   /* MATCH pin: keeps a statement from moving across a call/branch */
