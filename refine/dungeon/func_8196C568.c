...
/* Updates a staged particle effect, spawning trails and bursts before cleanup. */
void func_8196C568(Work *work, s32 effect_arg, s32 aux_arg)
{
    OffsetTable offsets;
    Effect *effect;
    Sprite *sprite;
...
init:
    dungeon_page = (u8 *)0x80080000;
    ASM_KEEP_NV(dungeon_page);
    dungeon = *(DungeonState **)(dungeon_page + 0x14A8);
    init_timer = 33;
    ...
    *(s16 *)(effect_page + 0x69B4) = 1;
    ...
    active_work->timer = 45;
    dungeon_count = *(u16 *)&dungeon->pad0[0xA6];
    saved_effect_arg = effect_arg;
    dungeon_count--;
    *(u16 *)&dungeon->pad0[0xA6] = dungeon_count;
    object_kind = active_work->kind;
    saved_aux_arg = aux_arg;
    dungeon->pad0[0xA8] = object_kind;
    dungeon = *(DungeonState **)(active_page + 0x14A8);
    direction_index = ((*(u16 *)&dungeon->pad0[0x2A]) >> 9) & 7;
    active_work->state++;
    active_work->counter = 0;
    active_work->table_index = direction_index;
    func_8002553C(active_work, saved_effect_arg, saved_aux_arg);
    func_800A56E0(0x300);
    func_800542BC();
    return;
        ...
        if (func_8003DF74(search->field8, search, &work->x, 0)) {
            ...
            if (work->timer < 7) {
                for (particle_index = 0; particle_index < 2; particle_index++) {
                    func_80024AF8(work, effect_arg, aux_arg, work->x, work->y, work->z);
                }
            }
            if (work->timer < 3) {
                particle_index = 0;
                ...
                for (; particle_index < 2; particle_index++) {
                    func_80024AF8(work, effect_arg, aux_arg,
        ...
        if (work->timer >= 13) {
            ...
            if (func_8003DF74(search->field8, search, &work->x, 0)) {
                for (particle_index = 0; particle_index < 4; particle_index++) {
                    func_80024CA4(work, effect_arg, aux_arg, work->x, work->y, work->z);
                }
                for (particle_index = 0; particle_index < 4; particle_index++) {
                    func_80024F80(work, effect_arg, aux_arg, work->x, work->y, work->z);
                }
                effect_arg = work->timer;
                if (effect_arg == 16) {
                    effect = func_8003FC64(0x212);
                    if (effect != 0) {
