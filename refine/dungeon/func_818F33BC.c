...
void func_80024BBC(
    S_80024BBC_1 *source, s16 field_14_value, void *payload, s16 field_32_value,
    s32 x_offset, s32 y_offset, s32 z_offset)
{
    register s32 held_x_offset ASM_REG("$18") = x_offset;   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 held_y_offset ASM_REG("$19") = y_offset;   /* MATCH pin: retail address form (%hi/%lo vs state+offset) depends on it */
    register s32 held_z_offset ASM_REG("$20") = z_offset;   /* MATCH pin: retail address form (%hi/%lo vs state+offset) depends on it */
    void *object;
    S_80024BBC_5 *state;
    ...
    if (object != NULL) {
        ...
        x_jitter = rand() & 0x1F;
        x_pos = ((S_80024BBC_6 *)(source->unk_08))->unk_02;
        ...
        x_dest->unk_02 = x_pos;
        y_jitter = rand() & 0x1F;
        y_pos = ((S_80024BBC_6 *)(source->unk_08))->unk_06;
        ...
        z_jitter = rand() & 0x1F;
        z_pos = ((S_80024BBC_6 *)(source->unk_08))->unk_0A;
        ...
        ASM_KEEP(held_x_offset);   /* MATCH pin: retail address form (%hi/%lo vs state+offset) depends on it */
        ASM_KEEP(held_y_offset);   /* MATCH pin: keeps a statement from moving across a call/branch */
        ASM_KEEP(held_z_offset);   /* MATCH pin: load-bearing for the whole function shape */
    }
}
