...
have_slot:
    if (*item_slot != 0) {
        ...
        ASM_KEEP(main_link_test);   /* MATCH pin: retail basic-block layout depends on it */
        if (main_link_test) {
...
copy_link:
                {
                    register u8 *linked_actor ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
