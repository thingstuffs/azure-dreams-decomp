...
void func_8009D3B0(void) {
    UA64 map_params;
    s16 neighbor_x;
    register s32 x_limit ASM_REG("$14");   /* MATCH pin: retail register colouring depends on it */
    register s32 x_limit_shifted ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s16 next_coord;
    s32 tile_value;
    s16 x;
    s16 x_end_or_neighbor;
    register s16 y ASM_REG("$10");   /* MATCH pin: load-bearing for the whole function shape */
    s16 next_x;
    s32 y_limit;
    ...
    register s16 y_end ASM_REG("$15");   /* MATCH pin: retail register colouring depends on it */
    u8 *tile_or_map;
    u8 *grid_config;
    register s32 tile_index_or_level ASM_REG("$3");   /* MATCH pin: keeps a constant in a register as retail does */
    register u8 *status_or_level ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s8 region_index;
    ...
    if (region_index >= 0) {
        register u8 *region ASM_REG("$11");   /* MATCH pin: load-bearing for the whole function shape */
        u8 *grid;
        u8 *map;
        s32 row_y;
        s32 row_x_end;
        ...
        register s32 packed_levels ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
...
scan_row:
            ...
            if (next_coord < x_limit) {
                ASM_KEEP(y);   /* MATCH pin: retail keeps a computation the compiler would drop */
...
scan_tile:
                ...
                if (tile_value != 0) {
                    ASM_KEEP(x);   /* MATCH pin: retail keeps a computation the compiler would drop */
                    excluded_tile_type = 3;
                    if (tile_value != excluded_tile_type) {
                        ASM_KEEP(tile_index_or_level);   /* MATCH pin: retail keeps a computation the compiler would drop */
                        ...
                        ASM_KEEP(status_or_level);   /* MATCH pin: retail register colouring depends on it */
...
advance_y:
        ...
        u8 *position;
        register s32 neighbor_packed_levels ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
...
scan_neighbor:
        ...
        if (neighbor_value != 0) {
            ASM_KEEP(neighbor_x);   /* MATCH pin: retail keeps a computation the compiler would drop */
