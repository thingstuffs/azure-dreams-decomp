...
{
  u8 *effect;
  register s32 scale ASM_REG("$20");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
  register s32 step;
  u32 addr_mask;
  register s32 angle ASM_REG("$23");   /* MATCH pin: retail immediate-load split depends on it */
  register u8 *scratch ASM_REG("$18");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
  u8 *render_global;
  u8 *line_packet;
  ...
  *((void **) (((u8 *) scratch) + 0x18)) = (*((u8 **) D_80083160)) + 0xB0;
  do
  {
    ...
    register s32 radial_offset ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    *((s32 *) (((u8 *) scratch) + 0x108)) = radial_offset;
    ASM_KEEP_NV(radial_offset);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
