...
void func_8187B9E8(s32 radius, s32 initial_value, s16 extent, u16 position_x, U16Arg position_y, U16Arg position_z) {
    register s32 held_radius ASM_REG("$23") = radius;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u16 position_xyz[3];
    s32 point_count;
    s32 angle_a;
    s32 angle_b;
    s32 plane_radius;
    register s32 component ASM_REG("$2");   /* MATCH pin: keeps angle_a statement from moving across angle_a call/branch */
    register s32 work_value ASM_REG("$8");   /* MATCH pin: retail register colouring depends on it */
    void *effect;
    S_8187B9E8_4 *state;
    S_8187B9E8_2 *position;
    S_8187B9E8_3 *scale;
    register u8 *point ASM_REG("$19");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (effect != NULL) {
        ...
        ASM_KEEP(work_value);   /* MATCH pin: retail register colouring depends on it */
