...
void func_818F9E48(void *state, void *position_out, void *effect_arg) {
    ...
    origin = D_8002400C;
    ASM_KEEP(position);   /* MATCH pin: retail keeps a owner_position the compiler would otherwise drop/add */
    ASM_KEEP(effect);   /* MATCH pin: retail keeps a owner_position the compiler would otherwise drop/add */
    D_800266BC = 1;
    ...
    if (frame_or_phase < 0x15) {
        ...
        ASM_TAILSLOT_PIN(fade_or_phase);   /* MATCH pin: retail keeps a owner_position the compiler would otherwise drop/add */
        func_80025758();
        return;
    }
