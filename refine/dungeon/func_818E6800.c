...
void BODY_NAME(void *effect_arg, void *motion_arg, void * volatile context_arg) {
    ...
    register u8 *motion ASM_REG("$22") = (u8 *)motion_arg;
    register u8 *actor ASM_REG("$20");
    u8 *actor_data;
    u8 *actor_header;
    u8 *particle;
    u8 *render_data;
    register u8 *particle_data ASM_REG("$16");
    ...
    if (state > 0) {
        ...
        return;
state_1: {
            register s32 frames_left ASM_REG("$2");
...
mode_0: {
            ...
            if ((U16(S32(effect, 4), 0) & 0x80) != 0) {
                ...
                if ((U16(render_data, 20) & 0x8000) != 0) {
                    register u8 *context_data ASM_REG("$8") = (u8 *)context_arg;
...
mode_1: {
            ...
            if (S16(effect, 80) <= 0 && S16(effect, 82) == 7) {
                register s32 hold_frames ASM_REG("$3") = 8;
                s32 current_state = U16(effect, 10);
                ASM_KEEP_DEP_NV(hold_frames, current_state);
