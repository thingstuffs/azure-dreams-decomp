...
void func_800C4E00(void *state)
{
    s32 countdown;
    s32 entry_index;
    register u32 addr_or_flags ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    ...
    if (D_80083462 & 0x10) {
        ...
        if ((countdown << 0x10) <= 0) {
            if (((S_800C4E00_0 *)state)->unk_0A == 1) {
                ...
                ASM_KEEP(entry_index);   /* MATCH pin: retail immediate-load split depends on it */
