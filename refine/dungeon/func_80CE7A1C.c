...
void func_8017121C(void *source_handle, S_8017121C_4 *origin, s32 unused, S_8017121C_7 *target)
{
    Vec3u16 offset;
    void *effect;
    void *init_effect;
    void *init_data;
    S_8017121C_3 *coords;
    register u8 *effect_state ASM_REG("$18");   /* MATCH pin: retail register colouring depends on it */
    S_8017121C_6 *source;
    S_8017121C_2 *part;
    S_8017121C_9 *target_coords;
    register s16 *direction_table ASM_REG("$5");   /* MATCH pin: retail immediate-load split depends on it */
    register s32 coord_delta ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    u32 untracked_mask;
    u32 target_flags;
    u32 x_table_addr;
    register void *target_handle ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s32 effect_x;
    s16 *x_entry;
    s32 x_offset;
    s32 target_x;
    ...
    init_effect = effect;
    ASM_KEEP_NV(init_effect);   /* MATCH pin: retail schedule: same instructions, different order without it */
    init_data = D_80170CEC;
    ASM_KEEP(init_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if (target->unk_60 == NULL) {
        goto no_tracked;
    }
    target_flags = target->unk_14;
    ...
    if ((target_flags & untracked_mask) == 0) {
        ...
        x_table_addr = target->unk_2A;
        ASM_KEEP_NV(x_table_addr);   /* MATCH pin: retail schedule: same instructions, different order without it */
        target_handle = target->unk_60;
        ASM_KEEP_NV(target_handle);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
        effect_x = coords->unk_02.u;
        ASM_KEEP_NV(effect_x);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        y_table_addr = target->unk_2A;
        ASM_KEEP_NV(y_table_addr);   /* MATCH pin: load-bearing for the whole function shape */
        target_y = target_coords->unk_06;
        ASM_KEEP_NV(target_y);   /* MATCH pin: retail basic-block layout depends on it */
        effect_y = coords->unk_06.u;
        ASM_KEEP_NV(effect_y);   /* MATCH pin: load-bearing for the whole function shape */
        y_table_addr = (y_table_addr >> 7) & 0x1C;
        y_table_addr += (u32)direction_table;
        y_entry = (s16 *)y_table_addr;
        y_offset = y_entry[1];
        ASM_KEEP_NV(y_offset);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        coord_delta -= D_800DDC40[((S_8017121C_10 *)(target->unk_60))->unk_13] >> 1;
...
no_tracked:
    ...
    grid_axis = target->unk_73;
    grid_x_bits = target->unk_72;
    ASM_KEEP(grid_axis);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    grid_axis <<= 24;
    grid_y = grid_axis >> 24;
    grid_x_bits <<= 24;
    ...
    ((S_8017121C_0 *)effect_state)->unk_5A = coord_delta / 2;
    ASM_KEEP(effect_state);   /* MATCH pin: keeps a statement from moving across a call/branch */
    scaled_y = grid_y << 16;
