...
s32 func_800AFFB4(void *origin, void *unused, void *render_data_in, u8 *packet_buffer, volatile ShortArg texture_arg) {
    s32 depth_step;
    register s32 shade_uv ASM_REG("$4");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 raw_height;
    register s32 shade_u ASM_REG("$5");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s32 near_shade;
    s32 texture_height;
    register s32 coord_bits ASM_REG("$2");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    register s32 vertex_value ASM_REG("$3");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 x_in_bounds;
    s32 near_mid_x;
    s32 texture_right;
    s32 far_mid_x;
    ...
    s32 visible_left;
    s32 visible_near;
    register void *render_data ASM_REG("$17");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 visible_strips;
    TwelveByteEntry *texture_entry;
    s32 quad_index;
    register u8 *packet ASM_REG("$22");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    s32 strip_index;
    s32 packet_end;
    s32 visible_right;
    u16 screen_y;
    ...
    u16 shade_offset;
    u32 bucket_index;
    register u8 *uv_end ASM_REG("$16");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    u8 *far_vertex;
    register u8 *texture_info ASM_REG("$19");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
...
next_strip:
    ...
    if (func_80065610((u8 *)render_data + 0x70, (u8 *)render_data + 0x78,
        ...
        if ((visible_left | visible_right) != 0) {
            ...
            ASM_KEEP(vertex_value);   /* MATCH pin: load-bearing for the whole function shape */
            vertex_value <<= 1;
            ...
            do {
                ...
                ASM_KEEP(coord_bits);   /* MATCH pin: load-bearing for the whole function shape */
                coord_bits += vertex_value;
