...
DungeonObject *func_800C7380(s16 tile_x, s32 tile_y, u16 z, s32 height, s32 speed) {
    ...
    DungeonObject *child;
    DungeonState *state;
    register DungeonCoord *center ASM_REG("$19");   /* MATCH pin: retail register colouring depends on it */
    ...
    s32 remaining;
    s32 angle;
    register u16 saved_z ASM_REG("$16");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register u32 setup_word ASM_REG("$8");   /* MATCH pin: keeps a constant in a register as retail does */

    saved_z = z;
    ASM_KEEP_NV(saved_z);   /* MATCH pin: retail schedule: same instructions, different order without it */
    homes.arg1 = tile_y;
    setup_word = speed;
    ...
    if (parent != NULL) {
        ...
        ASM_USE(setup_word);   /* MATCH pin: keeps a statement from moving across a call/branch */
        ...
        ASM_USE(setup_word);   /* MATCH pin: keeps a statement from moving across a call/branch */
        state_limit = 0x14;
        ASM_USE(state_limit);   /* MATCH pin: keeps a statement from moving across a call/branch */
        ...
        do {
            ...
            if (child != NULL) {
                register DungeonObject *init_child ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
                register void *child_resource ASM_REG("$5");   /* MATCH pin: retail schedule: same instructions, different order without it */
                init_child = child;
                child_resource = D_800D0774;
                setup_word = (u32) D_800BB14C;
                ASM_KEEP(setup_word);   /* MATCH pin: retail schedule: same instructions, different order without it */
