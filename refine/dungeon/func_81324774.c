...
void func_8016BF74(void *raw_motion, void *context, void *raw_position, void *raw_actor) {
    ...
    void *actor;
    register s32 turn_index ASM_REG("$19");
    s32 near_target;
    register u8 *x_offsets ASM_REG("$23");
    M2C_UNK old_tile_mask;
    M2C_UNK new_tile_mask;
    register s8 *angle_offsets ASM_REG("$17");
    register s8 *offset_page ASM_REG("$2");
    s32 target_y;
    s32 target_x;
    s32 y_offset;
    s32 x_offset;
    ...
    s16 target_angle;
    s16 actor_distance;
    register s32 tile_x_or_offset ASM_REG("$4");
    u8 *x_offset_ptr;
    s32 move_value;
    register s32 initialized_bit ASM_REG("$3");
    s32 actor_flags;
    ...
    s8 *state;
    s32 state_flags;
    register void *target_entry ASM_REG("$3");
    S_8016BF74_4 *target_position;
    void *ahead_target;
    register void *nearby_target ASM_REG("$3");
    s32 active_actor;
    s32 path_status;
    s32 tile_y;
    s32 current_angle;
    ...
    s32 dest_x;
    s32 dest_y;
    register s32 old_tile_flags ASM_REG("$3");
    s32 new_tile_flags;
    s32 step_y;
    s32 next_x;
    s32 current_x;
    register s32 next_path_index ASM_REG("$2");
    register s32 current_y ASM_REG("$2");
    register u8 *y_offsets ASM_REG("$2");
    u8 *y_offset_ptr;
    u8 *world_position;
