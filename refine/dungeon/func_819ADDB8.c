...
void *func_800255B8(s32 x, s32 y, s16 z, u16 angle) {
    void *objects[12];
    register s32 origin_x ASM_REG("$21");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s32 origin_y ASM_REG("$22");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s16 origin_z ASM_REG("$23");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u16 heading;
    s32 piece_index;
    void **object_base;
    register void **object_slot ASM_REG("$16");   /* MATCH pin: load-bearing for the whole function shape */
    s16 *x_offsets;
    register u8 *color_table;
    u8 *effect_state;
    register u8 *component_data ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    u8 *sprite;
    u8 color_first;
    u32 color_second;
    u32 offset_addr;
    ...
    origin_z = z;
    heading = angle;
    ASM_KEEP4_NV(origin_x, origin_y, origin_z, heading);   /* MATCH pin: retail schedule: same instructions, different order without it */
...
#else
    {
        ...
        ASM_KEEP_NV(page_base);   /* MATCH pin: load-bearing for the whole function shape */
...
loop:
        {
            ...
            if (piece_index != 0) {
                ...
                ASM_KEEP_NV(template_page);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
...
#endif
            ASM_KEEP_NV(resource_page);   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_SCHED_BARRIER();   /* MATCH pin: retail immediate-load split depends on it */
        {
            ...
            ASM_KEEP_NV(y_offsets);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        ...
        ASM_SCHED_BARRIER();   /* MATCH pin: retail immediate-load split depends on it */
        {
            ...
            ASM_KEEP_NV(sprite_table);   /* MATCH pin: load-bearing for the whole function shape */
            sprite_table += 0x74C0;
#endif
        ...
        if (piece_index != 0) {
            register u32 link_value ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
            register u8 *link_entry ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */

            link_value = (u32)D_80027580;
            link_entry = (u8 *)(piece_index + link_value);
            ASM_KEEP_NV(link_entry);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            link_value = *link_entry;
            ASM_KEEP_NV(link_value);   /* MATCH pin: load-bearing for the whole function shape */
            if (link_value != 0) {
