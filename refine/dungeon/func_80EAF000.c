...
BODY_STORAGE s32 BODY_NAME(void *origin, void *actor) {
    ...
    if (ground_slot >= 0) {
        ...
        ASM_KEEP(transfer_base);   /* MATCH pin: keeps a statement from moving across a call/branch */
        item_word = D_800E3548[ground_slot];
        ASM_SCHED_BARRIER();   /* MATCH pin: keeps a statement from moving across a call/branch */
        transfer_slot = transfer_base + 1017;
        ASM_KEEP(transfer_slot);   /* MATCH pin: load-bearing for the whole function shape */
    ...
    if (target == (void *)D_800E3D7C[0]) {
        ...
        {
#ifdef __mips__
            register s32 slot_index ASM_REG("$5");   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            s16 item_count;
            register s32 *inventory_scan ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
#else
            s32 slot_index;
            s16 item_count;
            s32 *inventory_scan;
            ...
            }
            {
                ...
                s32 *transfer_base;
#ifdef __mips__
                register s32 transfer_addr ASM_REG("$2");   /* MATCH pin: retail delay-slot fill depends on it */
#else
                s32 transfer_addr;
#endif
                ...
                ASM_TAILSLOT_PIN(transfer_addr);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                func_8014CADC();
                __builtin_unreachable();
            }
        }
...
#ifdef __mips__
        s32 *transfer_base;
        register s32 *transfer_slot ASM_REG("$2");   /* MATCH pin: retail delay-slot fill depends on it */
#else
        s32 *transfer_base;
        s32 *transfer_slot;
#endif
