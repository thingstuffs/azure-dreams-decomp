...
void *func_80026ED0(s16 x, s16 y, s16 z, s16 size) {
    ...
    register s16 saved_y ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s16 saved_z ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register s16 saved_size ASM_REG("$20");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ...
    ASM_KEEP(saved_size);   /* MATCH pin: retail schedule: same instructions, different order without it */
    if (object != NULL) {
