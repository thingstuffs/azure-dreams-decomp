...
s32 BODY_NAME(void *object_data, void *position_data) {
    ...
    ASM_KEEP_NV(render_slot);   /* MATCH pin: keeps a statement from moving across a call/branch */
    ASM_KEEP_DEP_NV(addr_mask, render_ctx);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    addr_mask |= 0xFFFF;
