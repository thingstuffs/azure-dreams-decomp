...
void func_80174428(void *state, void *motion, void *actor, void *object)
{
    ...
    s32 y_step;
    s32 x_step;
    register s16 *x_step_ptr ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    u32 direction_offset;
    register u32 raw_direction ASM_REG("$2");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s16 height;
    register s32 search_result ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    register s32 height_valid ASM_REG("$2");   /* MATCH pin: keeps a statement from moving across a call/branch */
    register s32 attempts_left ASM_REG("$17");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    u8 *world;
    s16 *level;
...
case_4:
    ...
    ASM_KEEP(attempts_left);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    attempts_left--;
case_4_check:
    if (attempts_left <= 0) {
        goto case_3;
