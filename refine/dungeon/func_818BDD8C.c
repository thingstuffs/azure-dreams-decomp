...
void *func_818BDD8C(S_818BDD8C_2 *owner, S_818BDD8C_4 *initial_data)
{
    ...
    s32 copy_word_3;
    s32 draw_flags;
    register s32 render_flags ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    S_818BDD8C_3 *render;
    void *object;
    S_818BDD8C_1 *state;
    ...
    if (object != NULL) {
        ...
        {
            register void *pinned_object ASM_REG("$2");   /* MATCH pin: retail keeps a computation the compiler would drop */
            pinned_object = object;
            ASM_KEEP(pinned_object);   /* MATCH pin: retail keeps a computation the compiler would drop */
        }
