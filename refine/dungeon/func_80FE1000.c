...
void *BODY_NAME(s16 kind_flags, s8 part_value_24, s8 part_value_25, s16 part_value_0a)
{
    ...
    s32 flags;
    s32 paired_flags;
    register s8 saved_value_24 ASM_REG("$22");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s16 saved_value_0a;
    register s8 saved_value_25 ASM_REG("$21");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s16 init_kind_flags;
    register void *init_obj ASM_REG("$4");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void *init_part_a;
    ...
    if (obj != 0) {
        ...
        ASM_KEEP(init_kind_flags);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
        work = (u8 *)obj + 0x20;
