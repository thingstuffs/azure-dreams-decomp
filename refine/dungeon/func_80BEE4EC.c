...
void func_80173CEC(S_80173CEC_0 *state, Rec_D_800E3D7C *position, Rec_D_80082E80 *actor, void *model) {
    ...
    s32 color;
    s32 variant_index;
    register s32 effect_value ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    u16 fade_left;
    u16 elapsed;
    u16 fade_timer;
    s32 event_done;
    ...
    register S_80173CEC_7 *effect_globals ASM_REG("$5");   /* MATCH pin: retail delay-slot fill depends on it */
    register S_80173CEC_8 *effect_position ASM_REG("$6");   /* MATCH pin: retail schedule: same instructions, different order without it */
...
jt_c4:
    ...
    color = 0x808080;
    ASM_KEEP(color);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    state->unk_AC = effect;
    effect_model = model;
    ((S_80173CEC_12 *)(((S_80173CEC_11 *)effect)->unk_08))->unk_00 = (s32) position->unk_00.at00_s32.v;
    ASM_KEEP(effect_model);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    ASM_KEEP(variant_index);   /* MATCH pin: retail delay-slot fill depends on it */
