...
void BODY_NAME(void) {
    ...
    *(s16 *)(view_state + 0x68) = 0;
    ...
    ASM_KEEP_NV(light_matrix);   /* MATCH pin: retail schedule: same instructions, different order without it */
    light_coeff_a = -0x800;
    light_coeff_b = 0x800;
    *(s16 *)(view_state + 0x38) = light_coeff_a;
    *(s16 *)(view_state + 0x3C) = light_coeff_a;
    ASM_KEEP_NV(light_coeff_b);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    light_coeff_a = 0x800;
