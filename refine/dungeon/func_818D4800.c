...
s32 BODY_NAME(void *object_data, void *position_data)
{
    ...
    ASM_KEEP(position);   /* MATCH pin: keeps a statement from moving across a call/branch */
#ifdef __mips__
    {
        register void *scratch_arg ASM_REG("$4") = scratch;   /* MATCH pin: retail delay-slot contents depend on it */
        void *next_object_arg = next_object;

        ASM_TAILSLOT_PIN(scratch_arg);   /* MATCH pin: retail delay-slot contents depend on it */
        func_8002409C();
    }
#else
