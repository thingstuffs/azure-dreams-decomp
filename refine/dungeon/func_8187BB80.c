...
void func_8187BB80(void *effect_data, s32 unused_arg, void *color_data) {
    ...
    ASM_KEEP(colors);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    D_8002694C = 1;
    ((S_8187BB80_0 *)effect)->unk_02.u--;

    if (((S_8187BB80_0 *)effect)->unk_40.at00.v == 0) {
        s32 mid_y;
        register u32 mid_x ASM_REG("$2");   /* MATCH pin: retail basic-block layout depends on it */
    ...
    if (*(volatile s32 *)(effect + 0x40) == 0x10000) {
        ...
        ASM_KEEP(start_x);   /* MATCH pin: retail delay-slot fill depends on it */
        ASM_KEEP(start_y);   /* MATCH pin: retail schedule: same instructions, different order without it */
...
jt_c1: {
            ...
            s32 next_phase = 2;
            ASM_KEEP(phase_value);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            ASM_TAILSLOT_PIN(next_phase);   /* MATCH pin: retail delay-slot contents depend on it */
