...
{
    s32 velocity;
    register s32 next_velocity ASM_REG("$3");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 position;

    velocity = ((S_800220A8_1 *)motion)->unk_14;
    next_velocity = velocity;
    ASM_KEEP_NV(next_velocity);   /* MATCH pin: keeps a statement from moving across a call/branch */
...
decelerate:
{
    register s32 position_or_velocity ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    s32 velocity_step;
...
withdraw:
{
    register s32 position_or_flags ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    register s32 exit_check ASM_REG("$3");   /* MATCH pin: retail schedule: same instructions, different order without it */
