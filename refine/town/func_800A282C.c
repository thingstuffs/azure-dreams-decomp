...
s32 func_8009FF8C(s32 record_set, void *target)
{
    ...
    u8 *flag_base;
    register u8 *target_pos ASM_REG("$23");   /* MATCH pin: retail immediate-load split depends on it */
    s32 count;
    s32 best_distance;
    register s32 best_index ASM_REG("$19");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    s32 record_index;
    s32 base_x;
    s32 base_y;
    s32 distance;
    ...
    target_pos = target;
    ASM_KEEP_NV(target_pos);   /* MATCH pin: retail schedule: same instructions, different order without it */
