...
void func_80026978(S_80026978_1 *origin)
{
    ...
    if (effect != NULL) {
        ...
        ASM_KEEP(random_delta);   /* MATCH pin: retail schedule: same instructions, different order without it */
        effect_state = (u8 *)effect + 0x20;
        transform->unk_14 = random_delta;
        ASM_KEEP(effect_state);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
