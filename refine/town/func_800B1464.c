...
void func_800AEBC4(void *vector_data, S_800AEBC4_4 *config, S_800AEBC4_1 *position_data, S_800AEBC4_3 *secondary_data,
    ...
    ASM_KEEP(vectors);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
#ifdef NON_MATCHING
    template_base = (u8 *)D_8002E5D8 + 0x1A28;
#else
    template_base = (u8 *)0x80030000;
#endif
    ASM_KEEP(template_base);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
    ...
    ASM_UNDEF(data_ptr);   /* MATCH pin: keeps a statement from moving across a call/branch */
#ifdef NON_MATCHING
    data_ptr = (u8 *)D_8002E5E8 + 0x1A18;
#else
    data_ptr = (u8 *)0x80030000;
#endif
    ASM_KEEP(data_ptr);   /* MATCH pin: retail immediate-load split depends on it */
