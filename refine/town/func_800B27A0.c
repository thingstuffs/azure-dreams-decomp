...
void func_800AFF00(S_800AFF00_4 *state, S_800AFF00_0 *primary, S_800AFF00_2 *secondary) {
    ...
    ASM_UNDEF(primary_fields);   /* MATCH pin: retail schedule: same instructions, different order without it */
    primary_fields = primary;
    ASM_KEEP(primary_fields);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
