...
s32 func_800B85E8(s32 entry_id, s8 *slots_out) {
...

    {
        ...
        ASM_KEEP_NV(dispatch_addr);   /* MATCH pin: load-bearing for the whole function shape */
        if (dispatch_addr >= 16) {
            goto case_default;
        }
        (void)case_labels;
...
case_4_8:
    {
        ...
        u8 *slot_table_base;
        u8 *slot_row;
        register u32 selected_id ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
        s32 slot;
        u32 slot_offset;

        {
            register u8 *entry_base ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
        ...
        ASM_KEEP_NV(selected_id);   /* MATCH pin: retail register colouring depends on it */
        {
            u32 link_id;

            link_id = selected_entry[6];
            ASM_KEEP_NV(link_id);   /* MATCH pin: retail register colouring depends on it */
            if (link_id == 0) {
                goto case_default;
            }
        }
        ...
        ASM_KEEP_NV(filter_entry);   /* MATCH pin: retail delay-slot fill depends on it */
        do {
            slot_offset = slot & 0xFF;
            slot_offset *= 2;
            slot_row = (u8 *)((u32)slot_offset + (u32)slot_table_base);
            ASM_KEEP_NV(slot_row);   /* MATCH pin: load-bearing for the whole function shape */
...
case_16:
    {
        u8 *selected_entry;
        register u8 *entry_base ASM_REG("$3");   /* MATCH pin: retail immediate-load split depends on it */
        u32 selected_id;
        register u32 link_id ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
        ...
        }
        {
            ...
            ASM_KEEP_NV(special_base);   /* MATCH pin: keeps a constant in a register as retail does */
            special_entry_id = special_base[0x33E7];
            ASM_KEEP_NV(special_entry_id);   /* MATCH pin: retail register colouring depends on it */
...
case_1:
    {
        ...
        u8 *slot_row;
        register u32 selected_id ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
        register s32 slot ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
        u32 slot_index;
        ...
        ASM_KEEP_NV(data_base);   /* MATCH pin: keeps a statement from moving across a call/branch */
        slot_data_base = data_base + 0x2EA4;
        selected_id = (u8)raw_entry_id;
        data_base = D_800D2644_case1;
        entry_offset = selected_id << 5;
        selected_entry = data_base + entry_offset;
        ASM_KEEP_NV(slot_data_base);   /* MATCH pin: retail schedule: same instructions, different order without it */
        do {
...
case_default:
    *slot_out = 0;
return_count:
    ASM_KEEP_NV(raw_entry_id);   /* MATCH pin: retail schedule: same instructions, different order without it */
