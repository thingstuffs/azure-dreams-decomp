...
void func_8009C648(u8 *source_data, M2C_UNK spawn_arg) {
    ...
    s32 random_z;
    S_8009C648_5 *motion;
    register void *source ASM_REG("$22") = source_data;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    M2C_UNK spawn_context = spawn_arg;

    memcpy(part_indices, D_80088D78, 6);
    part_index = 0;
    velocity_bias = 0xFFFC0000;
    ASM_KEEP(velocity_bias);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    part_params = D_800D06C8;
    ASM_KEEP(source);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP(spawn_context);   /* MATCH pin: load-bearing for the whole function shape */
