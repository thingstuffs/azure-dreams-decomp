...
void func_800A8CA8(void *object, s32 phase, void *source_data, S_800A8CA8_2 *extent) {
    register S_800A8CA8_1 *source ASM_REG("$18");   /* MATCH pin: load-bearing for the whole function shape */
    u32 phase_bits;
    s32 offset_phase;
    register s32 double_phase ASM_REG("$16");   /* MATCH pin: load-bearing for the whole function shape */
    s32 y_offset;
    s32 x_offset;
    void *start_point;
    ...
    source = source_data;
    ASM_DEP(source);   /* MATCH pin: retail schedule: same instructions, different order without it */
