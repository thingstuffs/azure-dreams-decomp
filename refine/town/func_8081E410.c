...
void func_80020C10(void *state_arg, void *target_arg, void *effect_arg)
{
    ...
    register void *spin_data ASM_REG("$20");   /* MATCH pin: retail register colouring depends on it */
    s32 prev_angle;
    LocalPoint point;
    s32 distance;
    s32 dx_squared;
    ...
    s32 abs_dy;
    s32 speed_limit;
    register s32 speed_step ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    s32 sector_start;
    s32 reduced_speed;
    s32 icon_x;
    s32 icon_frame;
    ...
    state = state_arg;
    target = target_arg;
    ASM_KEEP_NV(target);   /* MATCH pin: load-bearing for the whole function shape */
    effect = effect_arg;
...
middle_range:
    if (distance < 209) {
        ...
        abs_dx = dx;
        if (abs_dx < 0) {
            ASM_KEEP(abs_dx);   /* MATCH pin: retail register colouring depends on it */
            abs_dx = -abs_dx;
        }
        abs_dy = dy;
        if (abs_dy < 0) {
            ASM_KEEP(abs_dy);   /* MATCH pin: retail register colouring depends on it */
            abs_dy = -abs_dy;
        }
        if (abs_dx > abs_dy) {
            goto x_axis;
