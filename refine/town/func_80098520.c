...
void func_80095C80(S_80095C80_1 *position) {
    ...
    register s32 axis_test ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    M2C_UNK *motion_page;

    motion_page = (M2C_UNK *)D_80100000;
    ASM_KEEP_NV(motion_page);   /* MATCH pin: retail immediate-load split depends on it */
    motion = (M2C_UNK *)((s8 *)motion_page - 0x1A40);
    {
        ...
        if (((S_80095C80_0 *)motion)->unk_10 > 0) {
            register s32 x_hit ASM_REG("$16");   /* MATCH pin: keeps a statement from moving across a call/branch */
            s32 boundary_test;
            register s32 probe_coord ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
            void *probe_ptr;
            ...
            probe_ptr = &probe;
            ASM_KEEP_NV(probe_ptr);   /* MATCH pin: retail schedule: same instructions, different order without it */
...
check_pos_xy_corner:
            ...
            ASM_KEEP_NV(pos_xy_y);   /* MATCH pin: retail schedule: same instructions, different order without it */
            pos_xy_test = ((S_80095C80_2 *)pos_xy_motion)->unk_10;
            ASM_CLOBBER("$5");   /* MATCH pin: retail delay-slot fill depends on it */
            y_step_or_side = 0;
            ASM_KEEP_NV(y_step_or_side);   /* MATCH pin: retail schedule: same instructions, different order without it */
        ...
        if (((S_80095C80_0 *)motion)->unk_10 < 0) {
            ...
            if (pos_x_hit == 0) {
                ...
                if (neg_y_hit == 0) {
                    ...
                    x_offset &= 0x3F;
                    ASM_KEEP_NV(x_offset);   /* MATCH pin: retail schedule: same instructions, different order without it */
                    if (axis_test < 0) {
...
check_pos_x_neg_y_corner:
            ...
            ASM_KEEP_NV(pos_x_neg_y_y);   /* MATCH pin: retail schedule: same instructions, different order without it */
            ...
            ASM_KEEP_NV(pos_x_neg_y_test);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
...
check_neg_x:
        motion_or_hit = (unsigned long)((s8 *)motion_page - 0x1A40);
        {
            ...
            if (((S_80095C80_4 *)((M2C_UNK *)motion_or_hit))->unk_10 > 0) {
                register s32 y_step_or_side ASM_REG("$5");   /* MATCH pin: retail delay-slot fill depends on it */
                register s32 probe_coord ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
                s32 y_test;
...
check_neg_x_pos_y_corner:
                ...
                ASM_KEEP_NV(neg_x_pos_y_y);   /* MATCH pin: retail schedule: same instructions, different order without it */
            ...
            if (((S_80095C80_4 *)((M2C_UNK *)motion_or_hit))->unk_10 < 0) {
                ...
                if (motion_or_hit == 0) {
                    ...
                    if (axis_test == 0) {
                        ...
                        axis_test &= 0x3F;
                        ASM_KEEP_NV(axis_test);   /* MATCH pin: retail delay-slot fill depends on it */
                        if (y_offset < 0) {
                            y_offset = -y_offset;
                        }
                        y_offset &= 0x3F;
...
check_neg_xy_corner:
                ...
                ASM_KEEP_NV(neg_xy_y);   /* MATCH pin: retail schedule: same instructions, different order without it */
