...
void func_800B94FC(void) {
    s32 entry_count;
    register s32 choice_index ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    s32 map_kind;
    s32 variant;
    register s32 selector ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
    void **entries;
    register void **entry_slot ASM_REG("$3");   /* MATCH pin: keeps a constant in a register as retail does */
    void *entry;
    u8 *map_records;
    u8 *fallback_records;
    u8 *town_state;
...
kind_5_default:
    ...
    ASM_KEEP(entry_count);   /* MATCH pin: retail immediate-load split depends on it */
...
kind_15_prep:
    ASM_UNDEF(selector);   /* MATCH pin: retail basic-block layout depends on it */
kind_15:
    variant = D_800136B8;
    ASM_KEEP(selector);   /* MATCH pin: retail register colouring depends on it */
    if (variant == 0xC) {
        goto kind_15_12;
    }
    if ((s32)variant < 0xD) {
...
store_value:
    *entry_slot = entry;
    ASM_KEEP(entry);   /* MATCH pin: keeps a constant in a register as retail does */
    entry_count++;
    entries_base = D_800718E4;
    entry_slot = &entries_base[entry_count];
    entry = D_800D1E7C;
    *entry_slot = entry;
    ASM_KEEP(entry_slot);   /* MATCH pin: retail immediate-load split depends on it */
    entry_count++;
    goto finish;
