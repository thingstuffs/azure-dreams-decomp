...
void func_80018D14(u8 *object_ref)
{
    void **global_page;
    u8 *context;
    register u8 *loaded_state ASM_REG("$2");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register u32 object_flags ASM_REG("$2");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    u8 *callbacks;
    ...
    ASM_KEEP(object_flags);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ...
    if (object_ref[0] != object_table[slot * 0x54 + 0x13]) {
        ...
        ASM_KEEP(callback_168);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        ...
        callback_174(1);
        ASM_KEEP(slot);   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    }
