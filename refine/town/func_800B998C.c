...
void func_800B70EC(void) {
    register s32 scene_id ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    s32 special_scene;
    register s32 below_extra_range ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s32 scene_index;
    s32 scene_offset;
    s32 single_row;
    s32 size;
    ...
    u8 *shared_state;
    u16 *upload_rect;
    register u16 *initial_rect ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
    u8 *image_data;
    register u8 *scene_entry ASM_REG("$4");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register u8 *scene_table ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
    ...
    limit = 0x7F;
    size = 0x2000;
    ASM_KEEP(upload_rect);   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    ASM_KEEP(image_data);   /* MATCH pin: retail schedule: same instructions, different order without it */
    ASM_KEEP4(asset_state, seven, limit, size);   /* MATCH pin: retail immediate-load split depends on it */
    ...
    ASM_KEEP(scene_id);   /* MATCH pin: keeps a statement from moving across a call/branch */
    single_row = 1;
    special_scene = 0x21;
    if (scene_id != special_scene) {
        ASM_KEEP(special_scene);   /* MATCH pin: load-bearing for the whole function shape */
...
common:
    ...
    *(void **)asset_state = asset_data;
    ...
    ASM_KEEP(scene_offset);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP(scene_entry);   /* MATCH pin: load-bearing for the whole function shape */
