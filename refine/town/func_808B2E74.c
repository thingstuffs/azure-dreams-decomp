...
void func_808B2E74(void)
{
    ...
    register s32 slot_offset ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */

    do {
        s32 first_id;
        register s32 first_bits ASM_REG("$3");   /* MATCH pin: load-bearing for the whole function shape */
        register u8 *first_byte ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
        ...
        *first_byte &= ~first_bits;
        {
            s32 second_id;
            register s32 second_bits ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
            u8 *second_byte;
