...
void func_8009CEE8(void) {
    ...
    register s32 call_x ASM_REG("$6");   /* MATCH pin: retail schedule: same instructions, different order without it */
    register s32 call_y ASM_REG("$7");   /* MATCH pin: retail schedule: same instructions, different order without it */
    M2C_UNK setup_a[2];
    M2C_UNK setup_b[2];
    s32 *item_cursor;
    s32 coord_x;
