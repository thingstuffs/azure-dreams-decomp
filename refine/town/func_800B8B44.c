...
void func_800B62A4(void *source, s32 output) {
    void *entries[3];
    s32 index;
    register s32 slot_count ASM_REG("$17");   /* MATCH pin: retail register colouring depends on it */
    s32 scan_base;
    s32 has_room;
    void *entry;
    s32 *slot_value;
    ...
    index = slot_count;
    if ((has_room != 0) && (((S_800B62A4_1 *)source)->unk_49 != 0)) {
        func_800B61C0(source + 0x48, output, slot_count);
        slot_count += 1;
        ASM_KEEP(slot_count);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
        index = slot_count;
    }
    if (index < 3) {
        ...
        do {
            ...
            index += 1;
            *slot_value = 0;
            ASM_KEEP(slot_value);   /* MATCH pin: keeps a statement from moving across a call/branch */
            slot_cursor += 4;
        } while (index < 3);
    }
}
