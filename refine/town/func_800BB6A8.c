...
void func_800B8E08(s32 draw_param) {
    ...
    s32 entry_kind;
    s32 draw_x;
    register s32 draw_kind ASM_REG("$16");   /* MATCH pin: retail delay-slot fill depends on it */
    u32 overlay_index;
...
switch_done:
            {
                s32 status_c8;
                register u8 *status_c8_page ASM_REG("$3");   /* MATCH pin: retail immediate-load split depends on it */
                ...
                ASM_KEEP(status_page);   /* MATCH pin: keeps a constant in a register as retail does */
        ...
        {
            register u8 *draw_state_page ASM_REG("$2");   /* MATCH pin: keeps a constant in a register as retail does */
            register volatile u8 *final_state ASM_REG("$3");   /* MATCH pin: retail immediate-load split depends on it */
            u8 *draw_entry;
            s32 final_x;
            s32 final_y;
            s32 entry_offset;
            ...
            ASM_KEEP(draw_state_page);   /* MATCH pin: keeps a constant in a register as retail does */
