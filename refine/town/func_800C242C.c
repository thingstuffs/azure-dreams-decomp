...
{
    register s16 fixed_coord ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s16 state;
    s32 coord_term;
    register s32 random_value ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    s32 spawn_count;
    s32 count_bits;
    register s32 random_coord ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 spawn_coord ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s32 base_coord;
    u16 next_value;
    u8 *town;
    u8 *kind;
    void *coords;
    register void *emitter ASM_REG("$18");   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void *new_object;
    void *object;
    void *callback;
