...
s32 func_800254A4(void)
{
    void *left_data[2];
    void *right_data[2];
    register u8 *parent_state ASM_REG("$21") = NULL;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    void *object;
    ...
    ASM_KEEP(parent_state);   /* MATCH pin: retail schedule: same instructions, different order without it */
...
create_child:
    ...
    *child_slot = object;
    ...
    do {
        ...
        ASM_KEEP(first_dst_byte);   /* MATCH pin: load-bearing for the whole function shape */
        ASM_KEEP(second_dst_byte);   /* MATCH pin: retail register colouring depends on it */
        index--;
...
next_right_row:
    ...
    if (object != NULL) {
        ...
        ASM_KEEP(final_state);   /* MATCH pin: retail schedule: same instructions, different order without it */
        final_extent = 0x1C;
