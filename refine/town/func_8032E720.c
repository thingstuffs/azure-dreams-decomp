...
s32 func_80018F20(s32 *slot_list)
{
    ...
    s32 free_slot;
    u32 next_slot_addr;
    u8 *used_map;   /* MATCH pin: retail register colouring depends on it */
    u8 *used_flag;
    u8 *initial_page;
    u8 *initial_root;
    u8 *loaded_base;
    ...
    if (*slot_cursor != 0) {
        ...
        do {
            ...
            if (used_map[(u32)(next_slot_addr - slot_base) >> 2] != 0) {
                ...
                ASM_KEEP(message_page);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
            ...
            ASM_KEEP(used_flag);   /* MATCH pin: load-bearing for the whole function shape */
            *used_flag = 1;
            ASM_SCHED_BARRIER();   /* MATCH pin: retail schedule: same instructions, different order without it */
            slot_cursor++;
            next_slot_addr = *slot_cursor;
    ...
    if (free_slot >= 0x14) {
        ...
        call_table = ((S_80018F20_3 *)root)->unk_20;
        report_error = (*(TownCall3 *)((u8 *)call_table + 0x168));
        ASM_KEEP(report_error);   /* MATCH pin: retail basic-block layout depends on it */
