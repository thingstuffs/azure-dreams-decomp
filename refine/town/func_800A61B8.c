...
void func_800A3918(S_800A3918_0 *anim, s32 unused) {
    s16 state;
    s32 phase;
    register u8 *global_base ASM_REG("$4");   /* MATCH pin: retail register colouring depends on it */
    s16 offset;
    u16 offset_bits;
...
state_one:
    ASM_KEEP_NV(global_base);   /* MATCH pin: retail basic-block layout depends on it */
