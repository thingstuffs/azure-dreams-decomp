...
{
    register DataRecord *record ASM_REG("$16");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register u8 *entry ASM_REG("$17");   /* MATCH pin: keeps a statement from moving across a call/branch */
    s32 record_flags;
    u32 tag_bits;
    u32 flag_bits;
    u32 header_word;
    register u32 packed_word ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s32 half_bits;
    u32 header_or_addr;
    ...
    ASM_KEEP(record_flags);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP(tag_bits);   /* MATCH pin: load-bearing for the whole function shape */
    ASM_KEEP(flag_bits);   /* MATCH pin: retail register colouring depends on it */
    ASM_KEEP(header_word);   /* MATCH pin: retail schedule: same instructions, different order without it */
    return record;
}
