...
s32 func_80096E9C(s32 *x, s32 *y) {
    s32 nearest_side[2];
    s32 edge_dist[4];
    register volatile s32 *bounds_page ASM_REG("$6");   /* MATCH pin: load-bearing for the whole function shape */
    register volatile s32 *bounds ASM_REG("$7");   /* MATCH pin: load-bearing for the whole function shape */
    s32 hit_sides;
    s32 first_min_x;
    s32 min_y_side;

    bounds_page = (volatile s32 *)0x80100000;
    ASM_KEEP(bounds_page);   /* MATCH pin: retail delay-slot fill depends on it */
