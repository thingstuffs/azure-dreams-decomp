...
{
    register void *self ASM_REG("$21") = effect;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register void *origin ASM_REG("$20") = effect_origin;   /* MATCH pin: retail address form (%hi/%lo vs base+offset) depends on it */
    register u16 next_state ASM_REG("$2");   /* MATCH pin: retail register colouring depends on it */
    s32 state;
    u16 timer;
    s32 spawn_count;
    s32 rand_value;
