...
void func_8001A51C(u16 *source, S_8001A51C_1 *rect) {
    s32 col;
    s32 row;
    register s32 dst_index ASM_REG("$4");   /* MATCH pin: retail delay-slot fill depends on it */
    s32 row_offset;
    s32 stride_shift;
    volatile u16 *src_cursor;
    u16 *dst_base;
    register u16 *dst_cell ASM_REG("$2");   /* MATCH pin: load-bearing for the whole function shape */
    s8 *grid_info;
    void *buffer_state;

    src_cursor = source;
    ASM_KEEP(src_cursor);   /* MATCH pin: retail delay-slot fill depends on it */
        ...
        do {
            ...
            if (rect->unk_04 > 0) {
                do {
                    ...
                    dst_index += col;
                    ASM_KEEP(dst_index);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                    row_offset = rect->unk_02;
                    row_offset += row;
                    ASM_KEEP(row_offset);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
                    ...
                    if (*src_cursor != 0) {
                        ...
                        ASM_KEEP(dst_cell);   /* MATCH pin: retail delay-slot fill depends on it */
                        *dst_cell = *src_cursor;
                    }
                    col += 1;
                    src_cursor += 1;
