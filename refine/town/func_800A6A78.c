...
void *func_800A41D8(void *origin, void *render_flags, void *draw_state, void *prim_buffer, u16 texture_index) {
    ...
    u16 screen_y;
    s16 strip_shade;
    register s32 strip_offset ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    s16 shade;
    register s32 uv_top ASM_REG("$4");   /* MATCH pin: load-bearing for the whole function shape */
    register s32 uv_value;
    s32 bucket_offset;
    s32 bucket;
    register s32 uv_left ASM_REG("$5");   /* MATCH pin: retail register colouring depends on it */
    register s32 uv_height ASM_REG("$6");   /* MATCH pin: retail register colouring depends on it */
    s32 coord_value;
    s32 bottom_x;
    s32 top_y;
    s32 bottom_y;
    ...
    u16 uv_bottom;
    u16 uv_right;
    register s32 right_u ASM_REG("$3");   /* MATCH pin: retail register colouring depends on it */
    u16 bottom_right_uv;
    u16 right_x;
    u16 left_x;
    u32 bucket_index;
    ...
    void *right_colors;
    void *buffer_end;
    register void *packet_cursor ASM_REG("$16");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    void *state;
    register void *entry_cursor ASM_REG("$18");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    register void *next_prim ASM_REG("$19");   /* MATCH pin: retail schedule: same instructions, different order without it */
    s32 half_index;
    s32 texture_entry;
    s32 strip_index;
    s32 *bucket_list;
        ...
        if (func_80065610(state + 0x70, state + 0x78, state + 0x80, state + 0x88, state + 0xF0, state + 0xF4, state + 0xF8, state + 0xFC, state + 0x90, state + 0xC0, state + 0x94) > 0) {
            ...
            if ((first_three_visible | corner_visible) != 0) {
                ...
                do {
                    ...
                    uv_value = uv_left;
                    ASM_KEEP_NV(uv_value);   /* MATCH pin: retail keeps a copy the compiler would otherwise drop/add */
