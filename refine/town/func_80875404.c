...
void func_80875404(void)
{
    ...
    s32 value_offset;
    register s16 *value_base ASM_REG("$17");   /* MATCH pin: retail callee-saved set / frame layout depends on it */
    s16 *saved_base;
    register s32 choice_index ASM_REG("$19");   /* MATCH pin: retail register colouring depends on it */
    s16 *next_value;
    s32 callback_offset;
