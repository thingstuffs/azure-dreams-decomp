...
s32 func_800A7234(s32 arg0, s32 arg1, s32 arg2, s16 *arg3, s16 *arg4, s16 *arg5) {
    ...
    s32 scan_x1;
    s32 scan_y1;
    register s32 scan_y2 ASM_REG("$21");
    register s32 scan_x2 ASM_REG("$20");
    register s32 count8 ASM_REG("$19");
    register s32 dir8 ASM_REG("$17");
    register s32 count16 ASM_REG("$19");
    register s32 dir16 ASM_REG("$17");
    register s32 call_dir ASM_REG("$16");
    register s32 call_x ASM_REG("$4");
    register s32 call_y ASM_REG("$5");
    register s32 distance_z ASM_REG("$6");
    register s32 empty_arg ASM_REG("$6");
    register u16 *sp_arg ASM_REG("$7");
    register s32 table_index8 ASM_REG("$3");
    register s32 table_index16 ASM_REG("$2");
    register u32 scratch ASM_REG("$8");
    register s32 hard_zero ASM_REG("$0");
    register s32 next_dir8 ASM_REG("$2");
    register s32 next_dir16 ASM_REG("$2");
    ...
    u16 *dx;
    u16 *dy;
    register s32 raw_arg0 ASM_REG("$16");
    register s32 raw_arg1 ASM_REG("$17");
    register s32 raw_arg2 ASM_REG("$19");
    s32 norm_arg1;
    register s32 found_result ASM_REG("$2");
    ...
    raw_arg1 = arg1;
    raw_arg2 = arg2;
    ASM_KEEP_NV(raw_arg0);
    ASM_KEEP_NV(raw_arg1);
    ASM_KEEP_NV(raw_arg2);
...
found:
    ...
    *(s16 *)scratch = found_y;
    ...
    ASM_KEEP(scratch);
    found_result = 1;
    ASM_LIVE_SIBCALL_PIN(found_result, 1);
...
next8:
    ...
    ASM_KEEP(scan_y2);
    dir_seed = D_8008347E[0];
    ASM_KEEP_NV(dir_seed);
    shifted_y = base_y << 0x10;
    scan_x2 = shifted_y >> 0x10;
    dir16 = dir_seed & 0xF;
loop16:
    empty_arg = hard_zero;
    ASM_KEEP_NV(empty_arg);
    table_index16 = dir16 << 1;
    ASM_KEEP_NV(table_index16);
