...
Ent *func_8009C93C(Ent *a, Pos *b, u32 coordArg, s32 mult, Ent *ent2) {
    ...
    register u32 coord ASM_REG("$5");
    register s32 savedMult ASM_REG("$20");
    s32 off;
    s32 shifted;
    s32 value;
    s32 step;
    ...
    coord = coordArg + zero;
    savedMult = mult;
    ASM_USE_NV(savedMult);
    if (ent2 == 0) {
        if (a->flags1c & 0x400) {
            register s32 f ASM_REG("$2");
...
adjusted:
...

    {
        ...
        saved = dance;
        {
            register s32 r ASM_REG("$3") = func_800A6D30() & 0xFFFF;
    ...
    value = func_800A6D30();
    {
        register Ent *callArg ASM_REG("$4");
    ...
    if ((ent2->flags1c & 0x238) != 0) {
        ...
        {
            register s32 sum ASM_REG("$2") = ent2->unk70;
