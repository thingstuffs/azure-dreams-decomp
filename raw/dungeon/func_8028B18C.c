...
void func_8001E18C(void)
{
    ...
    s32 special_x;
    s32 special_y;
    register s32 limit ASM_REG("$22");
    s32 index;
    register s32 special_flag ASM_REG("$2");
    s32 setup_result;
    ...
    *(u16 *)(setup_page + 0x209E) += (func_800A6D30() & 3) + 0x33;
    ...
    if (D_80081468[3] == 1) {
        register s32 limit_random ASM_REG("$2");

        special_x = 0;
        limit_random = func_800A6D30() & 7;
        ASM_KEEP_NV(limit_random);
        limit = limit_random + 8;
    } else {
        register s32 limit_random ASM_REG("$2");
        register s32 half_limit ASM_REG("$4");
...
main_loop:
    ...
    if (state->active == 0) {
            ...
            if ((s16)func_8001E660(&kind, &subtype, 0, 0) == 0) {
                ...
                ASM_KEEP_NV(special_flag);
                if (special_flag != 0) {
                    register u16 page_value ASM_REG("$3");
...
cleanup:
    if (count < 0x40) {
        register DungeonState *cleanup_base ASM_REG("$2");
        register DungeonState *cleanup_state ASM_REG("$3");
