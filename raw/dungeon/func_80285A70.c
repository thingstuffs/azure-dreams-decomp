...
void func_80018A70(void) {
    ...
    s32 i;
    {
        ...
        ASM_KEEP_NV(xor_base);

        if (entry_flags & 2) {
            u32 v0role;
            s32 sum;
            register u32 a1role ASM_REG("$5");

            func_800A6D98(state->seed);
            v0role = func_800A6D30();
            common_arg = D_80083780;
            ASM_KEEP_NV(common_arg);
            sum = state->seed;
            v0role &= 0xFFFF;
            state->field8 = 0;
            state->fieldA = 0;
...
after_optional_setup:
    ...
    if (page1->mode != 2) {
        ...
        register s32 random_remainder ASM_REG("$16");
        s32 *table;
        s32 random_result;
        s32 division_magic;
...
do_action:
    ...
    ASM_SCHED_BARRIER();
    {
        register u8 *page8_temp ASM_REG("$2");
        page8_temp = (u8 *)0x80080000;
        ASM_KEEP_NV(page8_temp);
