...
{
    register u8 *obj ASM_REG("$19") = arg0;
    register u8 *input ASM_REG("$8") = arg1;
    register u8 *global_addr ASM_REG("$21") = D_80083160;
    register u32 low_mask ASM_REG("$18") = 0x00FFFFFF;
    register DungeonState818D4800 *state ASM_REG("$2") =
        *(DungeonState818D4800 **)D_80083160;
    register u32 high_mask ASM_REG("$20") = 0xFF000000;
    register Scratch818D4800 *scratch ASM_REG("$17") = (Scratch818D4800 *)0x1F800000;
    register u8 *prim;
    u32 value;
    void *next_obj;
    u8 *initial_next;
...
#ifdef __mips__
    {
        register void *tail_a0 ASM_REG("$4") = scratch;
        register void *tail_a1 ASM_REG("$5") = next_obj;

        ASM_KEEP(tail_a1);
        ASM_TAILSLOT_PIN(tail_a0);
        func_8002409C();
    }
#else
