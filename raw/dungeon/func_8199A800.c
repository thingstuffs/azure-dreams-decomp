...
case_0:
    ...
    {
        register u8 *manager_page ASM_REG("$2") = (u8 *)0x80080000;
        register void *manager ASM_REG("$3") =
            *(void **)(manager_page + 0x14A8);
        ASM_KEEP(manager_page);
...
case_1:
    ...
    {
        u16 countdown = 0x10;
        register u16 live ASM_REG("$2") = U16_AT(state, 0x0A);
        ASM_KEEP(live);
...
case_2:
    {
        register void *entry ASM_REG("$5") = PTR_AT(MANAGER_PTR(), 0x60);

        if (entry != 0) {
            s32 value = (s32)U32_AT(entry, 0x1C) | 0x10000000;
            register u32 effect ASM_REG("$4") = 0;
            register void *child ASM_REG("$6");
    ...
    }
    {
        ...
        if (entry != 0) {
            register u32 mask ASM_REG("$2") = 0xEFFFFFFF;
            register u32 effect ASM_REG("$4") = 0x00808080;
