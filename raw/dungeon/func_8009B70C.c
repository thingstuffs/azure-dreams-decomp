#include "common.h"

typedef struct { u8 b0; u8 b1; u8 b2; u8 b3; } Elem;
typedef struct { u8 pad0[0xE]; s16 fieldE; u8 pad1[4]; } E2970;

extern E2970 D_800E2970[];
extern s16 D_8006CD00[];
extern s16 D_8006CD02[];
extern u16 D_8008347E;
extern u16 D_800DCEAC[];
extern u16 D_800DCEBC[];

extern Elem *func_8009FCAC(s32);
extern s16 func_800A6D30(void);
extern s32 func_800A0818(u8, u8, s8, s8, u16 *);
extern s16 func_800BCB04(u16, u16, s32);

s32 func_800A0E6C(void *arg0, s32 arg1, void *arg2, u16 *arg3) {
    register u8 *p3 ASM_REG("$19") = (u8 *)arg0;
    register u8 *p4 ASM_REG("$20") = (u8 *)arg2;
    u16 *fp = arg3;
    register Elem *s1 ASM_REG("$17");
    Elem *scan_entry;
    Elem *entry;
    register s16 s0 ASM_REG("$16");
    s16 i;
    s16 count;
    s16 c2;
    s16 c3;
    register s32 mod ASM_REG("$3");
    s16 r;
    s32 dir;
    register s32 zero ASM_REG("$4");
    u8 type;
    s32 b72, b73;
    s32 d;
    register s32 s5v ASM_REG("$21");
    s32 idx;
    register s32 tail ASM_REG("$2");
    E2970 *ct2;
    E2970 *ct1;
    register E2970 *table ASM_REG("$6");
    register s16 *offsets ASM_REG("$22");
    register u16 *x_offsets ASM_REG("$23");
    register s32 scan_s0 ASM_REG("$16");
    register s32 bitmap_idx ASM_REG("$5");
    register u8 *bit_next ASM_REG("$3");
    register u8 *bit_prev_v0 ASM_REG("$2");
    register u8 *bit_prev_s0 ASM_REG("$16");
    u8 bitmap[8];

    if (*(s8 *)(p3 + 0x26) < 0) {
        goto L_BD00;
    }

    b72 = p4[0x72];
    b73 = p4[0x73];
    if (((b72 | b73) != 0) && (*(s8 *)(p3 + 0x26) == (s16)arg1)) {
        goto L_B9A8;
    }

    p4[0x73] = 0;
    p4[0x72] = 0;
    p4[0x67] = 0;
    p3[0x27] = 0xFF;
    s1 = func_8009FCAC(*(s8 *)(p3 + 0x26));
    if (s1 != 0) {
        count = D_800E2970[*(s8 *)(p3 + 0x26)].fieldE;
        if (count > 0) {
            i = 0;
            do {
                tail = (s16)i * sizeof(Elem);
                ASM_KEEP_NV(tail);
                scan_entry = (Elem *)((unsigned long)tail + (unsigned long)s1);
                if (scan_entry->b0 != 0) {
                    tail = p3[0x24];
                    d = *(volatile u8 *)&scan_entry->b0;
                    tail -= d;
                    if (tail < 0) tail = -tail;
                    if (tail < 2) {
                        tail = p3[0x25];
                        d = scan_entry->b1;
                        tail -= d;
                        if (tail < 0) tail = -tail;
                        if (tail < 2) {
                            goto L_B998;
                        }
                    }
                }
                i++;
            } while ((s16)i < count);
        }
L_B84C:
        i = 0;
        s0 = func_800A6D30();
        ASM_KEEP(s0);
        ct1 = D_800E2970;
        if (c3 = ct1[*(s8 *)(p3 + 0x26)].fieldE, type = p3[0x26], c3 > 0) {
            table = ct1;
            ASM_KEEP(table);
            ASM_USE(ct1);
            do {
                zero = (s16)s0;
                mod = (s8)type;
                c2 = table[mod].fieldE;
                mod = zero % c2;
                entry = (Elem *)((unsigned long)((s16)mod * sizeof(Elem)) + (unsigned long)s1);
                s0 = mod;
                if (entry->b0 != 0) {
                    if (p3[0x27] != (s16)mod) {
                        zero = 0;
                        dir = p4[0x67];
                        switch (dir) {
                        case 0:
                            tail = *(s16 *)&entry->b2 < 2;
                            break;
                        case 1:
                            tail = *(s16 *)&entry->b2 == 2;
                            break;
                        case 2:
                            tail = *(s16 *)&entry->b2 == 0;
                            break;
                        default:
                            goto L_DIR1_DONE;
                        }
                        ASM_KEEP(tail);
                        zero = tail;
L_DIR1_DONE:
                        tail = zero;
                        ASM_KEEP(tail);
                        if (tail) {
                            goto L_BCB8;
                        }
                    }
                }
                i++;
                s0++;
            } while (c3 = table[*(s8 *)(p3 + 0x26)].fieldE, type = p3[0x26], (s16)i < c3);
            return 0;
        }
    }
    return 0;

L_B998:
    p3[0x27] = i;
    p4[0x67] = scan_entry->b2;
    goto L_B84C;

L_B9A8:
    if ((p3[0x24] == b72) && (p3[0x25] == b73)) {
        s1 = func_8009FCAC(*(s8 *)(p3 + 0x26));
        if (s1 == 0) {
            goto L_RET0;
        }
        s0 = D_8008347E;
        ct2 = D_800E2970;
        if (c3 = ct2[*(s8 *)(p3 + 0x26)].fieldE, type = p3[0x26], c3 > 0) {
            i = 0;
            table = ct2;
            ASM_KEEP(table);
            ASM_USE(ct2);
            do {
                zero = (s16)s0;
                mod = (s8)type;
                c2 = table[mod].fieldE;
                mod = zero % c2;
                entry = (Elem *)((unsigned long)((s16)mod * sizeof(Elem)) + (unsigned long)s1);
                s0 = mod;
                if (entry->b0 != 0) {
                    if (p3[0x27] != (s16)mod) {
                        zero = 0;
                        dir = p4[0x67];
                        switch (dir) {
                        case 0:
                            tail = *(s16 *)&entry->b2 < 2;
                            break;
                        case 1:
                            tail = *(s16 *)&entry->b2 == 2;
                            break;
                        case 2:
                            tail = *(s16 *)&entry->b2 == 0;
                            break;
                        default:
                            goto L_DIR2_DONE;
                        }
                        ASM_KEEP(tail);
                        zero = tail;
L_DIR2_DONE:
                        tail = zero;
                        ASM_KEEP(tail);
                        if (tail) {
                            goto L_BCB8;
                        }
                    }
                }
                i++;
                s0++;
            } while (c3 = table[*(s8 *)(p3 + 0x26)].fieldE, type = p3[0x26], (s16)i < c3);
            return 0;
        }
        return 0;
    }

L_BB24: {
    register s32 masked ASM_REG("$2");
    register u32 base_page ASM_REG("$3");
    register s32 call_a2 ASM_REG("$6");

    tail = (s16)((((s16)*(u16 *)(p4 + 0x2A) >> 9) - 4) & 7);
    *(s32 *)&bitmap[0] = 0;
    *(s32 *)&bitmap[4] = 0;
    bit_next = &bitmap[tail];
    *bit_next = 1;
    ASM_KEEP(bit_next);
    idx = (tail + 1) & 7;
    ASM_KEEP(idx);
    bit_next = &bitmap[idx];
    ASM_KEEP(bit_next);
    tail = (tail - 1) & 7;
    bit_prev_v0 = &bitmap[tail];
    *bit_next = 1;
    *bit_prev_v0 = 1;
    i = 0;
    masked = (func_800A0818(p3[0x24], p3[0x25], *(s8 *)(p4 + 0x72), *(s8 *)(p4 + 0x73), fp) & 0xFFF) << 16;
    ASM_KEEP_NV(masked);
    s5v = masked >> 16;
    ASM_KEEP(s5v);
    base_page = 0x80070000;
    ASM_KEEP_NV(base_page);
    offsets = (s16 *)(base_page - 0x3300);
    base_page = 0x800E0000;
    ASM_KEEP_NV(base_page);
    x_offsets = (u16 *)(base_page - 0x3154);
    ASM_KEEP_NV(x_offsets);
    tail = (s16)(((masked >> 25) - 4) & 7);
    bit_next = &bitmap[tail];
    *bit_next = 1;
    ASM_KEEP(bit_next);
    idx = (tail + 1) & 7;
    ASM_KEEP(idx);
    bit_next = &bitmap[idx];
    ASM_KEEP(bit_next);
    tail = (tail - 1) & 7;
    bit_prev_s0 = &bitmap[tail];
    *bit_next = 1;
    *bit_prev_s0 = 1;
    do {
        if (*fp & 2) {
            tail = s5v - offsets[(s16)i];
        } else {
            tail = s5v + offsets[(s16)i];
        }
        scan_s0 = (tail >> 9) & 7;
        ASM_KEEP_NV(scan_s0);
        s0 = scan_s0;
        bitmap_idx = scan_s0;
        ASM_KEEP_NV(bitmap_idx);
        if (bitmap[bitmap_idx] == 0) {
            bitmap_idx <<= 1;
            bit_next = (u8 *)(bitmap_idx + (s32)x_offsets);
            ASM_KEEP_NV(bit_next);
            masked = p3[0x24];
            ASM_KEEP_NV(masked);
            zero = *(u16 *)bit_next;
            call_a2 = *(u16 *)(p4 + 0x88);
            ASM_KEEP4_NV(masked, zero, call_a2, bit_next);
            masked <<= 6;
            zero += masked;
            zero &= 0xFFFF;
            ASM_USE(zero);
            masked = 0x800E0000;
            ASM_KEEP_NV(masked);
            masked -= 0x3144;
            ASM_KEEP_NV(masked);
            bitmap_idx += masked;
            ASM_KEEP_NV(bitmap_idx);
            call_a2 = (s16)(call_a2 - 0x20);
            ASM_KEEP_DEP_NV(masked, call_a2);
            masked = p3[0x25];
            ASM_KEEP_NV(masked);
            bitmap_idx = *(u16 *)bitmap_idx;
            ASM_KEEP_DEP_NV(masked, bitmap_idx);
            masked <<= 6;
            bitmap_idx += masked;
            bitmap_idx &= 0xFFFF;
            r = func_800BCB04(zero, bitmap_idx, call_a2);
            if ((s16)r < 0x200) {
                goto L_BD8C;
            }
        }
        i++;
    } while ((s16)i < 8);
    return 0;
}

L_BCB8:
    tail = (s16)s0 * sizeof(Elem);
    ASM_KEEP_NV(tail);
    tail += (s32)s1;
    ASM_KEEP_NV(tail);
    mod = *(u8 *)tail;
    p4[0x72] = mod;
    mod = *(u8 *)(tail + 1);
    p4[0x73] = mod;
    tail = *(u8 *)(tail + 2);
    p4[0x67] = tail;
    tail = func_800A0818(p3[0x24], p3[0x25], *(s8 *)(p4 + 0x72), *(s8 *)(p4 + 0x73), fp);
    *(u16 *)(p4 + 0x2A) = tail;
    p3[0x27] = s0;
    goto L_RET0;

L_BD00: {
    register s32 final_a2 ASM_REG("$6");

    tail = 0xFF;
    mod = 0x80070000;
    ASM_KEEP_NV(mod);
    p3[0x27] = tail;
    final_a2 = *(u16 *)(p4 + 0x88);
    tail = *(s16 *)(p4 + 0x2A);
    mod = *(s16 *)(mod - 0x32FE);
    final_a2 = (s16)(final_a2 - 0x20);
    ASM_KEEP_DEP_NV(mod, final_a2);
    tail -= mod;
    tail >>= 9;
    scan_s0 = tail & 7;
    r = func_800BCB04(D_800DCEAC[scan_s0] + (p3[0x24] << 6),
                      D_800DCEBC[scan_s0] + (p3[0x25] << 6), final_a2);
    if ((s16)r < 0x200) {
        goto L_BD8C;
    }
    return 0;
}

L_BD8C:
    tail = scan_s0 << 9;
    *(u16 *)(p4 + 0x2A) = tail;
L_RET0:
    return 0;
}
