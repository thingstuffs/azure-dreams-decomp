...
{
  register u8 *init_page ASM_REG("$4") = D_80080000;
  register s32 count ASM_REG("$22") = 15;
  register s32 factor_base ASM_REG("$21") = (s16) arg3;
  register s32 src_base ASM_REG("$2") = arg5;
  register s32 initial_arg4 ASM_REG("$3") = arg4;
  s16 index = (s16) arg7;
  void *sp20 = (void *) 0x1F800084;
  register s32 color ASM_REG("$23") = arg6;
  register s32 mask ASM_REG("$20") = 0x00FFFFFF;
  s32 top = 0xFF000000;
  register s32 *src ASM_REG("$19");
  register u8 *scratch ASM_REG("$18");
  register u8 *prim ASM_REG("$17");
  register RenderState *initial_ctx ASM_REG("$2");
  GlobalState *base = &D_80083160;
  register s32 mid ASM_REG("$5");
  ...
  ASM_KEEP(mask);
  ASM_KEEP(color);
  ASM_KEEP(factor_base);
  *((u16 *) (scratch + 0x70)) = (u16) initial_arg4;
  *((u16 *) (scratch + 0x68)) = (u16) initial_arg4;
  *((void **) (scratch + 0x18)) = ((u8 *) initial_ctx) + 0xB0;
  do
  {
    register s32 index_value ASM_REG("$2") = index;
    register s32 pos ASM_REG("$5") = count + index_value;
    s32 rounded = pos;
    s32 next_pos;
    s32 scale_tmp;
    register s32 value ASM_REG("$2");
    register s32 raw_color ASM_REG("$4");
    register RenderState *ctx ASM_REG("$3");
    register s32 code ASM_REG("$2");
    register s32 sign ASM_REG("$3");
    register s32 magic ASM_REG("$3");
    union {
      s64 both;
      struct { s32 hi; u32 lo; } word;
    } wide_product;
    raw_color = color << 16;
    ASM_KEEP_NV(raw_color);
    ...
    {
      register s32 scale ASM_REG("$4") = scale_tmp;
      register s32 saved_scale ASM_REG("$7") = scale;
      register s32 product ASM_REG("$4");
      if (pos < 0)
      {
        rounded = pos + 15;
      }
    ...
    *((s32 *) (scratch + 0xB4)) = mid;
    if (((u32) mid) < 0x1E0U)
    {
      register u8 *prim2 ASM_REG("$16");
      *((s32 *) prim) = ((*((s32 *) prim)) & top) | (((s32 *) (*((void **) (scratch + 0x18))))[mid] & mask);
      {
        register u32 entry ASM_REG("$6");
        register u32 *table ASM_REG("$2");
        register u32 old_entry ASM_REG("$3");
        register u32 prim_low ASM_REG("$2");
        ...
        ASM_KEEP_NV(entry);
        old_entry = *((u32 *) entry);
        ASM_KEEP_NV(old_entry);
        prim_low = ((u32) prim) & mask;
        ASM_KEEP_NV(prim_low);
