...
{
    register void *base ASM_REG("$21") = arg0;
    void *dest = arg1;
    register void *ctx ASM_REG("$18") = arg2;
    ...
    u8 *ptr2492c;
    s32 s4;
    register s32 t0 ASM_REG("$8");
    register s32 particle_color ASM_REG("$6");
    register s32 particle_alpha ASM_REG("$7");
    register void *special_obj ASM_REG("$4");
    register void *special_data ASM_REG("$5");
    s32 state;
    s16 timer;
    u16 oldtimer;
    u16 tail0;
...
L2:
    ...
    if (F(base, s16, 0x20) >= 11) {
        ...
        do {
            ...
            if (s3 != 0) {
                ...
                s0 = F(s3, void *, 0xC);
                {
                    register void *tail_obj ASM_REG("$4") = s0;
                    if (s4 != 0) {
                        register void *tail_a1 ASM_REG("$5") = D_800DED28;
                        register void *tail_a2 ASM_REG("$6") = 0;
                        ASM_USE2(tail_a1, tail_a2);
                        r = 0x20;
                        ASM_KEEP_NV(r);
                        F(s0, u16, 0x10) = (u16)r;
                        ...
                        register void *tail_a1 ASM_REG("$5") = D_800DEB28;
                        register void *tail_a2 ASM_REG("$6") = 0;
                        ASM_USE2(tail_a1, tail_a2);
    ...
    {
        register u32 tail_a0 ASM_REG("$4");
        r = F(base, u16, 0xC);
        i = F(base, u16, 0xE);
...
L3_tail:
    {
        register void *obj ASM_REG("$5") = F(D_800814A8[0], void *, 0x60);
        register s32 v ASM_REG("$2");
        if (obj != 0) {
            F(obj, u32, 0x1C) |= 0x10000000;
            {
                register void *sub ASM_REG("$6") = F(obj, void *, -20);
...
L4:
    if (F(D_8002571C, s16, 0) == 0) {
        register void *l4_obj ASM_REG("$5") =
            F(D_800814A8[0], void *, 0x60);
        register s32 l4_color ASM_REG("$4");
        if (l4_obj != 0) {
            r = (u32)0xEFFFFFFF;
            l4_color = 0x00808080;
        ...
        {
            register D_80083460_t *dstate ASM_REG("$4") = &D_80083460;
            r = dstate->fieldA;
            F(dstate, u32, 0xC) = 0;
