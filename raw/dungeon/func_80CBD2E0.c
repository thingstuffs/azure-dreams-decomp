...
void func_80170AE0(void *a0, void *a1, void *a2)
{
    ...
    register s16 old_direction ASM_REG("$22");
    ...
    if (FIELD(entity, u8, 0xA7)) {
        D_801763E0[FIELD(entity, u8, 0x9A)](entity, motion, monster, actor);
    } else {
        D_80176380[FIELD(entity, u8, 0x9A)](entity, motion, monster, actor);
    }

    if (old_direction != FIELD(actor, s8, 0x6D)) {
    ...
    if (!(FIELD(entity, u16, 0x98) & 4)) {
        ...
        if (floor < 0x200) {
            ...
            if (FIELD(entity, s16, 0x92) + actor_height < floor) {
                (void)FIELD(entity, volatile u16, 0x98);
