...
void func_800CEA44(void *arg0) {
    register u8 *ctx ASM_REG("$23");
    LocalScratch local;
    s16 state;
    register s32 y ASM_REG("$16");
    s32 x;
    s32 entry_x;
    register s32 entry_y ASM_REG("$4");
    register s32 count ASM_REG("$22");
    s16 attempt;
    s32 variant;
    register s32 type ASM_REG("$21");
    s32 availability;
    u8 *entry;
    u8 *selection;
    void *object;
    ...
    if (state == 0) {
        {
            register u32 area_byte ASM_REG("$2");
            register s32 area_raw ASM_REG("$8");
...
spawn_loop:
        ...
        if (availability != 0) {
            attempt = 0xF;
            {
                register unsigned long area_raw ASM_REG("$8");
                register s32 area_calc ASM_REG("$3");
                register s32 area_offset ASM_REG("$2");
...
retry_position:
            ...
            if (attempt >= 0) {
                register s32 spawn_type ASM_REG("$17");

                D_800E2968 = 1;
                spawn_type = type;
                ASM_KEEP_NV(spawn_type);
