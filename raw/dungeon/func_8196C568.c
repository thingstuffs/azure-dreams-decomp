...
{
    OffsetTable offsets;
    register Effect *effect ASM_REG("$19");
    register Sprite *sprite ASM_REG("$18");
    register Motion *motion ASM_REG("$17");
    register s32 i;
    register Position *pos ASM_REG("$5");
    register Position *base ASM_REG("$4");
    register Work *active ASM_REG("$4");
    register DungeonState *dungeon ASM_REG("$3");
    register void **state_table ASM_REG("$2");
    register u8 *page_v0 ASM_REG("$2");
    register u8 *page_v1 ASM_REG("$3");
    register u8 *page_a0 ASM_REG("$4");
    register u8 *page_a3 ASM_REG("$7");
    register u8 *copy_hold ASM_REG("$2");
    ...
    register u16 dungeon_count ASM_REG("$2");
    register u8 object_kind ASM_REG("$2");
    register s16 init_timer ASM_REG("$2");
    register s32 state;
    register u32 random;
    register u16 coord_y ASM_REG("$2");
    static void *const keepalive[] = {
        &&state0, &&state1, &&state2, &&state2, &&state4
    };

    copy_hold = (u8 *)0x80020000;
    ASM_KEEP_NV(copy_hold);
    offsets = D_80024004;
    work->counter++;
    state = work->state;
    if ((u32)state >= 5) {
