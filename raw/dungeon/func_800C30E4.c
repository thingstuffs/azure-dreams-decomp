...
#else
register s32 dispatch_v1 ASM_REG("$3");
#endif

s32 func_800C8844(State *arg0, s16 arg1, s8 arg2_in) {
    register State *state ASM_REG("$16") = arg0;
    register s16 value ASM_REG("$17") = arg1;
    register s8 arg2 ASM_REG("$18") = arg2_in;
    register s32 result ASM_REG("$2");
    register s32 dividend ASM_REG("$4");
    ...
    if (dispatch_v1 != 0) {
        ...
        ASM_TAILSLOT_PIN_TIED(dead_shift);
        return func_800C88A4();
    }
    {
        register s32 shifted ASM_REG("$2");
        register s32 signed_value ASM_REG("$4");
