...
{
    register u8 *page_scratch ASM_REG("$9");
    register u8 *obj ASM_REG("$18") = arg0;
    u8 *global = (KEEP_MEMDEP_NV_EXPR(obj, page_scratch, *(u8 **)GFX_ROOT_SLOT),
                  *(u8 **)GFX_ROOT_SLOT);
    s32 i = 0;
    s32 xscale = arg2;
    register s32 divisor ASM_REG("$21") = arg3;
    s32 scaled = xscale < divisor;
    register u32 lowmask ASM_REG("$20") = 0x00FFFFFF;
    register u32 highmask ASM_REG("$23") = 0xFF000000;
    register u8 *iter ASM_REG("$19") = obj;
