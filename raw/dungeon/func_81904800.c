...
void BODY_NAME(u8 *arg0, u8 arg1) {
    ...
    register s32 tail_a1 ASM_REG("$5");
    register u8 *call_arg ASM_REG("$4");
    ...
    if (arg0[0x27] >= 2) {
        ...
        {
            s32 raw;
            register s32 fixed ASM_REG("$4");
            register s32 carrier ASM_REG("$5");
...
#else
        {
            register s32 tail_a0 ASM_REG("$4") = 0x53;
            register u8 *tail_a1_ptr ASM_REG("$5") = arg0;
            register s32 tail_a2 ASM_REG("$6") = count;

            ASM_USE2(tail_a0, tail_a1_ptr);
