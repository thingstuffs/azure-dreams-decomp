...
void BODY_NAME(void *arg0, void *arg1, void * volatile arg2) {
    register u8 *base ASM_REG("$21") = (u8 *)arg0;
    register u8 *in ASM_REG("$22") = (u8 *)arg1;
    register u8 *obj ASM_REG("$20");
    register u8 *node ASM_REG("$23");
    u8 *header;
    register u8 *item ASM_REG("$18");
    register u8 *child ASM_REG("$17");
    ...
    ASM_CLOBBER("$6");
    {
        register s32 original_count ASM_REG("$2");
        obj = (u8 *)S32(base, 0);
        ASM_KEEP_NV(obj);
        original_count = U16(base, 80);
        ASM_KEEP(original_count);
        state = S16(base, 10);
        ASM_KEEP(state);
...
state_other:
        ...
        while (i >= 0) {
            ...
            if (item != 0) {
                ...
                ASM_SCHED_BARRIER();
                {
                    register u8 *callback ASM_REG("$8") = D_800249DC_store;
                    ASM_KEEP_DEP_NV(callback, child);
                ...
                {
                    register s32 init_word ASM_REG("$3") = 0x101010;
                    register s32 flags ASM_REG("$2");
                    register u8 *template ASM_REG("$8");
                    s32 template_word;
                    ASM_KEEP(init_word);
                    S16(child, 30) = 4096;
                    S16(child, 28) = 4096;
    ...
    {
    register s32 mode ASM_REG("$4") = S16(arg0, 10);
    ASM_KEEP_NV(mode);
...
mode_0: {
            s32 found;
            register s32 delta_x ASM_REG("$3");
            register s32 delta_y ASM_REG("$2");
            register s32 abs_y ASM_REG("$4");
            if ((U16(S32(arg0, 4), 0) & 0x80) != 0) {
                ...
                if ((U16(child, 20) & 0x8000) != 0) {
                    register u8 *target_arg2 ASM_REG("$8") = (u8 *)arg2;
                ...
                ASM_SCHED_BARRIER();
                {
                    register u8 *call_base ASM_REG("$4") = arg0 - 32;
                    register u8 *callback ASM_REG("$5") = D_80024798;
                    ASM_KEEP(call_base);
                    ASM_KEEP(callback);
                    ASM_SCHED_BARRIER();
                ...
                i = (s16)func_800BCB04(tmp, value, U16(S32(header, 8), 10) - 48) - 192;
                {
                    ...
                    divisor = S16(arg0, 80);
                    ASM_KEEP_DEP_NV(quotient, divisor);
...
mode_1: {
            ...
            if (S16(arg0, 80) <= 0 && S16(arg0, 82) == 7) {
                register s32 new_count ASM_REG("$3") = 8;
                register s32 tail_state ASM_REG("$2") = U16(arg0, 10);
                ASM_KEEP_DEP_NV(new_count, tail_state);
                ...
                if (S32(obj, 96) != 0) {
                    ...
                    if (item != 0) {
                        ...
                        S32(item, 16) = (s32)D_80024B60;
                        {
                            ...
                            ASM_KEEP_DEP_NV(item, coordinate);
                            entry = item + 32;
                            ASM_KEEP_NV(entry);
