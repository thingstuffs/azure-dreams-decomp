...
void func_8196096C(s32 arg0, Input *arg1, Input *arg2, s32 arg3) {
    ...
    Globals *globals;
    TableEntry *table;
    register Scratch *scratch ASM_REG("$17");
    register void *call_a0;
    register void *call_a1;
    register s32 *mmio ASM_REG("$1");
    register s32 saved_arg0 ASM_REG("$21") = arg0;
    u8 *config;
    u16 tail_value14;
    s32 tail_color;
    s32 shade;
    ...
    if (input != 0) {
        ...
        ASM_KEEP_DEP_NV(record, tail_value14);
        tail_arg = record;
        ASM_KEEP_NV(record);
    ...
    *(volatile u8 *)&record->f4.b[1] = shade;
    ...
    {
        register s32 coord_y ASM_REG("$3");
        ASM_SET(coord_y);
