...
loop:
        {
        Entry *entry;
        register u32 *call0 ASM_REG("$4");
        register u32 *call1 ASM_REG("$5");
        register u32 *call2 ASM_REG("$6");
        register u32 *call3 ASM_REG("$7");
        register u32 *upper_table ASM_REG("$7");
        register u32 value88 ASM_REG("$8");
        s32 masked_index;
        s32 neighbor_index;
        u16 record_half;
        register u16 neighbor_half ASM_REG("$3");
        ...
        ASM_KEEP_NV(neighbor_index);
        neighbor_half = ((u16 *)table_base)[(neighbor_index * 4) + 2];
        ASM_KEEP_NV(value88);
        ASM_JALDELAY_PIN(neighbor_half);
