...
void func_8197CEC0(Actor *actor, Vec3 *target, Sprite *sprite) {
    ...
    case 1:
        ...
        if ((s16)actor->counter >= 0x1C) {
            register u16 actor_value ASM_REG("$2");
            register s32 global_value ASM_REG("$3");
...

    {
        ...
        if (loop_count >= 0) {
            ...
            do {
                register Spawn *spawn ASM_REG("$17");
                spawn = func_8003FD64(0x312, D_80083498);
                if (spawn != 0) {
                    register s32 difference ASM_REG("$2");
                    s32 dx;
                    s32 random;
                    register s32 z_random ASM_REG("$2");
                    register s32 field_c ASM_REG("$5");
                    register s32 base_value ASM_REG("$2");
                    SpawnState *state;
                    register SpawnChild *child ASM_REG("$22");
                    s16 target_y;
                    s16 start_y_signed;
