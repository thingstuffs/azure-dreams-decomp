#include "common.h"
typedef float f32;
typedef double f64;
typedef long long s64;
typedef unsigned long long u64;
#ifndef NULL
#define NULL 0
#endif
/*
 * This header contains macros emitted by m2c in "valid syntax" mode,
 * which can be enabled by passing `--valid-syntax` on the command line.
 *
 * In this mode, unhandled types and expressions are emitted as macros so
 * that the output is compilable without human intervention.
 */


/* Unknown types */
typedef s32 M2C_UNK;
typedef s8  M2C_UNK8;
typedef s16 M2C_UNK16;
typedef s32 M2C_UNK32;
typedef s64 M2C_UNK64;

/* Unknown field access, like `*(type_ptr) &expr->unk_offset` */
#define M2C_FIELD(expr, type_ptr, offset) (*(type_ptr)((s8 *)(expr) + (offset)))

/* Bitwise (reinterpret) cast */
#define M2C_BITWISE(type, expr) ((type)(expr))

/* Unaligned reads */
#define M2C_LWL(expr) (expr)
#define M2C_FIRST3BYTES(expr) (expr)
#define M2C_UNALIGNED32(expr) (expr)

/* Unhandled instructions */
#define M2C_ERROR(desc) (0)
#define M2C_TRAP_IF(cond) (0)
#define M2C_BREAK() (0)
#define M2C_SYNC() (0)

#define GLUE_F64(a, b) (0.0)
#define MULT_HI(a, b) (0)
#define MULTU_HI(a, b) (0)
#define DMULT_HI(a, b) (0)
#define DMULTU_HI(a, b) (0)
#define CLZ(x) (0)
#define REVERSE_BITS(x) (0)
#define ROTATE_RIGHT(x, shift) (0)
#define ARM_RRX(x, carry) (0)
#define BSWAP32(x) (0)
#define BSWAP16(x) (0)
#define BSWAP16X2(x) (0)

/* Carry/overflow bits from partially-implemented instructions */
#define M2C_CARRY 0
#define M2C_OVERFLOW(a) (0)

/* Memcpy patterns */
#define M2C_MEMCPY_ALIGNED memcpy
#define M2C_MEMCPY_UNALIGNED memcpy
#define M2C_STRUCT_COPY memcpy

/* cfail-repair: tf7-phase1-cache-v3 */
extern u8 D_80083498[];
extern u8 D_80045340[];
void *func_8003FD64();                 /* extern */
M2C_UNK func_8004491C();                /* extern */
M2C_UNK func_800A48F0();        /* extern */
s32 func_800A6D30();                          /* extern */
M2C_UNK func_800A9C18(); /* extern */
M2C_UNK func_800AA36C(); /* extern */
M2C_UNK func_800BC318();                      /* extern */
extern M2C_UNK D_8014CA8C;
extern M2C_UNK D_8014CF74;
extern M2C_UNK D_8014FD30;
extern M2C_UNK D_8014FD78;

#ifdef __mips__
static const u32 bank_words[] __asm__("func_8014C800")
    __attribute__((section(".text.func_8014C800"), aligned(4))) = {
    0x8014C854, 0x8014CA8C, 0x8014D358, 0x8014D358,
    0x8014D358, 0x8014D384, 0x8014D304, 0x8014D304,
    0x8014D304, 0x8014D2B0, 0x8014D2E8, 0x8014D384,
    0x8014D384, 0x8014D348, 0x8014EBC4, 0x8014EBBC,
    0x8014EBB4, 0x8014EBCC, 0x8014EB74, 0x8014EB6C,
    0x8014EB64
};
__asm__(".globl func_8014C800\n"
        ".size func_8014C800, 652");
#define BODY_NAME composite_body
#define BODY_STORAGE static
#define BODY_ATTR __attribute__((used, section(".text.func_8014C800")))
#else
#define BODY_NAME func_8014C800
#define BODY_STORAGE
#define BODY_ATTR
#endif

BODY_STORAGE void *BODY_NAME(s16 arg0, s16 arg1, s16 arg2, s16 arg3) BODY_ATTR;
BODY_STORAGE void *BODY_NAME(s16 arg0, s16 arg1, s16 arg2, s16 arg3) {
    s32 unksp20;
    s32 temp_v0_2;
    s32 var_s5;
    void *temp_s0;
    void *temp_s2;
    void *temp_s6;
    void *temp_s3;
    void *temp_v0;
    void *var_s1;

    unksp20 = (s32) *(s8 *)-0x149C;
    var_s1 = NULL;
    var_s5 = 1;
    temp_v0 = func_8003FD64(0x112, D_80083498);
    if (temp_v0 == NULL) {
        goto done;
    } else {
        var_s1 = temp_v0 + 0x20;
        M2C_FIELD(temp_v0, M2C_UNK **, 0x10) = &D_8014CA8C;
        M2C_FIELD(var_s1, s8 *, 0x13) = 0x23;
        func_8004491C(temp_v0, D_80045340);
        temp_s6 = M2C_FIELD(temp_v0, void **, 8);
        M2C_FIELD(temp_s6, s16 *, 0xA) = arg3;
        temp_s2 = M2C_FIELD(temp_v0, void **, 0xC);
        M2C_FIELD(temp_s2, s8 *, 0x25) = arg2;
        temp_s3 = var_s1;
        M2C_FIELD(temp_s2, M2C_UNK **, 0x2C) = &D_8014FD30;
        temp_v0_2 = arg0 & 3;
        M2C_FIELD(temp_s2, s8 *, 0x24) = arg1;
        if (temp_v0_2 == 1) {
            M2C_FIELD(var_s1, s32 *, 0x14) = (s32) (M2C_FIELD(var_s1, s32 *, 0x14) | 0x6000);
            M2C_FIELD(var_s1, s32 *, 0x1C) = (s32) (M2C_FIELD(var_s1, s32 *, 0x1C) | 0x6000);
        } else if (temp_v0_2 >= 2) {
            M2C_FIELD(var_s1, s32 *, 0x14) = (s32) (M2C_FIELD(var_s1, s32 *, 0x14) | 0x2000);
            M2C_FIELD(var_s1, s32 *, 0x1C) = (s32) (M2C_FIELD(var_s1, s32 *, 0x1C) | 0x2000);
        } else if (((arg0 & ~3) << 0x10) == 0) {
            if (M2C_FIELD(var_s1, s32 *, 0x14) & 0x200) {
                goto status_done;
            }
            if (!(func_800A6D30() & 1)) {
                goto status_done;
            }
            M2C_FIELD(var_s1, s32 *, 0x1C) = (s32) (M2C_FIELD(var_s1, s32 *, 0x1C) | 0x200);
            func_800A48F0(var_s1, 1, (func_800A6D30() & 0x3F) | 0x20);
            M2C_FIELD(temp_s2, M2C_UNK **, 0x2C) = &D_8014FD78;
        status_done:
            var_s5 = func_800A6D30() & 3;
        }
        func_800A9C18(temp_v0, temp_s6, temp_s2, arg0);
        M2C_FIELD(temp_s3, u8 *, 0x9A) = 0xFF;
        M2C_FIELD(temp_s3, s8 *, 0x9C) = -1;
        M2C_FIELD(temp_s3, M2C_UNK **, 0x8C) = &D_8014CF74;
        M2C_FIELD(temp_s3, s16 *, 0xA4) = -1;
        if (var_s5 != 0) {
            M2C_FIELD(temp_s3, s16 *, 0xA6) = 0;
        } else {
            temp_s0 = temp_s3 - 0x20;
            M2C_FIELD(temp_s3, s16 *, 0xA6) = 1;
            M2C_FIELD(temp_s2, s16 *, 0x10) = 0x60;
            M2C_FIELD(temp_s2, u16 *, 0x14) = (u16) (M2C_FIELD(temp_s2, u16 *, 0x14) | 0xC);
            M2C_FIELD(temp_s2, u16 *, 0x12) = (u16) (M2C_FIELD(temp_s2, u16 *, 0x12) - 0x80);
            func_80044A50(temp_s0);
            func_800BC318(temp_s0);
        }
        func_800AA36C(temp_s3, temp_s6, temp_s2, var_s1);
        goto done;
    }
done:
    return var_s1;
}
