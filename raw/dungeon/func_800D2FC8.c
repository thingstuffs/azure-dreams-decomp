...
{
    register void *self ASM_REG("$4") = arg0;
    register void *entity ASM_REG("$17") = self;
    register void *motion ASM_REG("$20") = arg1;
    register void *monster ASM_REG("$19") = arg2;
    register void *actor ASM_REG("$18");
    s16 previous_direction;
    register s32 direction_temp ASM_REG("$2");
    ...
    if (!(flags & 0x8000)) {
        ...
        if (FIELD(entity, s16, 0x94) != compare_direction) {
            ...
            if (table != 0) {
                register s32 dir_index ASM_REG("$2") = (s16)compare_direction * 4;
