...
{
    register Actor *actor ASM_REG("$17") = arg0;
    register Motion *motion ASM_REG("$20") = arg1;
    register Entity *entity ASM_REG("$19") = arg2;
    register Actor *subject ASM_REG("$18") = actor;
    register Actor *entryArg0 ASM_REG("$4");
    s16 direction;
    s16 tableIndex;
    s16 oldType;
    register s32 velocity ASM_REG("$4");
    register u32 rawVelocity ASM_REG("$3");
    register s32 velocityLimit ASM_REG("$2");
    ...
    if (!(entityFlags & 0x8000)) {
        ...
        {
            register u16 continuationFlags ASM_REG("$2");
