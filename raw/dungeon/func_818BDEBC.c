...
void func_800256BC(EffectState *arg0, Motion *arg1, register ColorPart *part) {
    ...
    register Motion *motion ASM_REG("$22");
    register void *owner ASM_REG("$23");
    register u8 *color_part ASM_REG("$6") = (u8 *)part;
    register void *meta ASM_REG("$17");
    void *node;
    void *source;
    register void *target ASM_REG("$7");
    ...
    s32 value;
    u32 color;
    register s32 first_delta ASM_REG("$5");
    s16 z;
    u16 ground;
    ...
    register s32 signed_x ASM_REG("$5");
    s32 work_x;
    s32 work_y;
    u16 saved_y;
...
case_0:
    ...
    if (PTR_AT(owner, 0x60) != 0) {
        ...
        {
        register s32 y_delta ASM_REG("$2") = S16_AT(target, 6);
        register s32 y_rhs ASM_REG("$3") = S16_AT(motion, 6);
        ASM_SCHED_BARRIER();
    ...
    ASM_SCHED_BARRIER();
    {
    register void *tile_node ASM_REG("$3");
    ...
    temp_t0 = saved_y;
    out_idx = (s16)state->direction;
    ASM_KEEP_DEP_NV(temp_t0, out_idx);
