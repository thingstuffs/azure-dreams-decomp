...
s32 func_8001EAA4(s8 *arg0, s8 *arg1) {
    ...
    register s32 result_index ASM_REG("$12");
    u32 rng_result;
    register u32 temp_a0 ASM_REG("$4");
    unsigned long var_t2;
    register u8 *group_saved ASM_REG("$11");
    register u8 *temp_a3 ASM_REG("$13");
...
loop_1:
    ...
    if (var_t0 < var_v0) {
        ...
        ASM_KEEP_NV(result_index);
        var_a3 = (u8 *)0x14;
loop_6:
        temp_v1 = *(u16 *)(var_a3 + FIELD(group_saved + ((var_a1 + var_a2) * 4), s32, 0xC));
        if (!(temp_v1 & 0x10)) {
            if (temp_v1 & 0x40) {
                ASM_SCHED_BARRIER();
                var_a1 = var_a2 * 4;
            ...
            {
                var_v0 = *(u16 *)(var_a3 + FIELD(temp_a3, s32, 0xC)) & 0x3000;
                if (var_v0 < 0) {
                    var_v0 += 0xFFF;
                }
