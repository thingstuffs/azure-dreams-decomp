...
s32 func_818E6F98(void *arg0, void *arg1)
{
    ...
    s16 output0[4];
    WorkOutput output1;
    register u8 *uv_source ASM_REG("$19");
    DungeonState **state_address;
    register s16 *output_base ASM_REG("$23");
    register WorkOutput *output1_base ASM_REG("$20");
    s16 *output_cursor;
    s32 i;
    u32 ordering_index;
    s32 width;
    ...
    ASM_KEEP(output_base);

    node = *(s32 *)((u8 *)arg0 - 8);
    if (node != 0) {
        register s32 callback_state ASM_REG("$5");

        arg0 = (u8 *)node + 0x20;
        callback_state = *(s32 *)(node + 8);
        ASM_USE2(arg0, callback_state);
        func_800247D4();
    }
    ASM_MEM_BARRIER();
    {
