...
void func_819602D8(s16 arg0, s32 arg1) {
    ...
    register u32 var_s1 ASM_REG("$17");
    register u32 var_s4 ASM_REG("$20");
    ...
    {
        register u32 store_byte ASM_REG("$7");
        store_byte = subroutine_arg4.b;
        ASM_KEEP4_NV(store_page, var_s4, var_s5, var_s7);
        store_page[0x744C] = store_byte;
    }
    do { *D_8002744D = arg1; } while (0);
loop_1:
    {
        register u32 loop_arg ASM_REG("$7");
        register s32 sign_temp ASM_REG("$2");
        loop_arg = subroutine_arg4.h;
        ASM_KEEP_NV(loop_arg);
        var_s1 = 0;
        sign_temp = var_s5 << 16;
        ASM_KEEP_NV(sign_temp);
        var_s3 = sign_temp >> 16;
        temp_s6 = var_s3 << 6;
        ASM_KEEP_NV(temp_s6);
    ...
    ASM_USE2_NV(var_s0, var_s1);
    ASM_USE2_NV(var_s2, var_s3);
    ASM_USE2_NV(temp_s6, var_s5);
    ASM_USE_NV(var_s4);
loop_2:
    ...
    *var_s2 = 0 - first_result;
    ...
    if ((temp_v0 != NULL) && (temp_v0 != D_8002732C)) {
        M2C_FIELD(temp_v0, s32 *, 0x14) = (s32)(M2C_FIELD(temp_v0, s32 *, 0x14) | 0x100000);
    }
    var_a0 = (s16)var_s0;
    if (var_a0 < 0) {
        goto bounds_fail;
    ...
    {
        register s32 shift0 ASM_REG("$2");
        {
            register s16 *bounds0 ASM_REG("$7") = (s16 *)D_8008333C;
            shift0 = bounds0[10];
            ASM_USE_NV(bounds0);
            ASM_KEEP_NV(shift0);
        }
        {
            register s32 one0 ASM_REG("$7") = 1;
            ASM_KEEP_NV(one0);
            shift0 = one0 << shift0;
        }
    ...
    {
        register s32 shift1 ASM_REG("$2");
        {
            register s16 *bounds1 ASM_REG("$7") = D_8008333C_second;
            shift1 = bounds1[11];
            ASM_USE_NV(bounds1);
            ASM_KEEP_NV(shift1);
        }
        {
            register s32 one1 ASM_REG("$7") = 1;
            ASM_KEEP_NV(one1);
            shift1 = one1 << shift1;
        }
...
bounds_fail:
    {
        ...
        ASM_TAILSLOT_PIN(zero_arg);
        func_80025C14();
    }
bounds_ok:
    {
        register s32 *output_base ASM_REG("$2");
        register u32 output_row ASM_REG("$3");
        register s32 output_value ASM_REG("$4");
        var_a0 <<= 6;
        var_a0 += 0x20;
        var_a0 &= 0xFFE0;
        output_value = func_80025D30(var_a0, temp_s8 & 0xFFFF, -0x400);
        ASM_KEEP_NV(output_value);
        output_base = &D_800274DC[0][0];
        ASM_KEEP_NV(output_base);
        output_row = var_s4 << 5;
        ASM_KEEP_NV(output_row);
        output_row += (u32)output_base;
        ASM_USE_NV(output_row);
