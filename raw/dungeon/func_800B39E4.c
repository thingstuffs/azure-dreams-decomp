...
void func_800B9144(void *arg0, void *arg1, s32 arg2, s16 arg3)
{
    ...
    u8 *s3 = P32(arg1, -0x14);
    register s32 s6 ASM_REG("$22") = S32(arg1, -0x18);
    register s16 saved_arg3 ASM_REG("$23") = arg3;
...

    {
        ...
        register s32 temp0 ASM_REG("$2");
        register s32 temp1 ASM_REG("$3");
...
loop:
    if (!(U8(s2, 0) & 0x20)) {
        ...
        {
            register s32 out0 ASM_REG("$2");
            register s32 out1 ASM_REG("$3");
        ...
        {
            register s32 count ASM_REG("$3") = U16(s3, 0x12);
            if (count != 0) {
                ...
                {
                    register s32 tail_value ASM_REG("$2") = U16(s2, 6);
        ...
        {
            register s32 count ASM_REG("$3") = U16(s3, 0x10);
            if (count != 0) {
                register s32 tail_value ASM_REG("$2") = U16(s2, 4);
                tail_value &= 0xFF9F;
                tail_value = count + tail_value;
                ASM_TAILSLOT_PIN(tail_value);
                func_800B9540();
        ...
        {
            register s32 v ASM_REG("$4") = U8(s2, 1);
            register s32 result ASM_REG("$2");
            register s32 bits ASM_REG("$3");
            ASM_SCHED_BARRIER();
            U8(s3, 0x0F) = v;
        ...
        {
            register u8 *call_record ASM_REG("$5");
            register s32 stored ASM_REG("$2");
            stored = S32(s3, 0x0C);
            ASM_SCHED_BARRIER();
            call_record = s1;
            ASM_KEEP_DEP_NV(call_record, stored);
            ASM_KEEP_NV(s1);
            S32(s1, 4) = stored;
    ...
    if (callback != 0) {
        ...
        if (result > 0) {
            register u32 tag ASM_REG("$2") = 0x80000000;
    ...
    {
        register s32 tail_flags ASM_REG("$2") = saved_arg3 << 16;
        if (tail_flags != 0) {
