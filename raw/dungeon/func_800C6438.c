#include "common.h"
typedef float f32;
typedef double f64;
typedef long long s64;
typedef unsigned long long u64;
#ifndef NULL
#define NULL 0
#endif

typedef s32 M2C_UNK;
typedef s8  M2C_UNK8;
typedef s16 M2C_UNK16;
typedef s32 M2C_UNK32;
typedef s64 M2C_UNK64;

#define M2C_FIELD(expr, type_ptr, offset) (*(type_ptr)((s8 *)(expr) + (offset)))
#define M2C_BITWISE(type, expr) ((type)(expr))
#define M2C_LWL(expr) (expr)
#define M2C_FIRST3BYTES(expr) (expr)
#define M2C_UNALIGNED32(expr) (expr)
#define M2C_ERROR(desc) (0)
#define M2C_TRAP_IF(cond) (0)
#define M2C_BREAK() (0)
#define M2C_SYNC() (0)
#define GLUE_F64(a, b) (0.0)
#define MULT_HI(a, b) (0)
#define MULTU_HI(a, b) (0)
#define DMULT_HI(a, b) (0)
#define DMULTU_HI(a, b) (0)
#define CLZ(x) (0)
#define REVERSE_BITS(x) (0)
#define ROTATE_RIGHT(x, shift) (0)
#define ARM_RRX(x, carry) (0)
#define BSWAP32(x) (0)
#define BSWAP16(x) (0)
#define BSWAP16X2(x) (0)
#define M2C_CARRY 0
#define M2C_OVERFLOW(a) (0)
#define M2C_MEMCPY_ALIGNED memcpy
#define M2C_MEMCPY_UNALIGNED memcpy
#define M2C_STRUCT_COPY memcpy

M2C_UNK func_8003DB94();
void *func_8003FC64();
M2C_UNK func_8004491C();
M2C_UNK func_800A56E0();
extern M2C_UNK D_80045340;
extern u8 D_80083460[0xC];
extern M2C_UNK D_800CB9DC;
extern u8 D_800DF650[];

void *func_800CBB98(s32 arg0, s32 arg1, s16 arg2, void *arg3) {
    register s16 held_arg2 ASM_REG("$20") = arg2;
    register void *held_arg3 ASM_REG("$19") = arg3;
    register s32 held_arg0 ASM_REG("$17") = arg0;
    register s32 held_arg1 ASM_REG("$18") = arg1;
    void *temp_a0;
    void *temp_v0;
    void *temp_v1;
    u8 *global_base;
    u32 t0;
    u32 t1;

    temp_v0 = func_8003FC64(2);
    if (temp_v0 != NULL) {
        M2C_FIELD(temp_v0, M2C_UNK **, 0x10) = &D_800CB9DC;
        func_8004491C(temp_v0, &D_80045340);
        temp_v1 = M2C_FIELD(temp_v0, void **, 8);
        t0 = (u32)held_arg0 & 0xFFFFU;
        M2C_FIELD(temp_v1, s16 *, 2) = (s16) (t0 * 64 + 0x20);
        t1 = (u32)held_arg1 & 0xFFFFU;
        M2C_FIELD(temp_v1, s16 *, 6) = (s16) (t1 * 64 + 0x20);
        M2C_FIELD(temp_v1, s16 *, 0xA) = held_arg2;
        temp_a0 = M2C_FIELD(temp_v0, void **, 0xC);
        M2C_FIELD(temp_a0, s32 *, 0xC) = 0x808080;
        M2C_FIELD(temp_a0, s16 *, 0x1E) = 0x1000;
        M2C_FIELD(temp_a0, s16 *, 0x1C) = 0x1000;
        func_8003DB94(temp_a0, D_800DF650, 0);
        M2C_FIELD(temp_v0, void **, 0x20) = held_arg3;
        M2C_FIELD(held_arg3, s32 *, 0x14) = (s32) (M2C_FIELD(held_arg3, s32 *, 0x14) | 0x100000);
        func_800A56E0(0x614);
        ASM_KEEP(held_arg2);
        ASM_KEEP(held_arg3);
        ASM_KEEP(held_arg0);
        ASM_KEEP(held_arg1);
        global_base = D_80083460;
        M2C_FIELD(global_base, u16 *, 0xA) = (u16) (M2C_FIELD(global_base, u16 *, 0xA) + 1);
    }
    return temp_v0;
}
