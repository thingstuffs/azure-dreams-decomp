...
s32 func_800A0B94(s32 arg0, u8 *arg1, s32 arg2) {
    ...
    u8 *scratch;
    s32 magnitude;
    register s32 raw_magnitude ASM_REG("$2");
    register s32 first_value ASM_REG("$5");
    register s32 shifted_value ASM_REG("$2");
    s32 signed_value;
    register s32 block_size ASM_REG("$6");
    register s32 tile ASM_REG("$4") = arg0;
    register s32 delta ASM_REG("$3");
    register s32 term ASM_REG("$2");
    ...
    if (entry[1] == 0) {
        ...
        ASM_KEEP(first_value);
        if (first_value == 0x38) {
            register s32 call_tag ASM_REG("$7");
            data = D_80164800;
            call_tag = 0x56B1;
            ASM_KEEP(data);
            ASM_KEEP(call_tag);
