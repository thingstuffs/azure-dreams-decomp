...
s32 func_800B4194(s16 arg0, Entity800B4194 *entity, Context800B4194 *context)
{
  ...
  s32 blocked;
  s32 total;
  register s32 flag ASM_REG("$3");
  register u8 *saved ASM_REG("$16");
  u8 *text;
  s32 code;
  s32 delta;
  s16 amount;
  ...
  mode16:
  {
    register s32 modeTest ASM_REG("$2") = blocked;
    ASM_KEEP_NV(modeTest);
    if (modeTest)
    {
      goto common;
    }
  ...
  event3:
  {
  register s32 eventZero ASM_REG("$2") = 0;
  ASM_KEEP(eventZero);
  ...
  event4:
  {
    register s32 eventTest ASM_REG("$2") = blocked;
    ASM_KEEP_NV(eventTest);
  ...
  event5:
  {
    ...
    {
      register s32 low ASM_REG("$4") = 0x10;
      register s32 high ASM_REG("$5") = 0x18;
      ASM_KEEP_NV(low);
      ASM_KEEP_NV(high);
      saved[0] = 1;
    ...
    {
      register s32 low ASM_REG("$4") = 0x50;
      register s32 high ASM_REG("$5") = 0x78;
      ASM_USE2(low, high);
      saved[0] = 2;
      ASM_USE(saved);
  ...
  {
  register s32 eventZero ASM_REG("$2") = 0;
  register s32 eventTest ASM_REG("$2");
  ASM_KEEP(eventZero);
  eventTest = blocked;
  ASM_KEEP_NV(eventTest);
