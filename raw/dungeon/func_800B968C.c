...
s32 func_800BEDEC(u32 arg0, u8 *arg1, s16 arg2, s32 arg3) {
    ...
    register u32 object ASM_REG("$16") = arg0;
    register s32 chain ASM_REG("$17");
    register u8 *other ASM_REG("$18");
    register s32 temp ASM_REG("$19");
    register u8 *item ASM_REG("$20") = arg1;
    ...
    if (other != 0) {
        ...
        if (!(other[3] & 0x40)) {
            s32 next;
            register s32 old_arg ASM_REG("$5");
        ...
        {
            s32 message_value;
            register u8 *message_base ASM_REG("$4");
            register s32 message_arg ASM_REG("$5");

            message_value = func_80099368(other, temp);
            ASM_SCHED_BARRIER();
            message_base = D_800E11AB;
            ASM_KEEP(message_base);
            message_arg = message_value;
            ASM_KEEP(message_arg);
