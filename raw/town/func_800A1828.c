#include "common.h"

#ifndef NULL
#define NULL 0
#endif

#define FIELD(expr, type, offset) (*(type *)((u8 *)(expr) + (offset)))

extern s32 func_8009539C(void *);
extern s32 func_8009EB9C();
extern s32 func_8009F0E8();
extern s32 func_8009F124();
extern s32 func_8009F148();
extern s32 func_800C0C88();
extern s32 D_80083780[];
extern s32 D_800D09C8[];

void func_8009EF88(void *arg0, s32 arg1, void *arg2, s32 arg3) {
    s32 sp18[5];
    s32 *temp_a2;
    s8 *temp_a0;
    s8 *temp_a0_2;
    u16 temp_v0_2;
    u32 temp_v0;
    s32 temp_v1;
    u8 temp_s1;

    temp_a2 = D_80083780;
    FIELD(arg2, s32, 0xC) = (s32)((s32)(D_80083780[0] - FIELD(arg2, s32, 0)) / 2);
    temp_v0 = FIELD(temp_a2, s32, 4) - FIELD(arg2, s32, 4);
    FIELD(arg2, s32, 0x10) = (s32)((s32)(temp_v0 + (temp_v0 >> 31)) >> 1);
    temp_v1 = FIELD(arg2, s32, 8) + 0x200000;
    FIELD(arg2, s32, 0x14) = (s32)((s32)(FIELD(temp_a2, s32, 8) - temp_v1) / 2);
    func_8009539C(arg2);
    temp_v0_2 = FIELD(arg0, u16, 0x6C) - 1;
    FIELD(arg0, u16, 0x6C) = temp_v0_2;
    if ((temp_v0_2 << 16) <= 0) {
        temp_s1 = FIELD(arg0, u8, 0x95);
        if (func_8009EB9C(arg0, arg1, arg2, arg3, sp18) != 0) {
            if (FIELD(arg0, u8, 0x4D) == 0x13) {
                u8 *table = (u8 *)0x80010000;
                s32 index = table[temp_s1 * 4 + 0x983] & 0x3F;
                table[index * 0x54 + 0xAC4] = 1;
                temp_a0 = FIELD(arg0, s8 *, 0x98);
                if (temp_a0 != NULL) {
                    *temp_a0 = 0;
                }
                D_800D09C8[0] = 0;
                func_8009F0E8(temp_a0);
                return;
            }
            FIELD((temp_s1 * 4), s32, 0x80010980) = 0;
            temp_a0_2 = FIELD(arg0, s8 *, 0x98);
            if (temp_a0_2 != NULL) {
                *temp_a0_2 = 0;
            }
            func_800C0C88(temp_a0_2);
            func_8009F124();
            return;
        }
        FIELD(arg2, s32, 0) = 0;
        FIELD(arg2, s32, 4) = 0;
        FIELD(arg2, u16, 2) = FIELD(arg0, u16, 0x84);
        FIELD(arg2, u16, 6) = FIELD(arg0, u16, 0x86);
        func_8009F148(arg0, arg1, arg2, arg3);
    }
}

/* MECHANISM: The 0x48 frame comes from the five-word outgoing stack object and
   four held input roles plus s1. func_8009539C has a one-argument ABI: leaving
   the 0x200000 value in a1 and D_80083780 base in a2 removes the false a1->a3
   preservation copy and yields retail's exact prologue schedule and lifetimes. */
