...
state_done:
    ...
    if (U16_AT(PTR_AT(arg0, 0), 0x64) >= U16_AT(arg0, 0x20)) {
        ...
        register s32 target_y ASM_REG("$3");
        register s32 current_x ASM_REG("$4");
        register s32 current_y ASM_REG("$5");
        register u16 upper_flag ASM_REG("$2");
        ...
        ASM_TAILSLOT_PIN_TIED(upper_flag);
        func_8052E1B8();
    } else {
        register s32 target_const ASM_REG("$4");
        s32 retreat_x;
        register s32 retreat_y ASM_REG("$2");
        s32 delta_x;
