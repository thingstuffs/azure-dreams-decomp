    ...
    while (sc->outer_count < 2) {
        ...
        while (sc->inner_count < 2) {
...

            {
                ...
                yq = scan_y;
                { register u16 xm ASM_REG("$3"); register s32 xs ASM_REG("$2"); xm = map->x_mask; xs = xq >> 6; sc->cell = xm & xs; }
                if (yq < 0) {
                    yq += 0x3F;
                }
