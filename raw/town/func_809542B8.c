...
s32 func_800212B8(void) {
    ...
    {
        s32 loop_i;
        register Init10 *loop_init10 ASM_REG("$19");
        init10.h14 = 0x14;
        init10.h18 = 3;
        init10.h1C = 0;
        init10.h1A = 0x7C80;
        ...
        loop_i = 4;
        loop_init10 = &init10;
        ASM_KEEP_NV(loop_init10);
        do {
            register u8 *a0v ASM_REG("$4");
            register Init10 *a1v ASM_REG("$5");
            a0v = D_800225F0;
            a1v = loop_init10;
            ASM_KEEP_DEP_NV(a1v, a0v);
    ...
    {
        register s32 loop_i ASM_REG("$18") = 4;
        register s32 copy_page ASM_REG("$20") = (s32)0x80020000;
        register s32 loop_offset ASM_REG("$19") = 0x38;
        register s16 loop_value ASM_REG("$17") = 0x44;
        register Packed8 *packed_v0 ASM_REG("$2") = D_80024310;
        register Packed8 *loop_packed ASM_REG("$16") = packed_v0 + 4;
        register s32 call_page ASM_REG("$4");
        ASM_KEEP(loop_i);
        ASM_KEEP(loop_offset);
        ASM_KEEP(loop_value);
        ASM_KEEP(loop_packed);
        do {
            register Packed8 *copy_src ASM_REG("$5");
            ...
            *loop_packed = *copy_src;
            ...
            ASM_KEEP_DEP_NV(call_page, copy_page);
    ...
    {
        s32 loop_i = 7;
        register s32 asset_v0 ASM_REG("$2") = (s32)0x80020000;
        u8 *loop_asset;
        register s16 loop_scale ASM_REG("$17");
        u8 *loop_sprite;
        ASM_KEEP(loop_i);
        ASM_KEEP(asset_v0);
    ...
    {
        register u8 *ret_v0 ASM_REG("$2");
        register u8 *obj_s0 ASM_REG("$16");
        register s32 loop_i ASM_REG("$18");
        u8 *loop_state;
        register s16 loop_value ASM_REG("$21");
        register u8 *loop_tablep ASM_REG("$20");
        loop_state = D_80082E80;
        loop_i = 2;
        ASM_KEEP_DEP_NV(loop_i, loop_state);
        loop_value = 0xC;
        ASM_KEEP_NV(loop_value);
        loop_tablep = owner + 8;
        ASM_KEEP_NV(loop_tablep);
        ASM_USE(loop_state);
        do {
