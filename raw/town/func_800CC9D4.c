#include "common.h"
typedef float f32;
typedef double f64;
typedef long long s64;
typedef unsigned long long u64;
#ifndef NULL
#define NULL 0
#endif
/*
 * This header contains macros emitted by m2c in "valid syntax" mode,
 * which can be enabled by passing `--valid-syntax` on the command line.
 *
 * In this mode, unhandled types and expressions are emitted as macros so
 * that the output is compilable without human intervention.
 */


/* Unknown types */
typedef s32 M2C_UNK;
typedef s8  M2C_UNK8;
typedef s16 M2C_UNK16;
typedef s32 M2C_UNK32;
typedef s64 M2C_UNK64;

/* Unknown field access, like `*(type_ptr) &expr->unk_offset` */
#define M2C_FIELD(expr, type_ptr, offset) (*(type_ptr)((s8 *)(expr) + (offset)))

/* Bitwise (reinterpret) cast */
#define M2C_BITWISE(type, expr) ((type)(expr))

/* Unaligned reads */
#define M2C_LWL(expr) (expr)
#define M2C_FIRST3BYTES(expr) (expr)
#define M2C_UNALIGNED32(expr) (expr)

/* Unhandled instructions */
#define M2C_ERROR(desc) (0)
#define M2C_TRAP_IF(cond) (0)
#define M2C_BREAK() (0)
#define M2C_SYNC() (0)

#define GLUE_F64(a, b) (0.0)
#define MULT_HI(a, b) (0)
#define MULTU_HI(a, b) (0)
#define DMULT_HI(a, b) (0)
#define DMULTU_HI(a, b) (0)
#define CLZ(x) (0)
#define REVERSE_BITS(x) (0)
#define ROTATE_RIGHT(x, shift) (0)
#define ARM_RRX(x, carry) (0)
#define BSWAP32(x) (0)
#define BSWAP16(x) (0)
#define BSWAP16X2(x) (0)

/* Carry/overflow bits from partially-implemented instructions */
#define M2C_CARRY 0
#define M2C_OVERFLOW(a) (0)

/* Memcpy patterns */
#define M2C_MEMCPY_ALIGNED memcpy
#define M2C_MEMCPY_UNALIGNED memcpy
#define M2C_STRUCT_COPY memcpy

M2C_UNK func_800CA204();                            /* extern */
M2C_UNK func_800CA75C();                            /* extern */
extern u8 D_80083498[];

void func_800CA134(void *arg0, void *arg1) {
    u8 *base;
    s32 var_v0;
    s32 var_v0_2;
    u16 temp_v0;

    temp_v0 = M2C_FIELD(arg0, u16 *, 0x6C) - 1;
    M2C_FIELD(arg0, u16 *, 0x6C) = temp_v0;
    if ((temp_v0 << 0x10) <= 0) {
        func_800CA75C();
        func_800CA204();
        return;
    }
    base = D_80083498;
    var_v0 = M2C_FIELD(M2C_FIELD(base, void * volatile *, 8), s16 *, 2) - M2C_FIELD(arg1, s16 *, 2);
    if (var_v0 < 0) {
        var_v0 += 3;
    }
    M2C_FIELD(arg1, s16 *, 2) = (s16) ((u16) M2C_FIELD(arg1, s16 *, 2) + (var_v0 >> 2));
    var_v0_2 = M2C_FIELD(M2C_FIELD(base, void * volatile *, 8), s16 *, 6) - M2C_FIELD(arg1, s16 *, 6);
    if (var_v0_2 < 0) {
        var_v0_2 += 3;
    }
    M2C_FIELD(arg1, s16 *, 6) = (s16) ((u16) M2C_FIELD(arg1, s16 *, 6) + (var_v0_2 >> 2));
    {
        register void *target ASM_REG("$3");
        register s32 current ASM_REG("$2");
        register s32 target_value ASM_REG("$3");
        register u32 current_u ASM_REG("$3");

        target = M2C_FIELD(base, void * volatile *, 8);
        ASM_KEEP(target);
        current = M2C_FIELD(arg1, s16 *, 0xA);
        ASM_KEEP(current);
        target_value = M2C_FIELD(target, s16 *, 0xA);
        current += 0xD0;
        current = target_value - current;
        ASM_KEEP(current);
        current_u = M2C_FIELD(arg1, volatile u16 *, 0xA);
        if (current < 0) {
            current += 3;
        }
        current = current_u + (current >> 2);
        ASM_KEEP(current);
        M2C_FIELD(arg1, s16 *, 0xA) = (s16) current;
    }
}

/* MECHANISM: The retail 0x18 frame has only the $ra save; the first two interpolation
   regions fall out naturally from block-local D_80083498 liveness. The third reuses
   $v1 for pointer/target/reread and $v0 for current/difference/final sum; guarded
   pins plus a volatile final u16 reread preserve that exact load-delay schedule. */
