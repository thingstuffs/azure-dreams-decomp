...
s32 func_8008DFB4(s16 arg0, u16 arg1, u16 arg2)
{
    ...
    register s16 scan_z ASM_REG("$22");
    register s16 scan_x ASM_REG("$7");
    ...
    if ((s16)scratch->x >= 0x20) {
        register s32 z_pos ASM_REG("$2");

        z_pos = 0x40;
        scratch->z_step = z_pos;
    } else {
        register s32 z_neg ASM_REG("$2");
...
inner_loop:
    {
        ...
        if (scratch->x_step >= 0) {
            ASM_KEEP(scan_x);
            if (sum_x < D_800FE480) {
                register s32 e164_arg ASM_REG("$4");
                register s32 e164_hi ASM_REG("$2");

                e164_hi = (s32)((u32)(u16)scan_x << 16);
                e164_arg = e164_hi >> 16;
                ASM_TAILSLOT_PIN(e164_arg);
                func_8008E164();
            }
            goto next_outer;
        }
...

    {
        s16 x_part;
        u16 tile;
        register s32 sx ASM_REG("$4");
        register s32 sx_hi ASM_REG("$2");
        ...
        if (tiles[scratch->tile] != 0) {
            register Cell_80090854 *cell ASM_REG("$18");
            register u8 *tail ASM_REG("$17");
            u16 tile_value;
            register s32 tmp ASM_REG("$3");
            ...
            if (scratch->planes[*(u16 *)(tail - 6)].x > 0 &&
                ...
                if (func_8008CE08(scratch) != 0) {
                    u16 pi = *(u16 *)(tail - 6);
                    u16 ci = cell->v0;
                    register Vec_80090854 *plane ASM_REG("$6");
                    register Vec_80090854 *vertex ASM_REG("$5");
                    s32 dy;
                    s32 dz;
                    s32 y_product;
                    s32 z_product;
            ...
            {
            register u16 fl ASM_REG("$3");
            fl = *(u16 *)tail & 0x80FF;
            if (fl != 0x8001) {
                tail += 0x18;
                ASM_KEEP(tail);
                cell = (Cell_80090854 *)((u8 *)cell + 0x18);
                ASM_TAILSLOT_PIN(cell);
                func_8008E204();
            }
            }
        }
