...
void func_80057D20(u8 idx, u8 type, u32 val)
{
  ...
  goto *jtbl_80032F04[sel];
  {
    ...
    L_case_7:
    {
      register s32 call_flag ASM_REG("$7");
      e->f0c = new_var & 0xFF;
      call_flag = 1;
      ASM_KEEP(call_flag);
            ...
            {
              {
                ...
                register s32 acc ASM_REG("$7");
                v = D_80073740[i];
                acc = flag18;
                ASM_KEEP_DEP_NV(acc, v);
                acc |= v;
                flag18 = acc;
              }
              do
  ...
  {
    register s32 pending_flag ASM_REG("$7") = flag18;
    ASM_KEEP(pending_flag);
    if (pending_flag)
    {
  ...
  {
    register s32 pending_flag ASM_REG("$7") = flag14;
    ASM_KEEP(pending_flag);
    if (pending_flag)
    {
      for (k = 0; k < D_80073734; k++)
