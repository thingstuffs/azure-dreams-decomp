...
{
  register S_80042710 *src ASM_REG("$10") = a1;
  ...
  PIN_BARRIER(first);
  {
  register S_80042710 *dest ASM_REG("$9") = a0;
  PIN_KEEP(dest);
  dest->f00 = first;
  dest->f01 = src->f01;
  dest->f02 = src->f02;
  ...
  i = 2;
  {
    register u8 *sp ASM_REG("$4") = (u8 *)src + 6;
    register u8 *dp ASM_REG("$3") = (u8 *)dest + 6;
    do
    {
      dp[8] = sp[8];
      dp[10] = sp[10];
  ...
  i = 3;
  {
    register s8 *sp2 ASM_REG("$8") = (s8 *)src + 6;
    register s8 *dp2 ASM_REG("$7") = (s8 *)dest + 6;
  ...
  dest->f45 = src->f45;
  {
    ...
    {
      register S_80042710 *call_arg ASM_REG("$4") = dest;
      PIN_KEEP(call_arg);
