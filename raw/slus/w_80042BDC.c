...
L_10: {
    ...
    check_2E:
        ...
        ASM_KEEP_NV(a1v);
        {
        register S_80042BDC *r ASM_REG("$2") = func_800A1618(ent->x13, a1v);
        a2v = 0x3000;
        {
            register u32 bit ASM_REG("$3") = 0x10000;
            ASM_KEEP(bit);
            ent->x1C |= bit;
        }
        s0 = r;
        ...
        flag = (ent->x14 >> 14) & 1;
        {
            register int a24 ASM_REG("$2") = s2->x24;
            int a25 = s2->x25;
        ...
        {
        register int f2 ASM_REG("$2") = flag2;
        if (f2) {
            register S_80042BDC *p4 ASM_REG("$4") = s0;
            u8 *w;
            ASM_KEEP_NV(p4);
            w = D_800E3D7C[0];
