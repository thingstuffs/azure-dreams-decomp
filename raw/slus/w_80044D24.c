...
void func_80044D24(void *arg0, void *arg1, s32 arg2)
{
    ...
    u8 *context;
    u8 *script;
    register void *matrix ASM_REG("$4");
    register void *translation ASM_REG("$5");
    register void *record ASM_REG("$18");
    register void *packet ASM_REG("$17");
    s32 depth;
    s16 x;
    s16 y;
    u16 flags;
    ...
    for (;;) {
        if (!(script[0] & 0x20)) {
            ...
            if ((script[0] ^ U16_AT(scratch, 0x24)) & 1) {
                ...
                {
                    register s32 neg ASM_REG("$3") = -value;
                    ASM_KEEP_NV(neg);
                ...
                u16 doubled;
                s32 raw;
                register s32 combined ASM_REG("$2");
                s32 value;
                raw = U8_AT(record, 1);
                raw <<= 24;
                doubled = U16_AT(entry, 0x14) & 0x400;
                ASM_KEEP_DEP_NV(raw, doubled);
                value = raw >> 24;
                if (doubled) {
                    value *= 2;
                }
            ...
            U16_AT(entry, 0x14) &= 0x7FFF;
            {
                ...
                register s32 base_x ASM_REG("$4") = U32_AT(scratch, 8);
                register s32 sum_y ASM_REG("$2") = U32_AT(scratch, 0x14);
            ...
            {
                u8 code;
                register u16 local_flags ASM_REG("$3");
                code = U8_AT(record, 0);
                entry[0xF] = code;
                ASM_USE(code);
                local_flags = U16_AT(scratch, 0x24);
                if (local_flags & 8) {
                    register u8 adjusted ASM_REG("$2");
                    if (local_flags & 4) {
                        adjusted = code | 2;
                    } else {
                        adjusted = code & 0xFD;
                    }
                    ASM_KEEP_DEP_NV(adjusted, local_flags);
                    entry[0xF] = adjusted;
                }
            }
