...
void func_8003CCB0(s32 arg0)
{
    ...
    register u8 *packet ASM_REG("$17");
    register u8 *record ASM_REG("$19");
    register u8 *prim ASM_REG("$21");
    register u8 *flag ASM_REG("$22");
    ...
    s32 x;
    s32 y;
    register s32 off ASM_REG("$4");
    register s32 shade ASM_REG("$4");
    register s32 colour ASM_REG("$3");
    register u8 *in0 ASM_REG("$4");
    register u8 *out0 ASM_REG("$5");
    register u8 *arg2 ASM_REG("$6");
    ...
    PushMatrix();
    {
        register u8 *rec0 ASM_REG("$3") = (u8 *)D_80080AB0;
        record = rec0 + 4;
    }
    ...
    do {
        ...
        {
            register s32 sum_x ASM_REG("$2") = U32_AT(scratch, 0x10);
            register s32 base_x ASM_REG("$4") = U32_AT(scratch, 0x08);
            register s32 sum_y ASM_REG("$3") = U32_AT(scratch, 0x14);
            register s32 base_y ASM_REG("$5") = U32_AT(scratch, 0x0C);
            sum_x += base_x;
            sum_y += base_y;
        ...
        {
            register s32 tmpv ASM_REG("$2") = (rsin((D_80080AA4 << 8) + (i << 6)) >> 6) + 0x80;
            register s32 half ASM_REG("$3") = arg0 / 2;
            shade = tmpv >> half;
        }
