...
void func_8004C36C(u8 *parent, u8 *node)
{
    u8 buf[0x110];
    s32 count;
    register u8 *L ASM_REG("$17");
    register u8 *packet ASM_REG("$18");
    register u8 *m7 ASM_REG("$23");
    register u8 *m5 ASM_REG("$21");
    register u32 mask_lo ASM_REG("$20");
    register u32 mask_hi ASM_REG("$22");
    register u8 *cmd ASM_REG("$16");
    register u8 *m0 ASM_REG("$16");
    register u8 *rot0 ASM_REG("$4");
    u8 *ctx;
    u8 **root;
...
command:
    ...
    if (U32_AT(L, 8) != 0) {
        if (U8_AT(L, 0) & 1) {
            ...
            u32 b = U16_AT(L, 0xB0);
            u32 d = U16_AT(L, 0x20);
            ASM_KEEP_DEP_NV(raw, b);
            ASM_KEEP_DEP_NV(raw, d);
    ...
    {
        register s32 grp ASM_REG("$3") = U8_AT(L, 1) & 0xFC;
    ...
    }
    {
        ...
        {
            register u8 c1 ASM_REG("$3") = U8_AT(L, 1);
            register u8 c8 ASM_REG("$4") = U8_AT(L, 8);
            register s32 p8 ASM_REG("$5") = S16_AT(packet, 8);
            register s32 p10 ASM_REG("$2") = S16_AT(packet, 0x10);
            U8_AT(packet, 7) = c1;
            U8_AT(packet, 0xC) = c8;
            {
                register u16 c6 ASM_REG("$3") = U16_AT(L, 6);
                register u8 c9 ASM_REG("$4") = U8_AT(L, 9);
        ...
        {
            register u8 c8 ASM_REG("$3") = U8_AT(L, 8);
            register u8 ca ASM_REG("$2") = U8_AT(L, 0xA);
            u8 c9 = U8_AT(L, 9);
            s32 pa = S16_AT(packet, 0xA);
            register u8 sum ASM_REG("$3") = c8 + ca;
            U8_AT(packet, 0x24) = sum;
            U8_AT(packet, 0x14) = sum;
            U8_AT(packet, 0x15) = c9;
            {
                register u8 d8 ASM_REG("$4") = U8_AT(L, 8);
                register s32 p1a ASM_REG("$2") = S16_AT(packet, 0x1A);
                register u16 c4 ASM_REG("$3");
                ASM_KEEP_DEP_NV(p1a, d8);
                c4 = U16_AT(L, 4);
        ...
        {
            register u8 e9 ASM_REG("$3") = U8_AT(L, 9);
            register u8 eb ASM_REG("$2") = U8_AT(L, 0xB);
            register u8 sum2 ASM_REG("$3") = e9 + eb;
...
tail:
    {
        register u8 *nx ASM_REG("$5") = *(u8 **)(parent - 8);
        if (nx != 0) {
            parent = nx + 0x20;
            node = *(u8 **)(nx + 0xC);
            goto top;
